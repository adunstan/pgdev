...
def test_001_repl_stats(create_pg):
    ...
    # Insert some data.
    node.safe_sql("INSERT INTO test_repl_stat values(generate_series(1, 5));")
    ...
    node.safe_sql("SELECT pg_drop_replication_slot('regression_slot4')")
    ...
    node.safe_sql("SELECT pg_drop_replication_slot('regression_slot1')")
    node.safe_sql("SELECT pg_drop_replication_slot('regression_slot2')")
    ...
    node.safe_sql(f"SELECT pg_drop_replication_slot('{slot_name_restart}')")
