...
def invalid_entry_order_inner_page_test(node, blksize):
    ...
    string_replace_block(relpath, re.compile(b"rrrrrrrrrr"), b"zzzzzzzzzz", blkno, blksize)
...
def inconsistent_with_parent_key__parent_key_corrupted_test(node, blksize):
    ...
    string_replace_block(relpath, re.compile(b"nnnnnnnnnn"), b"aaaaaaaaaa", blkno, blksize)
...
def inconsistent_with_parent_key__child_key_corrupted_test(node, blksize):
    ...
    string_replace_block(relpath, re.compile(b"nnnnnnnnnn"), b"pppppppppp", blkno, blksize)
