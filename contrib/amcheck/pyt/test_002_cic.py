...
def test_002_cic(create_pg, tmp_path):
    ...
    node.append_conf("lock_timeout = " + str(1000 * TIMEOUT_DEFAULT) + "\n")
    node.start()
    ...
    node.command_checks_all(
        ["pgbench", "--no-vacuum", "--client=5", "--transactions=100", *file_opts],
        0,
    ...
    node.safe_sql("INSERT INTO quebec SELECT i FROM generate_series(1, 2) s(i);")
