...
KERBEROS_ENABLED = bool(re.search(r"\bkerberos\b", os.environ.get("PG_TEST_EXTRA", "")))
...
def _expand_expected_line(user, gssencmode, sslmode, sslnegotiation, expected):
    ...
    if user == "*":
        for x in ALL_TEST_USERS:
            result.update(
                _expand_expected_line(x, gssencmode, sslmode, sslnegotiation, expected)
            )
    elif gssencmode == "*":
        for x in ALL_GSSENCMODES:
            result.update(
                _expand_expected_line(user, x, sslmode, sslnegotiation, expected)
            )
    elif sslmode == "*":
        for x in ALL_SSLMODES:
            result.update(
                _expand_expected_line(user, gssencmode, x, sslnegotiation, expected)
            )
    elif sslnegotiation == "*":
        for x in ALL_SSLNEGOTIATIONS:
            result.update(_expand_expected_line(user, gssencmode, sslmode, x, expected))
    else:
        result[f"{user} {gssencmode} {sslmode} {sslnegotiation}"] = expected
    return result


_LINE_RE = re.compile(r"^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S.*)\s*->\s*(\S+)\s*$")
...
def _test_matrix(harness, test_users, gssencmodes, sslmodes, sslnegotiations, expected):
...
"""
    ...
    with open(os.path.join(pgdata, "pg_hba.conf"), "w", encoding="utf-8") as fh:
        fh.write(hba)
    node.reload()
...
"""
        ...
        if injection_points_supported:
            admin_psql("SELECT injection_points_attach('backend-initialize', 'error');")
            harness.connect_test(
                "user=testuser sslmode=prefer", "connect, backenderror -> fail"
            )
            node.restart()
            ...
            harness.connect_test(
                "user=testuser sslmode=prefer", "connect, v2error -> fail"
            )
            node.restart()
