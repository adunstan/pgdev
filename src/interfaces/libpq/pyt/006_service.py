...
def service_setup(pg, tmp_path):
    ...
    with open(srvfile_nested, "a", encoding="utf-8") as fh:
    ...
    with open(srvfile_nested_2, "a", encoding="utf-8") as fh:
...
def test_service_with_pgservicefile(service_setup):
    ...
    _connect_ok(node, _kw_user(""), "connect1_3", **dict(env, PGSERVICE="my_srv"))
    _connect_fails(
        node,
        _kw_user(""),
        r'definition of service "undefined-service" not found',
...
def test_service_with_default_pg_service_conf(service_setup):
    ...
    try:
        ...
        _connect_ok(node, _uri_user("postgres://?service=my_srv"), "connect2_2", **env)
        _connect_fails(
            node,
            _kw_user("service=undefined-service"),
            r'definition of service "undefined-service" not found',
        ...
        _connect_ok(node, _kw_user(""), "connect2_3", **dict(env, PGSERVICE="my_srv"))
