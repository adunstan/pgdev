...
"""
    ...
    def _connstr(role, gssencmode):
        ...
        # it explicitly here.
        cs = (f"user={role} host={host} hostaddr={hostaddr} "
              f"application_name={application}")
        if gssencmode:
            cs += f" {gssencmode}"
        return cs

    def test_access(role, query, expected_res, gssencmode, test_name,
                    *expect_log_msgs):
        ...
        if expected_res == 0:
            ...
            node.connect_ok(connstr, test_name, sql=query,
                            expected_stdout=r"^t$", log_like=log_like)
        else:
    ...
    test_access(
        "test1", "SELECT true", 2, "", "fails without mapping",
    ...
    test_access(
        ...
        "SELECT gss_authenticated AND encrypted AND NOT credentials_delegated "
        "FROM pg_stat_gssapi WHERE pid = pg_backend_pid();",
        0, "gssencmode=prefer gssdelegation=1",
    ...
    test_access(
        ...
        "SELECT gss_authenticated AND encrypted AND NOT credentials_delegated "
        "FROM pg_stat_gssapi WHERE pid = pg_backend_pid();",
        0, "gssencmode=require gssdelegation=1",
    ...
    test_query("test1", big_send, r"(?s)^100000$", "gssencmode=require",
    ...
    test_query("test1", "SELECT SYSTEM_USER;",
               rf"(?s)^gss:test1@{realm}$", "gssencmode=require",
               "testing system_user")
    ...
    test_access(
        ...
        "SELECT gss_authenticated AND encrypted AND NOT credentials_delegated "
        "FROM pg_stat_gssapi WHERE pid = pg_backend_pid();",
        0, "gssencmode=require gssdelegation=0",
        "succeeds with GSS-encrypted access required and hostgssenc hba and "
    ...
    assert re.search(no_deleg_re, stderr), \
    ...
    test_access("test1", "SELECT true", 2, "gssencmode=disable",
    ...
    test_access("test1", "SELECT true", 2, "gssencmode=require",
                "fails with GSS-encrypted access required and hostnogssenc hba")
    test_access(
        "test1",
        "SELECT gss_authenticated AND NOT encrypted AND credentials_delegated "
        "FROM pg_stat_gssapi WHERE pid = pg_backend_pid();",
        0, "gssencmode=disable gssdelegation=1",
    ...
    open(os.path.join(node.data_dir, "pg_ident.conf"), "w").close()
    set_hba(
        "    local all test2 scram-sha-256",
