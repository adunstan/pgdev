...
def expect_replication(node_publisher, node_subscriber, tbl, cnt, mn, mx, testname):
    ...
    result = node_subscriber.safe_sql(
        f"SELECT COUNT(i), MIN(i), MAX(i) FROM {tbl}")
...
def test_027_nosuperuser(create_pg):
    global publisher_connstr, offset
...
""")
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 3, 7, 11,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 3, 9, 13,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 2, 11, 13,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.hashpart", 2, 103, 120,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 3, 11, 15,
    ...
    node_subscriber.safe_sql(
        "ALTER TABLE alice.unpartitioned NO FORCE ROW LEVEL SECURITY")
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 3, 13, 17,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 4, 13, 19,
    ...
    expect_replication(
        node_publisher, node_subscriber, "alice.unpartitioned", 3, 17, 21,
