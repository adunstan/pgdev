...
def test_036_sequences(create_pg):
    ...
    node_publisher.safe_sql("CREATE PUBLICATION regress_seq_pub FOR ALL SEQUENCES")
    node_subscriber.safe_sql(
    ...
    result = node_subscriber.safe_sql("SELECT last_value, is_called FROM regress_s1;")
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION regress_seq_sub REFRESH PUBLICATION;")
    assert node_subscriber.poll_query_until(
        synced_query
    ), "Timed out while waiting for subscriber to synchronize data"

    result = node_publisher.safe_sql("SELECT last_value, is_called FROM regress_s1;")
    assert result == "200|t", "Check sequence value in the publisher"

    # Check - existing sequence ('regress_s1') is not synced
    result = node_subscriber.safe_sql("SELECT last_value, is_called FROM regress_s1;")
    assert result == "100|t", "REFRESH PUBLICATION will not sync existing sequence"

    # Check - newly published sequence ('regress_s2') is synced
    result = node_subscriber.safe_sql("SELECT last_value, is_called FROM regress_s2;")
    assert result == "100|t", "REFRESH PUBLICATION will sync newly published sequence"
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION regress_seq_sub REFRESH SEQUENCES;")
    assert node_subscriber.poll_query_until(
        synced_query
    ), "Timed out while waiting for subscriber to synchronize data"
    ...
    result = node_subscriber.safe_sql("SELECT last_value, is_called FROM regress_s3;")
    assert result == "1|f", "REFRESH SEQUENCES will not sync newly published sequence"
    ...
    node_publisher.safe_sql("CREATE SEQUENCE regress_s4 START 1 INCREMENT 2;")

    node_subscriber.safe_sql("CREATE SEQUENCE regress_s4 START 10 INCREMENT 2;")
    ...
    node_publisher.safe_sql("CREATE SEQUENCE regress_s4 START 10 INCREMENT 2;")
