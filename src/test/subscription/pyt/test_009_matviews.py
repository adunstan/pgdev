...
def test_009_matviews(create_pg):
    ...
    node_publisher.safe_sql("CREATE TABLE test1 (a int PRIMARY KEY, b text)")
    node_subscriber.safe_sql("CREATE TABLE test1 (a int PRIMARY KEY, b text);")
    ...
    node_publisher.safe_sql("CREATE MATERIALIZED VIEW testmv1 AS SELECT * FROM test1;")
    node_publisher.wait_for_catchup("mysub")
