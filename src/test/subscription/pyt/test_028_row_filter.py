...
def test_028_row_filter(create_pg):
    ...
    node_publisher.safe_sql(
        "CREATE TABLE public.tab_rf_partition "
        "(LIKE schema_rf_x.tab_rf_partitioned)")
    node_publisher.safe_sql(
    ...
    # create pub/sub
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_x FOR TABLE schema_rf_x.tab_rf_x "
        "WHERE (x > 10)")
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_greater_10k "
        "(LIKE tab_rowfilter_partitioned)")
    node_publisher.safe_sql(
        "ALTER TABLE tab_rowfilter_partitioned ATTACH PARTITION "
    ...
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_partition "
        "(LIKE tab_rowfilter_partitioned_2)")
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "ALTER PUBLICATION tap_pub_1 ADD TABLE tab_rowfilter_2 "
        "WHERE (c % 7 = 0)")
    ...
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_2 FOR TABLE tab_rowfilter_2 "
        "WHERE (c % 3 = 0)")
    ...
    node_publisher.safe_sql(
        ...
        "WHERE (a < 6000)")
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_not_used FOR TABLE tab_rowfilter_1 "
        "WHERE (a < 0)")

    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_4a FOR TABLE tab_rowfilter_4 "
        "WHERE (c % 2 = 0)")
    node_publisher.safe_sql(
