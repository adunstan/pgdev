...
def test_028_row_filter(create_pg):
    ...
    node_publisher.safe_sql("CREATE TABLE tab_rf_x (x int primary key)")
    node_subscriber.safe_sql("CREATE TABLE tab_rf_x (x int primary key)")
    ...
    node_publisher.safe_sql("CREATE PUBLICATION tap_pub_forall FOR ALL TABLES")
    ...
    # The subscription of the FOR ALL TABLES publication means there should be
    # no filtering on the tablesync COPY, so all expect all 5 will be present.
    result = node_subscriber.safe_sql("SELECT count(x) FROM tab_rf_x")
    assert (
        result == "5"
    ...
    node_publisher.safe_sql("INSERT INTO tab_rf_x (x) VALUES (-99), (99)")
    node_publisher.wait_for_catchup(appname)
    result = node_subscriber.safe_sql("SELECT count(x) FROM tab_rf_x")
    assert result == "7", "check table tab_rf_x should not be filtered"

    # cleanup pub
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_forall")
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_x")
    node_publisher.safe_sql("DROP TABLE tab_rf_x")
    # cleanup sub
    node_subscriber.safe_sql("DROP SUBSCRIPTION tap_sub")
    node_subscriber.safe_sql("DROP TABLE tab_rf_x")
    ...
    # create tables pub and sub
    node_publisher.safe_sql("CREATE SCHEMA schema_rf_x")
    node_publisher.safe_sql("CREATE TABLE schema_rf_x.tab_rf_x (x int primary key)")
    node_publisher.safe_sql(
        "CREATE TABLE schema_rf_x.tab_rf_partitioned (x int primary key) "
    ...
    node_publisher.safe_sql(
        "ALTER TABLE schema_rf_x.tab_rf_partitioned ATTACH PARTITION "
        "public.tab_rf_partition DEFAULT"
    )
    node_subscriber.safe_sql("CREATE SCHEMA schema_rf_x")
    node_subscriber.safe_sql("CREATE TABLE schema_rf_x.tab_rf_x (x int primary key)")
    node_subscriber.safe_sql(
        "CREATE TABLE schema_rf_x.tab_rf_partitioned (x int primary key) "
        "PARTITION BY RANGE(x)"
    )
    ...
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_allinschema FOR TABLES IN SCHEMA "
        "schema_rf_x, TABLE schema_rf_x.tab_rf_x WHERE (x > 10)"
    )
    node_publisher.safe_sql(
        "ALTER PUBLICATION tap_pub_allinschema ADD TABLE "
        "public.tab_rf_partition WHERE (x > 10)"
    )
    node_subscriber.safe_sql(
        f"CREATE SUBSCRIPTION tap_sub CONNECTION "
        f"'{publisher_connstr} application_name={appname}' "
    ...
    result = node_subscriber.safe_sql("SELECT count(x) FROM schema_rf_x.tab_rf_x")
    assert (
        result == "5"
    ), "check initial data copy from table tab_rf_x should not be filtered"
    ...
    # the tab_rf_partition does work because that partition belongs to a
    # different schema (and publish_via_partition_root = false).
    # Expected:
    #     tab_rf_x          :  5 initial rows + 2 new rows = 7 rows
    #     tab_rf_partition  :  1 initial row  + 1 new row  = 2 rows
    node_publisher.safe_sql("INSERT INTO schema_rf_x.tab_rf_x (x) VALUES (-99), (99)")
    node_publisher.safe_sql(
    ...
    result = node_subscriber.safe_sql("SELECT count(x) FROM schema_rf_x.tab_rf_x")
    assert result == "7", "check table tab_rf_x should not be filtered"
    result = node_subscriber.safe_sql("SELECT * FROM public.tab_rf_partition")
    assert result == "20\n25", "check table tab_rf_partition should be filtered"

    # cleanup pub
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_allinschema")
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_x")
    node_publisher.safe_sql("DROP TABLE public.tab_rf_partition")
    node_publisher.safe_sql("DROP TABLE schema_rf_x.tab_rf_partitioned")
    node_publisher.safe_sql("DROP TABLE schema_rf_x.tab_rf_x")
    node_publisher.safe_sql("DROP SCHEMA schema_rf_x")
    # cleanup sub
    node_subscriber.safe_sql("DROP SUBSCRIPTION tap_sub")
    node_subscriber.safe_sql("DROP TABLE public.tab_rf_partition")
    node_subscriber.safe_sql("DROP TABLE schema_rf_x.tab_rf_partitioned")
    node_subscriber.safe_sql("DROP TABLE schema_rf_x.tab_rf_x")
    node_subscriber.safe_sql("DROP SCHEMA schema_rf_x")
    ...
    node_publisher.safe_sql("CREATE TABLE tab_rowfilter_1 (a int primary key, b text)")
    node_publisher.safe_sql("ALTER TABLE tab_rowfilter_1 REPLICA IDENTITY FULL;")
    node_publisher.safe_sql("CREATE TABLE tab_rowfilter_2 (c int primary key)")
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_3 (a int primary key, b boolean)"
    )
    node_publisher.safe_sql("CREATE TABLE tab_rowfilter_4 (c int primary key)")
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_partitioned (a int primary key, b integer) "
    ...
    node_publisher.safe_sql(
        "ALTER TABLE tab_rowfilter_partitioned ATTACH PARTITION "
        "tab_rowfilter_less_10k FOR VALUES FROM (MINVALUE) TO (10000)"
    )
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "ALTER TABLE tab_rowfilter_partitioned ATTACH PARTITION "
        "tab_rowfilter_greater_10k FOR VALUES FROM (10000) TO (MAXVALUE)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_partitioned_2 "
        "(a int primary key, b integer) PARTITION BY RANGE(a)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_partition (LIKE tab_rowfilter_partitioned_2)"
    )
    node_publisher.safe_sql(
        "ALTER TABLE tab_rowfilter_partitioned_2 ATTACH PARTITION "
        "tab_rowfilter_partition DEFAULT"
    )
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "CREATE UNIQUE INDEX tab_rowfilter_toast_ri_index on "
        "tab_rowfilter_toast (a, b)"
    )
    node_publisher.safe_sql(
        "ALTER TABLE tab_rowfilter_toast REPLICA IDENTITY USING INDEX "
        "tab_rowfilter_toast_ri_index"
    )
    node_publisher.safe_sql("CREATE TABLE tab_rowfilter_inherited (a int)")
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_child (b text) "
        "INHERITS (tab_rowfilter_inherited)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_viaroot_part (a int) PARTITION BY RANGE (a)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_viaroot_part_1 PARTITION OF "
        "tab_rowfilter_viaroot_part FOR VALUES FROM (1) TO (20)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_parent_sync (a int) PARTITION BY RANGE (a)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_child_sync PARTITION OF "
        "tab_rowfilter_parent_sync FOR VALUES FROM (1) TO (20)"
    )
    node_publisher.safe_sql(
        "CREATE TABLE tab_rowfilter_virtual (id int PRIMARY KEY, x int, "
        "y int GENERATED ALWAYS AS (x * 2) VIRTUAL)"
    )

    # setup structure on subscriber
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_1 (a int primary key, b text)")
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_2 (c int primary key)")
    node_subscriber.safe_sql(
        "CREATE TABLE tab_rowfilter_3 (a int primary key, b boolean)"
    )
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_4 (c int primary key)")
    node_subscriber.safe_sql(
        "CREATE TABLE tab_rowfilter_partitioned (a int primary key, b integer) "
        "PARTITION BY RANGE(a)"
    )
    ...
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_inherited (a int)")
    node_subscriber.safe_sql(
        "CREATE TABLE tab_rowfilter_child (b text) "
        "INHERITS (tab_rowfilter_inherited)"
    )
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_viaroot_part (a int)")
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_viaroot_part_1 (a int)")
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_parent_sync (a int)")
    node_subscriber.safe_sql("CREATE TABLE tab_rowfilter_child_sync (a int)")
    node_subscriber.safe_sql(
        "CREATE TABLE tab_rowfilter_virtual (id int PRIMARY KEY, x int, "
        "y int GENERATED ALWAYS AS (x * 2) VIRTUAL)"
    )
    ...
    node_publisher.safe_sql(
        "ALTER PUBLICATION tap_pub_1 SET TABLE tab_rowfilter_1 "
        "WHERE (a > 1000 AND b <> 'filtered'), tab_rowfilter_2 "
        "WHERE (c % 2 = 0), tab_rowfilter_3"
    )
    ...
    node_publisher.safe_sql("CREATE PUBLICATION tap_pub_4b FOR TABLE tab_rowfilter_4")

    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_5a FOR TABLE tab_rowfilter_partitioned_2"
    )
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_5b FOR TABLE tab_rowfilter_partition "
        "WHERE (a > 10)"
    )
    ...
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_inherits FOR TABLE tab_rowfilter_inherited "
        "WHERE (a > 15)"
    )
    ...
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_viaroot_1 FOR TABLE "
        "tab_rowfilter_viaroot_part WHERE (a > 15) "
        "WITH (publish_via_partition_root)"
    )
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_viaroot_2 FOR TABLE "
        "tab_rowfilter_viaroot_part_1 WHERE (a < 15) "
        "WITH (publish_via_partition_root)"
    )
    ...
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_parent_sync FOR TABLE "
        "tab_rowfilter_parent_sync WHERE (a > 15) "
        "WITH (publish_via_partition_root)"
    )
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_child_sync FOR TABLE "
        "tab_rowfilter_child_sync WHERE (a < 15)"
    )

    # publication using virtual generated column in row filter expression
    node_publisher.safe_sql(
        "CREATE PUBLICATION tap_pub_virtual FOR TABLE tab_rowfilter_virtual "
        "WHERE (y > 10)"
    )

    #
    # The following INSERTs are executed before the CREATE SUBSCRIPTION, so
    # these SQL commands are for testing the initial data copy using logical
    # replication.
    #
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_1 (a, b) SELECT x, 'test ' || x "
        "FROM generate_series(990,1002) x"
    )
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_2 (c) SELECT generate_series(1, 20)"
    )
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_3 (a, b) SELECT x, (x % 3 = 0) "
    ...
    node_publisher.safe_sql("INSERT INTO tab_rowfilter_parent_sync(a) VALUES(14), (16)")

    # insert data into partitioned table and directly on the partition
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_partitioned (a, b) "
        "VALUES(1, 100),(7000, 101),(15000, 102),(5500, 300)"
    )
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_toast(a, b) "
        "VALUES(repeat('1234567890', 200), '1234567890')"
    )

    # insert data into parent and child table.
    node_publisher.safe_sql("INSERT INTO tab_rowfilter_inherited(a) VALUES(10),(20)")
    node_publisher.safe_sql(
    ...
    node_subscriber.safe_sql(
        f"CREATE SUBSCRIPTION tap_sub CONNECTION "
        f"'{publisher_connstr} application_name={appname}' "
        "PUBLICATION tap_pub_1, tap_pub_2, tap_pub_3, tap_pub_4a, tap_pub_4b, "
        "tap_pub_5a, tap_pub_5b, tap_pub_toast, tap_pub_inherits, "
        "tap_pub_viaroot_2, tap_pub_viaroot_1, tap_pub_parent_sync, "
        "tap_pub_child_sync, tap_pub_virtual"
    )
    ...
    #
    result = node_subscriber.safe_sql("SELECT a, b FROM tab_rowfilter_1 ORDER BY 1, 2")
    assert result == (
        "1001|test 1001\n1002|test 1002\n1980|not filtered"
    ...
    result = node_subscriber.safe_sql("SELECT count(a) FROM tab_rowfilter_3")
    assert result == "10", "check initial data copy from table tab_rowfilter_3"

    # Check expected replicated rows for partitions
    # publication option publish_via_partition_root is false so use the row
    ...
    # The parent table for this partition is published via tap_pub_5a, so there
    # is no filter for the partition. And expressions are OR'ed together so it
    # means OR everything - e.g. same as no filter at all.
    ...
    result = node_subscriber.safe_sql("SELECT count(*) FROM tab_rowfilter_toast")
    ...
    # tap_pub_child_sync.
    # Since the option publish_via_partition_root of tap_pub_parent_sync is
    # true, so the row filter of tap_pub_parent_sync will be used:
    ...
    # The following commands are executed after CREATE SUBSCRIPTION, so these
    # SQL commands are for testing normal logical replication behavior.
    #
    ...
    node_publisher.safe_sql("UPDATE tab_rowfilter_1 SET b = NULL WHERE a = 1600")
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql("DELETE FROM tab_rowfilter_1 WHERE a = 1700")
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_2 (c) VALUES (21), (22), (23), (24), (25)"
    )
    node_publisher.safe_sql("INSERT INTO tab_rowfilter_4 (c) VALUES (0), (11), (12)")
    node_publisher.safe_sql("INSERT INTO tab_rowfilter_inherited (a) VALUES (14), (16)")
    node_publisher.safe_sql(
    ...
    # Use a different partitioned table layout (exercise
    # publish_via_partition_root)
    node_publisher.safe_sql(
        "ALTER PUBLICATION tap_pub_3 SET (publish_via_partition_root = true)"
    )
    node_publisher.safe_sql(
        "ALTER PUBLICATION tap_pub_3 SET TABLE tab_rowfilter_partitioned "
        "WHERE (a < 5000), tab_rowfilter_less_10k WHERE (a < 6000)"
    )
    node_subscriber.safe_sql("TRUNCATE TABLE tab_rowfilter_partitioned")
    node_subscriber.safe_sql(
    ...
    node_publisher.safe_sql(
        "INSERT INTO tab_rowfilter_partitioned (a, b) "
        "VALUES(4000, 400),(4001, 401),(4002, 402)"
    )
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql("UPDATE tab_rowfilter_less_10k SET b = 30 WHERE a = 4001")
    node_publisher.safe_sql("DELETE FROM tab_rowfilter_less_10k WHERE a = 4002")

    node_publisher.wait_for_catchup(appname)

    # Check expected replicated rows for partitions
    # publication option publish_via_partition_root is true so use the row
    # filter from the root partitioned table
    # tab_rowfilter_partitioned filter: (a < 5000)
    # tab_rowfilter_less_10k filter:    (a < 6000)
    # tab_rowfilter_greater_10k filter: no filter
    #
    ...
    node_publisher.safe_sql("UPDATE tab_rowfilter_toast SET b = '1'")
    ...
    # - INSERT (15)        NO, 15 = 15
    # - INSERT (16)        YES, 16 > 15
    result = node_subscriber.safe_sql("SELECT a FROM tab_rowfilter_viaroot_part")
    ...
    result = node_subscriber.safe_sql("SELECT a FROM tab_rowfilter_viaroot_part_1")
