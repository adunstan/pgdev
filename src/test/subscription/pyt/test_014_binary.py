...
def test_014_binary(create_pg):
    # Create and initialize a publisher node.  Set log_statement='all'
    # explicitly here since this test relies on the publisher logging
    # COPY ... TO STDOUT statements.
    node_publisher = create_pg(
        "publisher", allows_streaming="logical", start=False
    )
    node_publisher.append_conf("log_statement = 'all'")
    ...
    node_publisher.safe_sql("CREATE PUBLICATION tpub FOR ALL TABLES")
    ...
    node_subscriber.safe_sql(
        f"CREATE SUBSCRIPTION tsub CONNECTION '{publisher_connstring}' "
    ...
    result = node_subscriber.safe_sql(sync_check)

    assert result == """1|1.2|1.3|10
2|2.2|2.3|20
{1,2,3}|{1.1,1.2,1.3}|{one,two,three}
...
4|4.2|4.3|40""", "check replicated data on subscriber"
    ...
    assert result == """{1,2,3}|{42,1.2,1.3}|
{1,3,2}|{42,1.3,1.2}|
{2,1,3}|{42,1.1,1.3}|
...
4|42||40""", "check updated replicated data on subscriber"
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tsub SET (binary = false);")
...
5|5.2|5.3|50""", "check replicated data on subscriber"
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tsub SET (binary = true);")
...
{3,1,2}|{42,1.1,1.2}|""", "check replicated data on subscriber"
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tsub REFRESH PUBLICATION")

    # It should fail
    node_subscriber.wait_for_log(
        r"ERROR: ( [A-Z0-9]+:)? no binary input function available for type",
        offset,
    )
    ...
    # Check the synced data on the subscriber
    result = node_subscriber.safe_sql("SELECT a FROM test_myvarchar;")

    assert result == "a", "check synced data on subscriber with custom type"
    ...
    # NB: the in-process safe_sql wraps a multi-statement string in a single
    # implicit transaction, but ALTER SUBSCRIPTION ... REFRESH PUBLICATION
    # cannot run inside a transaction block, so issue it separately.
    node_subscriber.safe_sql(
        """
        CREATE TABLE public.test_mismatching_types (
            a int PRIMARY KEY
