...
def test_024_add_drop_pub(create_pg):
    ...
    node_publisher.safe_sql("CREATE PUBLICATION tap_pub_1 FOR TABLE tab_1")
    ...
    result = node_subscriber.safe_sql("SELECT count(*), min(a), max(a) FROM tab_1")
    ...
    node_publisher.safe_sql("ALTER PUBLICATION tap_pub_2 ADD TABLE tab_2")

    # Dropping tap_pub_1 will refresh the entire publication list
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tap_sub DROP PUBLICATION tap_pub_1")

    # Wait for initial table sync to finish
    node_subscriber.wait_for_subscription_sync(node_publisher, "tap_sub")

    # Check the initial data of tab_2 was copied to subscriber
    result = node_subscriber.safe_sql("SELECT count(*), min(a), max(a) FROM tab_2")
    assert result == "10|1|10", "check initial data is copied to subscriber"

    # Re-adding tap_pub_1 will refresh the entire publication list
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tap_sub ADD PUBLICATION tap_pub_1")
    ...
    node_subscriber.safe_sql("ALTER SUBSCRIPTION tap_sub SET PUBLICATION tap_pub_3")
    ...
    node_publisher.safe_sql("CREATE PUBLICATION tap_pub_3 FOR TABLE tab_3")
