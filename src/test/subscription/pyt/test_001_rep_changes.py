...
def test_001_rep_changes(create_pg):
    ...
    assert node_subscriber.safe_sql(
        "SELECT c1 FROM tab_no_replidentity_index") == "1", \
    ...
    assert re.search(
        ...
        logfile), "update target row is missing"
    assert re.search(
    ...
    assert re.search(
        ...
        logfile), "delete target row is missing"
    ...
    result = node_subscriber.safe_sql(
        "SELECT * FROM tab_mixed ORDER BY a")
    assert result == "local|11.11|baz|1\nlocal|2.2|bar|2", \
    ...
    assert not re.search(r"QUERY STATISTICS", logfile), \
    ...
    assert re.search(
        ...
        reterr), 'CREATE PUBLICATION while "wal_level=minimal"'
