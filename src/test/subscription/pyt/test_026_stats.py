...
def create_sub_pub_w_errors(
    ...
    node_subscriber.safe_sql(
        f"ALTER SEQUENCE {sequence_name} INCREMENT 1", dbname=db
    )
...
def test_026_stats(create_pg):
    ...
    (pub1_name, sub1_name) = create_sub_pub_w_errors(
    ...
    node_subscriber.safe_sql(
        "SELECT pg_stat_reset_subscription_stats(NULL)", dbname=db
    )
    ...
    node_subscriber.safe_sql(
        f"ALTER SUBSCRIPTION {sub2_name} DISABLE", dbname=db
    )
    node_subscriber.safe_sql(
