...
def test_bug_16643_initial_sync(create_pg):
    ...
    d1_connstr = (
        f"host={node_twoways.host} port={node_twoways.port} dbname=d1")
    node_twoways.safe_sql(
    ...
    node_twoways.safe_sql(
        "ALTER SUBSCRIPTION testsub REFRESH PUBLICATION", dbname="d2")
...
def test_cascaded_replication_tablesync(create_pg):
    ...
    node_pub_sub = create_pg(
        "testpublisher_subscriber", allows_streaming="logical")
    ...
    publisher1_connstr = (
        f"host={node_pub.host} port={node_pub.port} dbname=postgres")
    publisher2_connstr = (
...
def test_replica_identity_full_dropped_columns(create_pg):
    ...
    result = node_subscriber.safe_sql(
        "SELECT count(*) FROM dropped_cols WHERE a = 100")
...
def test_pgoutput_missing_attributes_and_slot_drop(create_pg):
    ...
    try:
        ...
        assert res.error_message is None, \
            f"drop replication slot: {res.error_message}"
    finally:
        sess.close()
