...
def test_022_twophase_cascade(create_pg):
    ...
    node_A.safe_sql("CREATE PUBLICATION tap_pub_A FOR TABLE tab_full, test_tab")
    appname_B = "tap_sub_B"
    node_B.safe_sql(
        "CREATE SUBSCRIPTION tap_sub_B "
        f"CONNECTION '{connstr_A} application_name={appname_B}' "
    ...
    node_B.safe_sql("CREATE PUBLICATION tap_pub_B FOR TABLE tab_full, test_tab")
    appname_C = "tap_sub_C"
    node_C.safe_sql(
        "CREATE SUBSCRIPTION tap_sub_C "
        f"CONNECTION '{connstr_B} application_name={appname_C}' "
    ...
    assert node_C.poll_query_until(
        twophase_query
    ), "Timed out while waiting for subscriber to enable twophase"
    ...
    # check that transaction was committed on subscriber(s)
    result = node_B.safe_sql("SELECT count(*), count(c), count(d = 999) FROM test_tab")
    assert result == "3334|3334|3334", (
        "Rows inserted by 2PC have committed on subscriber B, and extra "
        "columns have local defaults"
    )
    result = node_C.safe_sql("SELECT count(*), count(c), count(d = 999) FROM test_tab")
    assert result == "3334|3334|3334", (
    ...
    result = node_B.safe_sql("SELECT count(*) FROM test_tab where b = 'foobar';")
    ...
    result = node_C.safe_sql("SELECT count(*) FROM test_tab where b = 'foobar';")
    ...
    assert result == "0", "check replication origin was dropped on subscriber node C"
    result = node_B.safe_sql("SELECT count(*) FROM pg_replication_slots")
    assert result == "0", "check replication slot was dropped on publisher node B"
    ...
    assert result == "0", "check replication origin was dropped on subscriber node B"
    result = node_A.safe_sql("SELECT count(*) FROM pg_replication_slots")
    assert result == "0", "check replication slot was dropped on publisher node A"
