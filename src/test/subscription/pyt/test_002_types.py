...
"""Test that more complex datatypes are replicated correctly by logical
replication.
"""
...
# The query used to dump the replicated data on the subscriber.  It is
# prefixed with SET timezone = '+2' so that timestamptz values are
...
def test_002_types(create_pg):
    ...
    # Create some preexisting content on publisher and setup structure on
    # both nodes
    ...
    result = node_subscriber.safe_sql(CHECK_QUERY)

    assert result == r"""1|{1,2,3}
2|{2,3,1}
3|{3,2,1}
4|{4,3,2}
5|{5,NULL,3}
...
2|"zzz"=>"foo"
3|"123"=>"321"
4|"yellow horse"=>"moaned""" + '"', "check replicated inserts on subscriber"
...
2|"updated"=>"value"
3|"also"=>"updated"
4|"yellow horse"=>"moaned""" + '"', "check replicated updates on subscriber"
...
2|"updated"=>"value"
3|"also"=>"updated"
4|"yellow horse"=>"moaned""" + '"', "check replicated deletes on subscriber"

    # Test a domain with a constraint backed by a SQL-language function,
    # which needs an active snapshot in order to operate.
    node_publisher.safe_sql("INSERT INTO tst_dom_constr VALUES (11)")

    node_publisher.wait_for_catchup("tap_sub")

    result = node_subscriber.safe_sql("SELECT sum(a) FROM tst_dom_constr")
    assert result == "21", "sql-function constraint on domain"
