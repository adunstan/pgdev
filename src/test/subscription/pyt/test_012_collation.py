...
def test_012_collation(create_pg):
    ...
    node_publisher.safe_sql("CREATE TABLE tab1 (a text PRIMARY KEY, b text)")

    node_publisher.safe_sql(r"INSERT INTO tab1 VALUES (U&'\00E4bc', 'foo')")

    node_subscriber.safe_sql(
        "CREATE TABLE tab1 (a text COLLATE ctest_nondet PRIMARY KEY, b text)"
    )

    node_subscriber.safe_sql(r"INSERT INTO tab1 VALUES (U&'\0061\0308bc', 'foo')")
    ...
    node_publisher.safe_sql(r"INSERT INTO tab2 VALUES (U&'\00E4bc', 'foo')")

    node_subscriber.safe_sql("CREATE TABLE tab2 (a text COLLATE ctest_nondet, b text)")
    node_subscriber.safe_sql("ALTER TABLE tab2 REPLICA IDENTITY FULL")

    node_subscriber.safe_sql(r"INSERT INTO tab2 VALUES (U&'\0061\0308bc', 'foo')")
    ...
    assert (
        node_subscriber.safe_sql("SELECT b FROM tab1") == "bar"
    ...
    assert (
        node_subscriber.safe_sql("SELECT b FROM tab2") == "bar"
