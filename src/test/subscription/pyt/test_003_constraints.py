...
"""Test that constraints work on the subscriber."""


def test_003_constraints(create_pg):
    ...
    node_publisher.safe_sql("CREATE TABLE tab_fk (bid int PRIMARY KEY);")
    ...
    node_subscriber.safe_sql("CREATE TABLE tab_fk (bid int PRIMARY KEY);")
    ...
    node_publisher.safe_sql("INSERT INTO tab_fk (bid) VALUES (1);")
    ...
    assert result == "1|1|1", "check replicated tab_fk inserts on subscriber"

    result = node_subscriber.safe_sql(
        "SELECT count(*), min(bid), max(bid) FROM tab_fk_ref;"
    )
    assert result == "1|1|1", "check replicated tab_fk_ref inserts on subscriber"

    # Drop the fk on publisher
    node_publisher.safe_sql("DROP TABLE tab_fk CASCADE;")

    # Insert data
    node_publisher.safe_sql("INSERT INTO tab_fk_ref (id, bid) VALUES (2, 2);")
    ...
    assert result == "2|1|2", "check FK ignored on subscriber"
...
"""
    ...
    # Insert data
    node_publisher.safe_sql("INSERT INTO tab_fk_ref (id, bid) VALUES (10, 10);")
    ...
    assert result == "2|1|2", "check replica insert trigger applied on subscriber"

    # Update data
    node_publisher.safe_sql("UPDATE tab_fk_ref SET bid = 2 WHERE bid = 1;")
    ...
    node_publisher.safe_sql("UPDATE tab_fk_ref SET id = 6 WHERE id = 1;")
