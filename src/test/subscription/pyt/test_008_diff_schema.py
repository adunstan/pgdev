...
"""Test behavior with a different schema on the subscriber."""


def test_008_diff_schema(create_pg):
    ...
    node_publisher.safe_sql("UPDATE test_tab SET b = encode(sha256(b::bytea), 'hex')")
    ...
    assert result == "2|2|2|2", "check extra columns contain local defaults after copy"
    ...
    assert result == "2|2|2", "check extra columns contain locally changed data"

    # Another insert
    node_publisher.safe_sql("INSERT INTO test_tab VALUES (3, 'baz')")
    ...
    assert result == "3|3|3|3", "check extra columns contain local defaults after apply"
    ...
    node_publisher.safe_sql("CREATE TABLE test_tab2 (a int)")

    node_subscriber.safe_sql("CREATE TABLE test_tab2 (a int)")
    ...
    node_subscriber.safe_sql("ALTER TABLE test_tab2 ADD COLUMN b serial PRIMARY KEY")

    node_publisher.safe_sql("INSERT INTO test_tab2 VALUES (1)")

    node_publisher.wait_for_catchup("tap_sub")

    assert (
        node_subscriber.safe_sql("SELECT count(*), min(a), max(a) FROM test_tab2")
        == "1|1|1"
    ), "check replicated inserts on subscriber"

    # Test if the expected error is reported when the subscriber table is
    # missing columns which were specified on the publisher table.
    node_publisher.safe_sql("CREATE TABLE test_tab3 (a int, b int, c int)")
    node_subscriber.safe_sql("CREATE TABLE test_tab3 (a int)")
