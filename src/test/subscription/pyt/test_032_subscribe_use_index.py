...
def test_032_subscribe_use_index(create_pg):
    ...
    node_subscriber.safe_sql(
        ...
        "FOR VALUES FROM (10) TO (20)")
    node_subscriber.safe_sql(
        "CREATE INDEX users_table_part_idx ON "
        "users_table_part(user_id, value_1)")
    ...
    assert result == "10907", \
        "ensure subscriber has the correct data at the end of the test"
    result = node_subscriber.safe_sql(
        "select count(DISTINCT(user_id,value_1, value_2)) "
    ...
    node_subscriber.safe_sql(
        "CREATE TABLE test_replica_id_full (x int, y int)")
    node_subscriber.safe_sql(
        "CREATE UNIQUE INDEX test_replica_id_full_idxy "
        "ON test_replica_id_full(x,y)")

    # insert some initial data
    node_publisher.safe_sql(
        "INSERT INTO test_replica_id_full SELECT i, i "
