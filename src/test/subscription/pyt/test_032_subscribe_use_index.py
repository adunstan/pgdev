...
def test_032_subscribe_use_index(create_pg):
    ...
    node_publisher.safe_sql("CREATE TABLE test_replica_id_full (x int, y text)")
    node_publisher.safe_sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")
    node_subscriber.safe_sql("CREATE TABLE test_replica_id_full (x int, y text)")
    ...
    node_subscriber.safe_sql(
        f"CREATE SUBSCRIPTION tap_sub_rep_full CONNECTION "
    ...
    # delete 2 rows
    node_publisher.safe_sql("DELETE FROM test_replica_id_full WHERE x IN (5, 6)")
    ...
    # cleanup pub
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_rep_full")
    node_publisher.safe_sql("DROP TABLE test_replica_id_full")
    # cleanup sub
    node_subscriber.safe_sql("DROP SUBSCRIPTION tap_sub_rep_full")
    node_subscriber.safe_sql("DROP TABLE test_replica_id_full")
    ...
    # create tables pub and sub
    node_publisher.safe_sql(
        "CREATE TABLE users_table_part(user_id bigint, value_1 int, "
        "value_2 int) PARTITION BY RANGE (value_1)"
    )
    node_publisher.safe_sql(
    ...
    node_publisher.safe_sql("ALTER TABLE users_table_part REPLICA IDENTITY FULL")
    node_publisher.safe_sql("ALTER TABLE users_table_part_0 REPLICA IDENTITY FULL")
    node_publisher.safe_sql("ALTER TABLE users_table_part_1 REPLICA IDENTITY FULL")
    ...
    node_publisher.safe_sql("UPDATE users_table_part SET value_1 = 0 WHERE user_id = 4")
    ...
    # cleanup pub
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_rep_full")
    node_publisher.safe_sql("DROP TABLE users_table_part")

    # cleanup sub
    node_subscriber.safe_sql("DROP SUBSCRIPTION tap_sub_rep_full")
    node_subscriber.safe_sql("DROP TABLE users_table_part")

    # Testcase end: Subscription can use index on partitioned tables
    # =========================================================================

    # =========================================================================
    # Testcase start: Subscription will not use indexes with only expressions
    # or partial index

    # create tables pub and sub
    node_publisher.safe_sql("CREATE TABLE people (firstname text, lastname text)")
    node_publisher.safe_sql("ALTER TABLE people REPLICA IDENTITY FULL")
    node_subscriber.safe_sql("CREATE TABLE people (firstname text, lastname text)")
    ...
    # create pub/sub
    node_publisher.safe_sql("CREATE PUBLICATION tap_pub_rep_full FOR TABLE people")
    node_subscriber.safe_sql(
        f"CREATE SUBSCRIPTION tap_sub_rep_full CONNECTION "
        f"'{publisher_connstr} application_name={appname}' "
        "PUBLICATION tap_pub_rep_full"
    ...
    node_publisher.safe_sql("DELETE FROM people WHERE firstname = 'first_name_3'")
    node_publisher.safe_sql(
    ...
    # make sure that the subscriber has the correct data
    result = node_subscriber.safe_sql("SELECT count(*) FROM people")
    assert (
        result == "199"
    ), "ensure subscriber has the correct data at the end of the test"
    ...
    # cleanup pub
    node_publisher.safe_sql("DROP PUBLICATION tap_pub_rep_full")
    node_publisher.safe_sql("DROP TABLE people")
    # cleanup sub
    node_subscriber.safe_sql("DROP SUBSCRIPTION tap_sub_rep_full")
    node_subscriber.safe_sql("DROP TABLE people")
    ...
    node_publisher.safe_sql("DELETE FROM people WHERE firstname = 'no-name'")

    # wait until the index is used on the subscriber
    node_publisher.wait_for_catchup(appname)
    assert node_subscriber.poll_query_until(
        "select idx_scan=2 from pg_stat_all_indexes "
        "where indexrelname = 'people_names';"
    ), (
        "Timed out while waiting for check subscriber tap_sub_rep_full "
        "deletes two rows via index scan with index on expressions and "
        "columns"
    )
    ...
    node_subscriber.safe_sql("DROP INDEX people_names")

    # delete 1 row
    node_publisher.safe_sql("DELETE FROM people WHERE lastname = 'last_name_18'")
    ...
    node_publisher.safe_sql("CREATE TABLE test_replica_id_full (x int)")

    node_publisher.safe_sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")

    node_subscriber.safe_sql("CREATE TABLE test_replica_id_full (x int, y int)")
    ...
    node_publisher.safe_sql("INSERT INTO test_replica_id_full VALUES (1), (2), (3)")
    node_publisher.safe_sql("UPDATE test_replica_id_full SET x = x + 1 WHERE x = 1")
    ...
    assert node_subscriber.poll_query_until(
        ...
        "Timed out while waiting for check subscriber tap_sub_rep_full "
        "updates test_replica_id_full table"
    )
    ...
    # =========================================================================
    # Testcase start: Subscription using a unique index when Pub/Sub has
    # different data
    #
    # The subscriber has duplicate tuples that publisher does not have. When
    # publisher updates/deletes 1 row, subscriber uses indexes and
    # updates/deletes exactly 1 row.
    #

    # create tables pub and sub
    node_publisher.safe_sql("CREATE TABLE test_replica_id_full (x int, y int)")
    node_publisher.safe_sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")
    node_subscriber.safe_sql("CREATE TABLE test_replica_id_full (x int, y int)")
    node_subscriber.safe_sql(
    ...
    node_subscriber.safe_sql(
        "INSERT INTO test_replica_id_full SELECT i+100, i "
        "FROM generate_series(0,21) i"
    )

    # now, we update only 1 row on the publisher and expect the subscriber to
    # only update 1 row although there are two tuples with y = 15 on the
    # subscriber
    node_publisher.safe_sql("UPDATE test_replica_id_full SET x = 2000 WHERE y = 15")
    ...
    node_subscriber.safe_sql("CREATE TABLE test_replica_id_full (x int, y text)")
    node_subscriber.safe_sql(
        "CREATE INDEX test_replica_id_full_idx "
        "ON test_replica_id_full USING HASH (x)"
    )
    ...
    assert node_subscriber.poll_query_until(
        ...
        "Timed out while waiting for check subscriber tap_sub_rep_full "
        "deletes 2 rows and updates 2 rows via index"
    )
