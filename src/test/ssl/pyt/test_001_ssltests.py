...
def test_001_ssltests(create_pg, ssl_server):
    ...
    supports_sslcertmode_require = ssl_server.check_pg_config(
        "#define HAVE_SSL_CTX_SET_CERT_CB 1"
    )
    # Determine whether build supports IPv6 in certificates.
    have_inet_pton = ssl_server.check_pg_config("#define HAVE_INET_PTON 1")
    ...
    assert not node.log_contains(
        r"no SSL error reported", log_pos
    ...
    if not libressl:
        ...
        if not windows_os:
            ...
            assert not (status.st_mode & 0o006), "keylog file is not world readable"
    ...
    node.connect_ok(
        f"{common_connstr} host=192.0.2.1 sslsni=0", "IP address in the Common Name"
    )
    ...
    node.connect_ok(
        f"{common_connstr} host=192.0.2.1 sslsni=0", "IP address in a dNSName"
    )
    ...
    switch_server_cert(
        certfile="server-cn-only+server_ca", keyfile="server-cn-only", cafile="root_ca"
    )
    common_connstr = (
        f"{default_ssl_connstr} user=ssltestuser dbname=trustdb "
    ...
    node.command_like(
        [
            ...
            "null=_null_",
            "--dbname",
            f"{common_connstr} sslrootcert=invalid",
            "--command",
    ...
    node.command_like(
        ...
        r"(?mx)^pid,ssl,version,cipher,bits,client_dn,client_serial,issuer_dn\r?\n"
        r"^\d+,t,TLSv[\d.]+,[\w-]+,\d+,/?CN=ssltestuser,"
        + serialno
