...
def test_004_sni(create_pg, ssl_server):
    ...
    connstr = (
        f"user=ssltestuser dbname=trustdb hostaddr={SERVERHOSTADDR} sslsni=1"
    )
    ...
    # without a CA.
    node.append_conf(
        "example.org server-cn-only+server_ca.crt server-cn-only.key "
        "root_ca.crt",
        filename="pg_hosts.conf",
    )
    node.reload()
    ...
    assert not _restart_fails(node), (
        "pg_hosts.conf: restart fails with default entry combined with "
        "hostnames"
    )
    ...
    assert not _restart_fails(node), (
        "pg_hosts.conf: restart fails with two identical hostname entries in "
        "lists"
    )
    ...
    node.connect_ok(
        f"{connstr} sslrootcert={SSL_DIR}/root+server_ca.crt sslmode=require "
        "host=localhost",
        "pg_hosts.conf: connect with correct server CA cert file after more "
        "reloads",
    )
    ...
    assert "cannot be reloaded because it requires a passphrase" not in log, (
    ...
    node.connect_fails(
        ...
        "host: 'example.org', ca: '': connect with sslcert, no client CA "
        "configured",
