...
def test_002_inline(exe, case, tmp_path):
    ...
    for size in reversed(range(1, chunk + 1)):
        if error is not None:
            assert b"SUCCESS" not in stdout[i], f"{name}, chunk size {size}: test fails"
            assert re.search(
                error, stderr[i]
            ), f"{name}, chunk size {size}: correct error output: {stderr[i]!r}"
        else:
            assert b"SUCCESS" in stdout[i], f"{name}, chunk size {size}: test succeeds"
