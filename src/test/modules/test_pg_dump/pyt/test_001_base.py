    ...
    return {
        ...
        "exclude_table": {
            "dump_cmd": [
                ...
                "--file", f"{tempdir}/exclude_table.sql",
                "postgres",
            ],
        },
        "extension_schema": {
            "dump_cmd": [
                "pg_dump",
                "--schema", "public",
                "--file", f"{tempdir}/extension_schema.sql",
                "postgres",
            ],
        },
        "pg_dumpall_globals": {
        ...
        "privileged_internals": {
            "dump_cmd": [
                "pg_dump", "--no-sync",
                "--file", f"{tempdir}/privileged_internals.sql",
        ...
        "with_extension": {
            "dump_cmd": [
                "pg_dump", "--no-sync",
                "--file", f"{tempdir}/with_extension.sql",
        ...
        "exclude_extension": {
            "dump_cmd": [
                "pg_dump", "--no-sync",
                "--file", f"{tempdir}/exclude_extension.sql",
        ...
        "exclude_extension_filter": {
            "dump_cmd": [
                "pg_dump",
                "--no-sync",
                "--file", f"{tempdir}/exclude_extension_filter.sql",
                "--filter", f"{tempdir}/exclude_extension_filter.txt",
                "postgres",
            ],
        },
        ...
        "without_extension": {
            "dump_cmd": [
                "pg_dump", "--no-sync",
                "--file", f"{tempdir}/without_extension.sql",
                "--extension", "plpgsql",
                "postgres",
            ],
        },
        ...
        "without_extension_explicit_schema": {
            "dump_cmd": [
                "pg_dump",
                "--no-sync",
                "--file", f"{tempdir}/without_extension_explicit_schema.sql",
                "--extension", "plpgsql",
                "--schema", "public",
                "postgres",
            ],
        ...
        "without_extension_internal_schema": {
            "dump_cmd": [
                "pg_dump",
                "--no-sync",
                "--file", f"{tempdir}/without_extension_internal_schema.sql",
                "--extension", "plpgsql",
...
def _tests():
    ...
    return {
        "ALTER EXTENSION test_pg_dump": {
            "create_order": 9,
            "create_sql":
                "ALTER EXTENSION test_pg_dump ADD TABLE "
        ...
        "CREATE ROLE regress_dump_test_role": {
            ...
            "regexp": re.compile(r"^CREATE ROLE regress_dump_test_role;\n",
                                 re.MULTILINE),
        ...
        "GRANT OPTION": {
            "create_order": 2,
            "create_sql":
                "GRANT SET, ALTER SYSTEM ON PARAMETER Custom.Knob TO "
        ...
        "COPY public.regress_table_dumpable (col1)": {
            "regexp": re.compile(
                r"^COPY\ public\.regress_table_dumpable\ \(col1\)\ "
                r"FROM\ stdin;\n",
                _XM,
            ),
            "like": full | {"data_only", "section_data", "extension_schema"},
        ...
        "GRANT SELECT regress_pg_dump_table_added pre-ALTER EXTENSION": {
            "create_order": 8,
            "create_sql":
                "GRANT SELECT ON regress_pg_dump_table_added TO "
                "regress_dump_test_role;",
            "regexp": re.compile(
        ...
        "REVOKE SELECT regress_pg_dump_table_added post-ALTER EXTENSION": {
            "create_order": 10,
            "create_sql":
                "REVOKE SELECT ON regress_pg_dump_table_added FROM "
                "regress_dump_test_role;",
            "regexp": re.compile(
        ...
        "ALTER INDEX pkey DEPENDS ON extension": {
            "create_order": 11,
            "create_sql":
                "CREATE TABLE regress_pg_dump_schema.extdependtab "
...
def test_pg_dump_base(pg, tmp_path):
    ...
    with open(os.path.join(tempdir, "exclude_extension_filter.txt"),
