...
def test_001_multixact(create_pg):
    ...
    node.safe_sql("SELECT injection_points_detach('multixact-create-from-members')")
    ...
    assert (
        node.safe_sql(f"SELECT test_read_multixact('{multi1}');") == ""
    ), "first recorded multi is readable"

    assert (
        node.safe_sql(f"SELECT test_read_multixact('{multi2}');") == ""
