...
def test_003_check_guc(create_pg):
    ...
    for param in missing_from_file:
        print(
            f"found GUC {param} in guc_tables.c, missing from " "postgresql.conf.sample"
        )
    ...
    for param in sample_intersect:
        print(
            f"found GUC {param} in postgresql.conf.sample, marked as " "NOT_IN_SAMPLE"
        )
    for param in lines_with_tabs:
        print(f"found tab in line {param} in postgresql.conf.sample")

    assert (
        len(missing_from_file) == 0
    ), "no parameters missing from postgresql.conf.sample"
    assert len(missing_from_list) == 0, "no parameters missing from guc_tables.c"
    assert (
        len(sample_intersect) == 0
    ), "no parameters marked as NOT_IN_SAMPLE in postgresql.conf.sample"
    assert len(lines_with_tabs) == 0, "no lines with tabs in postgresql.conf.sample"
