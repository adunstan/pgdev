...
def test_013_temp_obj_multisession(create_pg):
    ...
    try:
        ...
        assert re.match(r"^pg_temp_\d+$", tempschema), \
            f"got temp schema: {tempschema}"
        ...
        stderr = _probe_stderr(
            probe, f"UPDATE {tempschema}.foo SET val = NULL;"
        )
        assert re.search(
            r"cannot access temporary tables of other sessions", stderr
        ), f"UPDATE: {stderr}"
        ...
        ) == "0", "cross_session_func cleaned up by cascade from foo2"
    finally:
        probe.close()
        psql1.close()
