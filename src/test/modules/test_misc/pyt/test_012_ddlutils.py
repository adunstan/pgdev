...
def test_012_ddlutils(create_pg):
    ...
    result = node.safe_sql("SELECT * FROM pg_get_role_ddl('regress_role_ddl_test1')")
    assert re.search(
        r"CREATE ROLE regress_role_ddl_test1 .* NOLOGIN", result
    ), "basic role DDL"
    ...
    result = node.safe_sql("SELECT * FROM pg_get_role_ddl('regress_role_ddl_test2')")
    ...
    result = node.safe_sql("SELECT * FROM pg_get_role_ddl('regress_role_ddl_test2')")
    assert (
        "IN DATABASE regression_ddlutils_test SET work_mem TO '128MB'" in result
    ...
    result = node.safe_sql("SELECT * FROM pg_get_role_ddl('regress_role-with-dash')")
    ...
    assert re.search(r"\n\s+SUPERUSER", result), "role pretty-print indents attributes"
    ...
    result = node.safe_sql("SELECT * FROM pg_get_role_ddl('regress_role_ddl_member')")
    assert (
        "GRANT regress_role_ddl_group1 TO regress_role_ddl_member" in result
    ...
    assert "does not exist" in res.error_message, "non-existent role error message"
    ...
    try:
        ...
        assert res.error_message is not None, "role DDL denied without pg_authid access"
    finally:
        noaccess.close()
    ...
    res = node.sql("SELECT * FROM pg_get_database_ddl('regression_no_such_db')")
    assert res.error_message is not None, "non-existent database errors"

    # NULL input
    result = node.safe_sql("SELECT count(*) FROM pg_get_database_ddl(NULL)")
    ...
    assert "invalid value" in res.error_message, "invalid option error message"
    ...
    assert (
        "CREATE DATABASE regression_ddlutils_test" in result
    ...
    assert "OWNER TO regress_role_ddl_test2" in result, "database DDL includes OWNER"
    assert "CONNECTION LIMIT = 123" in result, "database DDL includes CONNLIMIT"
    assert (
        "SET random_page_cost TO '2.0'" in result
    ...
    assert re.search(r"\n\s+WITH TEMPLATE", result), "database DDL pretty-prints WITH"

    # Permission check
    node.safe_sql("REVOKE CONNECT ON DATABASE regression_ddlutils_test FROM PUBLIC")
    noaccess = node.connect()
    try:
        res = noaccess.query(
            "SET ROLE regress_role_ddl_noaccess;\n"
        ...
        assert res.error_message is not None, "database DDL denied without CONNECT"
    finally:
        noaccess.close()
    node.safe_sql("GRANT CONNECT ON DATABASE regression_ddlutils_test TO PUBLIC")
    ...
    res = node.sql("SELECT * FROM pg_get_tablespace_ddl('regress_nonexistent_tblsp')")
    ...
    result = node.safe_sql("SELECT count(*) FROM pg_get_tablespace_ddl(NULL::name)")
    assert result == "0", "NULL tablespace name returns no rows"
    result = node.safe_sql("SELECT count(*) FROM pg_get_tablespace_ddl(NULL::oid)")
    ...
    result = node.safe_sql("SELECT * FROM pg_get_tablespace_ddl('regress_ tblsp')")
    ...
    assert (
        "CREATE TABLESPACE regress_allopt_tblsp" in result
    ), "tablespace DDL includes CREATE"
    assert "OWNER regress_role_ddl_test1" in result, "tablespace DDL includes OWNER"
    ...
    assert re.search(r"\n\s+OWNER", result), "tablespace DDL pretty-prints OWNER"
    ...
    assert "CREATE TABLESPACE regress_allopt_tblsp" in result, "tablespace DDL by OID"
