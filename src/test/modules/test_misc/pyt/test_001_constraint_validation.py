...
def test_001_constraint_validation(create_pg):
    ...
    assert is_table_verified(output), \
    ...
    assert re.search(
        ...
        r"prove that it does not contain nulls", output), \
    ...
    output = run_sql_command(
        "alter table atacc1 alter test_b set not null, "
    ...
    output = run_sql_command(
        "alter table atacc1 alter test_b set not null, "
        "alter test_a set not null;")
    assert not is_table_verified(output), \
        "table was not scanned for both columns"
    assert re.search(
        r'existing constraints on column "atacc1.test_a" are sufficient to '
        r"prove that it does not contain nulls", output), \
        "test_a proved by constraints"
    ...
    assert re.search(
        ...
        r'"list_parted2_def" is implied by existing constraints', output), \
    ...
    assert not re.search(r'verifying table "part_5"', output), \
        "table part_5 not scanned"
    assert re.search(r'verifying table "list_parted2_def"', output), \
    ...
    assert not re.search(r'verifying table "part_6"', output), \
    ...
    assert not re.search(r'verifying table "quuux_default1"', output), \
    ...
    assert re.search(
        ...
        r"is implied by existing constraints", output), \
