...
def _send_prepare(sess, name, sql):
    ...
    lib = sess._lib
    ok = lib.PQsendPrepare(sess._conn, name.encode("utf-8"), sql.encode("utf-8"), 0, None)
...
def _run_pipeline(sess, sends):
    ...
    try:
        ...
        assert sync and lib.PQresultStatus(sync) == PGRES_PIPELINE_SYNC, (
...
"""
    ...
    try:
        ...
        assert not node.log_contains(r"STATEMENT:", log_offset), (
