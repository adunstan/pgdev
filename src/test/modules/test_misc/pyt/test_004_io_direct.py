...
def test_004_io_direct(create_pg, tmp_path):
    ...
    node.safe_sql("create table t1 as select 1 as i from generate_series(1, 10000)")
    ...
    assert node.safe_sql("select count(*) from t1") == "10000", "read back from shared"
    assert node.safe_sql("select * from t2count") == "10000", "read back from local"
    node.stop("immediate")

    node.start()
    assert (
        node.safe_sql("select count(*) from t1") == "10000"
