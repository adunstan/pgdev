...
def test_007_pgbench_standby(create_pg, pg_bin, checksums, bindir):
    ...
    test_iterations = (
        TEST_ITERATIONS_EXTENDED if extended else TEST_ITERATIONS_DEFAULT
    )
    ...
    try:
        ...
        pg_bin.command_ok(
            [
                "pgbench",
                "-h", node_primary.host,
                "-p", str(node_primary.port),
                "-i", "-s", str(scalefactor), "-q",
                "postgres",
            ],
        ...
        for i in range(test_iterations):
            ...
            if not node_primary._postmaster_alive():
            ...
            if not node_standby._postmaster_alive():
                node_standby.start()
            ...
            data_checksum_state = _flip_data_checksums(
                node_primary, node_standby, checksums,
                data_checksum_state, extended,
            )
            if extended:
                checksums.random_sleep()
