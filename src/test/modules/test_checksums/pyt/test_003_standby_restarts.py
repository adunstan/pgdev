...
def test_003_standby_restarts(create_pg, checksums):
    ...
    node_primary.safe_sql(f"SELECT pg_create_physical_replication_slot('{slotname}')")
    ...
    node_primary.safe_sql("CREATE TABLE t AS SELECT generate_series(1,10000) AS a;")
    ...
    assert result == "1000", "unlogged table readable on primary after checksum enable"
    ...
    assert result == "50", "indexed lookup on promoted standby returns expected row"
    ...
    assert not page_verify_re.search(
        log
