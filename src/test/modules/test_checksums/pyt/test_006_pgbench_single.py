...
def _check_no_checksum_errors(node, offset, label):
    ...
    assert not _PAGE_VERIFICATION_FAILED.search(log), (
...
def _background_rw_pgbench(node, extended, checksums, current):
    ...
    pg_bin = node.pg_bin
    argv = [pg_bin._resolve(cmd[0]), *cmd[1:]]
    print("# Running (background): " + " ".join(argv))
    devnull = subprocess.DEVNULL
    return subprocess.Popen(
        argv,
        env=pg_bin._env(None),
        stdin=devnull,
        stdout=devnull,
        stderr=devnull,
    )
...
def test_006_pgbench_single(create_pg, pg_bin, checksums):
    ...
    try:
        ...
        node.pg_bin.command_ok(
            ["pgbench", "-p", str(node.port), "-i", "-s", str(scalefactor),
             "-q", "postgres"],
