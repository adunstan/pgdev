...
class DataChecksums:
    ...
    def test_checksum_state(self, node, state):
        ...
        assert result == state, (
            f"ensure checksums are set to {state} on {node.name}"
        )
        return result == state
    ...
    def wait_for_checksum_state(self, node, state, timeout=TIMEOUT_DEFAULT):
        ...
        assert res, (
            f"ensure data checksums are transitioned to {state} on {node.name}"
        )
        return res
    ...
    def enable_data_checksums(self, node, cost_delay=0, cost_limit=100, wait=None):
        ...
        *wait* is given, wait for that state (and, for 'on'/'off', for the
        ...
        """
        node.safe_sql(
            f"SELECT pg_enable_data_checksums({cost_delay}, {cost_limit});"
        )
        if wait is not None:
