...
def test_004_restart(create_pg):
    ...
    assert re.search(
        r"cannot retrieve commit timestamp for transaction", res.error_message
    ), "expected error from InvalidTransactionId"

    res = node_primary.sql("SELECT pg_xact_commit_timestamp('1');")
    assert res.error_message is None, "getting ts of BootstrapTransactionId succeeds"
    assert res.psqlout == "", "timestamp of BootstrapTransactionId is null"

    res = node_primary.sql("SELECT pg_xact_commit_timestamp('2');")
    assert res.error_message is None, "getting ts of FrozenTransactionId succeeds"
    ...
    assert (
        node_primary.safe_sql("SELECT pg_xact_commit_timestamp('3');") == ""
    ...
    assert before_restart_ts not in ("", "null"), "commit timestamp recorded"

    node_primary.stop("immediate")
    node_primary.start()

    after_crash_ts = node_primary.safe_sql(f"SELECT pg_xact_commit_timestamp('{xid}');")
    assert (
...
"""
    ...
    assert re.search(
        r"could not get commit timestamp data", res.error_message
    ...
    res = node_primary.sql(f"SELECT pg_xact_commit_timestamp('{xid_disabled}');")
