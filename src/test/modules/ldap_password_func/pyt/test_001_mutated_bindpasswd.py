...
def test_001_mutated_bindpasswd(create_pg, ldap_server):
    ...
    _access(
        node, "test1", 2, "search+bind authentication fails with wrong ldapbindpasswd"
    )
    ...
    _access(node, "test1", 2, "search+bind authentication fails with clear password")
    ...
    _access(
        node, "test1", 0, "search+bind authentication succeeds with rot13ed password"
    )
