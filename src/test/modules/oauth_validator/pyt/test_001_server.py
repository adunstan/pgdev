...
''',
    ...
    n.connect_ok(
        ...
        log_like=[
            ...
            r'oauth_validator: issuer="' + re.escape(issuer) + r'", scope="openid postgres"',
        ...
        with _env(PGOAUTHDEBUG="UNSAFE:trace,dos-endpoint,call-count"):
            ...
            n.connect_ok(
                ...
                log_like=[
                    ...
                    + re.escape(f"{issuer}/.well-known/oauth-authorization-server/alternate")
            ...
            # ------------------------------------------------------------------
            common_connstr = f"user=testparam dbname=postgres oauth_issuer={issuer}/param "
            ...
            # several checks against it via _attempt_connection, which folds
            # fd 2 into the stderr it returns.
            # ------------------------------------------------------------------
            base["value"] = common_connstr
            ok, _stdout, captured = n._attempt_connection(
                connstr(stage="token", retries=2),
                "SELECT 'connected for call count'",
            )
            assert ok, f"call count connection succeeds, got {captured!r}"
            assert re.search(PROMPT_EXAMPLE_COM, captured), (
            ...
            assert re.search(r"connection succeeded", proc.stdout), (
                f"stress-async: stdout matches, got {proc.stdout!r}"
            )
            assert not re.search(r"connection to database failed", proc.stderr), (
