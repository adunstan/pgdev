...
""")
    ...
    try:
        ...
        sess.close()
    assert 'source database "postgres" is being accessed by other users' \
        in stderr, "background worker blocked the database creation"
    ...
    node.safe_sql(
        f"CREATE TABLESPACE test_tablespace LOCATION '{tablespace}'")
