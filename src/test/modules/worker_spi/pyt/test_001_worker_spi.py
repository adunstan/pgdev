...
def test_001_worker_spi(create_pg):
    ...
    node.safe_sql("INSERT INTO schema4.counted VALUES ('total', 0), ('delta', 1);")
...
"""
    ...
    worker1_pid = node.safe_sql(
        f"SELECT worker_spi_launch(10, {mydb_id}, {myrole_id});", dbname="mydb"
    )
    worker2_pid = node.safe_sql(
        f"SELECT worker_spi_launch(11, {postgresdb_id}, {myrole_id});", dbname="mydb"
    )
    ...
    sess = node.connect()
    try:
        sess.query(f"SELECT worker_spi_launch(12, {noconndb_id}, {myrole_id});")
    finally:
        sess.close()
    node.wait_for_log(
...
"""
    ...
    sess = node.connect()
    try:
        sess.query(f"SELECT worker_spi_launch(13, {mydb_id}, {nologrole_id});")
    finally:
        sess.close()
