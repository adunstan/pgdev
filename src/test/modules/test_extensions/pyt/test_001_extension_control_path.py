...
def test_extension_control_path(create_pg, tmp_path):
    ...
    assert (
        ecp == f"$system{sep}{ext_dir}{sep}{ext_dir2}"
    ...
    ret = node.safe_sql(
        "select * from pg_available_extension_versions " f"where name = '{ext_name2}'"
    )
    assert ret == (
    ...
    assert ret == "t", "$system extension is shown correctly in pg_available_extensions"
