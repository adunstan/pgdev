...
"""
    ...
    node.safe_sql("SELECT consume_xids_until('2000000000'::xid8)")

    # Make sure the latest completed XID is advanced
    node.safe_sql("INSERT INTO small(data) SELECT 1")
...
"""
    ...
    for tablename in ("large", "large_trunc", "small", "small_trunc"):
        assert re.search(
            r'bypassing nonessential maintenance of table '
            r'"postgres\.public\.' + tablename
            + r'" as a failsafe after \d+ index scans',
            log_contents,
        ), f"failsafe vacuum triggered for {tablename}"
