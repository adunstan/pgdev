...
def psql_like(io_method, session, name, sql, expected_stdout, expected_stderr):
    ...
    assert re.search(expected_stdout, output), (
...
def query_wait_block(io_method, node, session, name, sql, waitfor,
                     wait_current_session):
...
def sub_test_handle(io_method, node):
    ...
    try:
        ...
        # normal handle use
        psql_like(
            io_method, psql, "handle_get_release()",
...
def sub_test_ignore_checksum(io_method, node):
    ...
    try:
        ...
        node.wait_for_log(
            r"LOG:  ignoring checksum failure in block 2", log_location
        )
...
def sub_test_read_buffers(io_method, node):
    ...
    try:
        ...
        if io_method != "sync":
            ...
            buf_id = psql_b.query_oneval(
                f"SELECT buffer_create_toy('{table}', 3)"
            )
            psql_b.query_safe(
                f"SELECT buffer_call_start_io({buf_id}, for_input=>true, wait=>true)"
            )
...
def run_io_method(io_method, node, injection_points_available):
    ...
    assert node.safe_sql("SHOW io_method") == io_method, (
...
def test_001_aio(create_pg, bindir, libdir):
    ...
    probe.start()
    extension_available = probe.safe_sql(
    ...
    injection_points_available = probe.safe_sql(
