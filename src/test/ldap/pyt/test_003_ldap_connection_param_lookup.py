...
def _connect_ok(libdir, connstr, test_name, *, sql=None, expected_stdout=None):
    ...
    assert err is None or err == "", f"{test_name}: connection should succeed\n{err}"
...
def test_003_ldap_connection_param_lookup(create_pg, ldap_server, tmp_path):
    ...
    with open(ldif_valid, "w", encoding="utf-8") as fh:
        fh.write(
            "\nversion:1\n"
    ...
    with open(srvfile_valid, "w", encoding="utf-8") as fh:
        fh.write(
            "\n[my_srv]\n"
    ...
    with open(srvfile_empty, "w", encoding="utf-8") as fh:
        fh.write("")
    ...
    saved_env = {
        k: os.environ.get(k)
        for k in ("PGSYSCONFDIR", "PGSERVICEFILE", "PGSERVICE", "PGDATABASE")
    }
