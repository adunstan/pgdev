...
def test_001_auth(create_pg, ldap_server):
    ...
    (ldap_server_name, ldap_port, ldaps_port, ldap_url, ldaps_url, ldap_basedn) = (
        ldap.prop("server", "port", "s_port", "url", "s_url", "basedn")
    )
    ...
    os.environ["PGPASSWORD"] = "wrong"
    _access(
        node, "test0", 2, "search+bind authentication fails if user not found in LDAP"
    )
    _access(node, "test1", 2, "search+bind authentication fails with wrong password")
    os.environ["PGPASSWORD"] = "secret1"
    _access(
        node,
        "test1",
    ...
    os.environ["PGPASSWORD"] = "secret1"
    _access(node, "test1", 0, "simple bind with LDAP URL authentication succeeds")
    ...
    os.environ["PGPASSWORD"] = "secret1"
    _access(node, "test1", 0, "search+bind with LDAP URL authentication succeeds")
