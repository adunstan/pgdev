    ...
    *ssl_dir* is the path to src/test/ssl/ssl; *keydir* is a writable temp dir
    ...
    def _init_backend(self, pgdata):
        ...
        for name in _SERVER_FILES:
            shutil.copy(os.path.join(self.ssl_dir, name), os.path.join(pgdata, name))
    ...
    def is_libressl(self):
        ...
        return not self.check_pg_config("#define HAVE_SSL_CTX_SET_CERT_CB 1")

    def check_pg_config(self, needle):
        out = subprocess.run(
        ...
        try:
            with open(os.path.join(out, "pg_config.h"), encoding="utf-8") as fh:
    ...
    def configure_test_server_for_ssl(
        ...
        *,
        ...
        for role in ("ssltestuser", "md5testuser", "anotheruser", "yetanotheruser"):
        ...
        with open(os.path.join(pgdata, "sslconfig.conf"), "w", encoding="utf-8"):
            pass
    ...
    def switch_server_cert(
        ...
        *,
        ...
        lines.append("ssl_tls13_ciphers=TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256")
