...
def _test_failed(request):
    """Whether the test's setup or call phase failed.

    Reads the per-phase reports stashed by pgtap's pytest_runtest_makereport
    hook; absent (e.g. plugin not active) is treated as "not failed".
    """
    return any(
        getattr(getattr(request.node, f"_phase_{p}", None), "failed", False)
        for p in ("setup", "call")
    )
...
def test_datadir(request, tmp_path_factory):
    ...
    if not root:
        # pytest's tmp_path_factory manages its own cleanup (keeps the last
        # few runs), so just hand out a fresh directory.
        yield tmp_path_factory.mktemp(safe, numbered=True)
        return
    path = pathlib.Path(root) / safe
    path.mkdir(parents=True, exist_ok=True)
    yield path
    # TESTDATADIR is shared by every test in the file and kept for the whole CI
    # job, so removing this test's data (server data dirs, WAL, aux server
    # dirs, scratch) on success keeps it from accumulating gigabytes and
    # filling the runner disk.  Keep it on failure for post-mortem, like the
    # Perl framework.
...
def create_pg(bindir, libdir, test_datadir):
    ...
    def _create(
        ...
        *,
        ...
        # Track for teardown before init()/start(), so a failure in either
        # still stops a postmaster that may have been left running (start()
        # can set running=True and then raise when pg_ctl times out but the
        # postmaster is in fact alive).
...
def kerberos(test_datadir):
    ...
    saved_env = {
        k: os.environ.get(k) for k in ("KRB5_CONFIG", "KRB5_KDC_PROFILE", "KRB5CCNAME")
    }
    kdcs = []
    counter = [0]
