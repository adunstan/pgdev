...
def slurp_file(path, offset=0):
    """Return the contents of *path* as text, from character *offset* onward.

    *offset* is a character position in the text-decoded (CRLF-normalized)
    contents -- the kind of value PostgresServer.log_position() returns -- not
    a raw byte offset, so it stays correct for multibyte content and on Windows
    where the on-disk log has CRLF line endings.
    """
    with open(path, "r", encoding="utf-8", errors="replace") as fh:
        text = fh.read()
    return text[offset:] if offset else text
