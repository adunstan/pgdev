...
def _detect():
    ...
    if sys.platform == "darwin":
        base = "/opt/homebrew/opt/krb5" if os.path.isdir("/opt/homebrew") \
...
class Kerberos:
    ...
    *basedir* is a writable scratch dir (tmp_path).  *srvnam* is the Kerberos
    ...
    def __init__(self, basedir, host, hostaddr, realm, srvnam="postgres"):
        ...
        ver = subprocess.run([t["krb5_config"], "--version"],
        ...
        with open(self.krb5_conf, "w") as fh:
            fh.write(f"""[logging]
...
""")
        ...
        with open(self.kdc_conf, "w") as fh:
...
{realm} = {{
    ...
    key_stash_file = {self.kdc_datadir}/_k5.{realm}
        ...
        self._bail(self._kadmin_local, "-q", f"ktadd -k {self.keytab} {service_principal}")
    ...
    def stop(self):
        try:
            with open(self.kdc_pidfile) as fh:
