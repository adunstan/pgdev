...
SETUP_ERROR = None if AVAILABLE else (
    "ldap tests not supported on this platform"
    if sys.platform not in ("darwin",) and not sys.platform.startswith(
        ("linux", "freebsd"))
...
rootpw "{rootpw}"
"""
        with open(slapd_conf, "w") as fh:
            fh.write(conf)

        os.mkdir(ldap_datadir)
        os.mkdir(slapd_certs)
        shutil.copy(os.path.join(certdir, "server_ca.crt"),
                    os.path.join(slapd_certs, "ca.crt"))
        ...
        while True:
            rc = subprocess.run(
                ["ldapsearch", "-sbase", "-H", self.url, "-b", self.basedn,
                 "-D", self.rootdn, "-y", self.pwfile, "-n", "objectclass=*"],
