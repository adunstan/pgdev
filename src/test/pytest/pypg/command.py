...
from .util import TIMEOUT_DEFAULT, run_captured
...
class PgBin:
    ...
    def result(
        self, cmd: Sequence[str], *, extra_env=None, timeout=TIMEOUT_DEFAULT
    ) -> CommandResult:
        """Run *cmd* (list) and capture its result. cmd[0] is resolved in bindir.

        A wedged program is killed after *timeout* seconds (raising
        subprocess.TimeoutExpired); pass timeout=None to wait indefinitely.
        """
        ...
        returncode, stdout, stderr = run_captured(
            argv, env=self.command_env(extra_env), timeout=timeout
        )
