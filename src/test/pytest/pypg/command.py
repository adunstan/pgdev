...
class PgBin:
    ...
    def command_ok(self, cmd, msg=None, *, extra_env=None) -> CommandResult:
        ...
        assert res.returncode == 0, (msg or "command should succeed") + "\n" + _describe(cmd, res)
        return res

    def command_fails(self, cmd, msg=None, *, extra_env=None) -> CommandResult:
        res = self.result(cmd, extra_env=extra_env)
        assert res.returncode != 0, (msg or "command should fail") + "\n" + _describe(cmd, res)
        return res
    ...
    def command_fails_like(self, cmd, pattern, msg=None, *, extra_env=None) -> CommandResult:
        import re
