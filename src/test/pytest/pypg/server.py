...
from libpq import Session, conninfo_quote
from libpq.errors import PqConnectionError, QueryError
...
# The breadth of this class matches the cluster-management API the tests
# need; splitting it would not make the tests clearer.
class PostgresServer:  # pylint: disable=too-many-public-methods
    """One initdb'd data directory and the server running on it."""

    def __init__(self, name, bindir, libdir, basedir, port, sockdir, listen_host=None):
        self.name = name
    ...
    def connstr(self, dbname="postgres"):
        return (
            f"host='{conninfo_quote(self.host)}' port={self.port} "
            f"dbname='{conninfo_quote(dbname)}'"
        )
    ...
    def _run(self, *argv, check=True, timeout=TIMEOUT_DEFAULT):
        argv = [self.resolve(argv[0]), *map(str, argv[1:])]
        ...
        # A wedged command is killed after *timeout* seconds (TimeoutExpired);
        # pass timeout=None to wait indefinitely.
        returncode, stdout, _ = run_captured(argv, combine_stderr=True, timeout=timeout)
    ...
    def start(self, fail_ok=False):
        ...
        if proc.returncode == 0:
            self.running = True
            return True
        self.running = self.postmaster_alive()
        if not fail_ok:
    ...
    def stop(self, mode="fast", fail_ok=False):
        ...
        # Only clear running on a confirmed stop, so a failed fail_ok stop
        # leaves the flag set and teardown can force-kill the survivor.
        if proc.returncode == 0:
            self.running = False
            return True
        return False
    ...
    def write_wal(self, tli, lsn, segment_size, data):
        ...
        path = os.path.join(self.data_dir, "pg_wal", "%08X%08X%08X" % (tli, 0, segment))
    ...
    def wait_for_catchup(self, standby_name, mode="replay", target_lsn=None):
        ...
        *standby_name* may be a :class:`PostgresServer` (its name is used as
        ...
        if target_lsn is None:
            ...
            if target_lsn is None:
                raise RuntimeError(
                    f"{self.name}: could not determine a target LSN for "
                    "wait_for_catchup (no WAL position reported yet)"
                )
        ...
        if not self.poll_query_until(query):
            details = self.safe_sql("SELECT * FROM pg_catalog.pg_stat_replication")
            raise TimeoutError(
    ...
    def _open_session(self, dbname):
        ...
        Just after start()/restart() (or while a node finishes crash recovery)
        the server can transiently reject connections with "the database system
        is starting up", "... is not yet accepting connections" or "... is in
        ...
        transient_markers = (
            ...
            "in recovery mode",
            "Connection refused",  # postmaster not listening on its port yet
            "No such file or directory",  # Unix socket file not created yet
        )
        while True:
            try:
    ...
    def connect(self, dbname="postgres", user=None, password=None, options=None):
        ...
        if password is not None:
            connstr += f" password='{conninfo_quote(password)}'"
        if options is not None:
            connstr += f" options='{conninfo_quote(options)}'"
    ...
    def log_check(self, test_name, offset, *, log_like=None, log_unlike=None):
        ...
        for regex in log_like or []:
            assert re.search(regex, contents), f"{test_name}: log matches {regex!r}"
        for regex in log_unlike or []:
            assert not re.search(
                regex, contents
            ), f"{test_name}: log does not match {regex!r}"

    def attempt_connection(self, connstr, sql):
        ...
        argv = [
            ...
            "-t",
            "-d",
            self._full_connstr(connstr),
            "-c",
            sql if sql is not None else "SELECT 1",
        ]
    ...
    def connect_ok(
        ...
        *,
        ...
        ok, stdout, stderr = self.attempt_connection(connstr, sql)
        ...
        self.log_check(
            test_name, log_location, log_like=log_like, log_unlike=log_unlike
        )
    ...
    def connect_fails(
        ...
        *,
        ...
        ok, _stdout, stderr = self.attempt_connection(connstr, "SELECT 1")
        failed = not ok
    ...
    @staticmethod
    def _print_notices(sess):
        """Surface a session's captured NOTICE/WARNING output to the test log."""
    ...
    def poll_query_until(
        self, query, expected="t", dbname="postgres", timeout=TIMEOUT_DEFAULT
    ):
        """Run *query* repeatedly until its output equals *expected*.

        Reuses one connection across attempts, reconnecting only if it drops
        (e.g. across a server restart), rather than opening a fresh connection
        every iteration.
        """
        sess = None
        ...
        def _ready():
            ...
            try:
                ...
                if sess is None:
                    sess = self._open_session(dbname)
                res = sess.query(query)
                # An errored query (transient SQL error, or the connection
                # dropped -> status -1) is not a match; let other exceptions
                # (bugs, timeouts) propagate.
                return res.error_message is None and res.psqlout == expected
            except (QueryError, PqConnectionError):
                # Server not accepting connections yet / connection dropped:
                # retry (a dead session is detected via connected() next time).
                return False
    ...
    def slot(self, slot_name):
        ...
        res = self.sql(
            "SELECT " + ", ".join(columns) + " FROM pg_catalog.pg_replication_slots"
    ...
    def pg_recvlogical_upto(
        self, dbname, slot_name, endpos, timeout_secs=None, **plugin_options
    ):
    ...
    def command_checks_all(self, cmd, expected_ret, stdout_res, stderr_res, msg=None):
        return self.pg_bin.command_checks_all(
    ...
    def issues_sql_like(self, cmd, pattern, msg=None):
        ...
        assert re.search(pattern, log), (
            msg or "issues_sql_like"
        ) + f": SQL /{pattern}/ not found in server log\n{log}"
    ...
    def issues_sql_unlike(self, cmd, pattern, msg=None):
        ...
        assert not re.search(pattern, log), (
            msg or "issues_sql_unlike"
        ) + f": SQL /{pattern}/ unexpectedly found in server log\n{log}"
    ...
    def teardown(self):
        if not self.running:
            return
        try:
            self.stop("immediate", fail_ok=True)
        except Exception as exc:  # pylint: disable=broad-exception-caught
            # e.g. pg_ctl stop hung and hit the command timeout.
            print(f'### node "{self.name}": stop failed during teardown: {exc}')
        # Never leak a postmaster past the test: force-kill any survivor.
        if self.postmaster_alive():
            self.kill9()
