...
class Session:
    ...
    def wait_for_async_pattern(self, pattern, timeout=DEFAULT_TIMEOUT):
        ...
        sock = lib.PQsocket(conn)
        start = time.monotonic()
        # Compile only to validate the pattern; the output of the next result
        # is returned whether or not it matches (see below).
    ...
    def exitPipelineMode(self):  # pylint: disable=invalid-name
        return self._lib.PQexitPipelineMode(self._conn)

    def pipelineStatus(self):  # pylint: disable=invalid-name
