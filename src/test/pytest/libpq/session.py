...
from .errors import ConnectionError as PqConnectionError
...
class Session:
    ...
    def __init__(
        ...
        if not re.search(r"\buser\s*=", connstr):
            if user is None:
                user = os.environ.get("USERNAME") if sys.platform == "win32" \
    ...
    def query(self, sql):
        ...
        *sql* may contain several semicolon-separated statements; their output
        ...
        if not lib.PQsendQuery(conn, _enc(sql)):
            ...
            return ResultData(
                status=-1, error_message=_dec(lib.PQerrorMessage(conn))
            )
        ...
        # Roll back an aborted transaction, like psql with on_error_stop => 0.
    ...
    def query_tuples(self, *sql_statements):
        ...
        for sql in sql_statements:
            ...
            if not res.rows:
                continue
            tuples = ["|".join("" if v is None else v for v in row) for row in res.rows]
            results.append("\n".join(tuples))
        return "\n".join(results)
    ...
    def _get_result(self):
        """Fetch the next async result, waiting on the socket (cf _get_result)."""
        lib = self._lib
        conn = self._conn
        sock = lib.PQsocket(conn)
        while lib.PQisBusy(conn):
            lib.PQsocketPoll(sock, 1, 0, -1)  # block until readable
    ...
    def wait_for_async_pattern(self, pattern, timeout=DEFAULT_TIMEOUT):
        """Wait for an async result whose psqlout matches *pattern*."""
        lib = self._lib
        conn = self._conn
        sock = lib.PQsocket(conn)
        start = time.monotonic()
        ...
        while True:
            ...
            if not lib.PQisBusy(conn):
                result = lib.PQgetResult(conn)
                if result:
                    res = extract_result_data(lib, result, conn)
                    lib.PQclear(result)
                    while True:
                        ...
                        if not extra:
                            break
                        lib.PQclear(extra)
                    # Return the output whether or not it matched, so the
                    # caller can inspect it.
                    return res.psqlout
            if time.monotonic() - start > timeout:
                raise TimeoutError("timed out waiting for async result")
            end_time = lib.PQgetCurrentTimeUSec() + 1_000_000
            lib.PQsocketPoll(sock, 1, 0, end_time)

    def try_get_async_result(self):
        """Non-blocking: ResultData if ready, else None."""
        lib = self._lib
        conn = self._conn
        lib.PQconsumeInput(conn)
        if lib.PQisBusy(conn):
            return None
        result = lib.PQgetResult(conn)
        if not result:
            return None
    ...
    def isnonblocking(self):
        return self._lib.PQisnonblocking(self._conn)

    def enterPipelineMode(self):
        return self._lib.PQenterPipelineMode(self._conn)

    def exitPipelineMode(self):
        return self._lib.PQexitPipelineMode(self._conn)

    def pipelineStatus(self):
        return self._lib.PQpipelineStatus(self._conn)

    def pipelineSync(self):
    ...
    def query_tuples_pipelined(self, *queries):
        ...
        for sql in queries:
            if not lib.PQsendQueryParams(conn, _enc(sql), 0, None, None, None, None, 0):
                ...
                raise QueryError("Failed to send query: " + (_dec(lib.PQerrorMessage(conn)) or ""))
