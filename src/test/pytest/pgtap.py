...
def pytest_runtest_makereport(item, call):
    """Stash each phase's report on the item so fixtures can tell, at teardown,
    whether the test failed (used to keep data dirs on failure)."""
    outcome = yield
    setattr(item, f"_phase_{call.when}", outcome.get_result())
...
def pytest_runtest_logfinish(nodeid, location):  # noqa: ARG001
    ...
    # Check failed before skipped: a test can be reported skipped (setup/call)
    # and then have a teardown that fails; the failure must win, otherwise a
    # broken finalizer on a skipped test would be emitted as "ok # skip".
    if rec["failed"]:
