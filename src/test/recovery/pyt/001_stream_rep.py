...
def advance_wal(node, num):
    ...
    for _ in range(num):
        node.safe_sql("SELECT pg_logical_emit_message(false, '', 'foo')")
...
def test_001_stream_rep(create_pg):
    ...
    node_primary.safe_sql("CREATE TABLE tab_int AS SELECT generate_series(1,1002) AS a")
...
"""
    ...
    check_target_session_attrs(
        node_primary, node_standby_1, node_primary, "read-write", 0
    )

    # Connect to primary in "read-write" mode with standby1,primary list.
    check_target_session_attrs(
        node_standby_1, node_primary, node_primary, "read-write", 0
    )

    # Connect to primary in "any" mode with primary,standby1 list.
    check_target_session_attrs(node_primary, node_standby_1, node_primary, "any", 0)

    # Connect to standby1 in "any" mode with standby1,primary list.
    check_target_session_attrs(node_standby_1, node_primary, node_standby_1, "any", 0)

    # Connect to primary in "primary" mode with primary,standby1 list.
    check_target_session_attrs(node_primary, node_standby_1, node_primary, "primary", 0)

    # Connect to primary in "primary" mode with standby1,primary list.
    check_target_session_attrs(node_standby_1, node_primary, node_primary, "primary", 0)

    # Connect to standby1 in "read-only" mode with primary,standby1 list.
    check_target_session_attrs(
        node_primary, node_standby_1, node_standby_1, "read-only", 0
    )

    # Connect to standby1 in "read-only" mode with standby1,primary list.
    check_target_session_attrs(
        node_standby_1, node_primary, node_standby_1, "read-only", 0
    )

    # Connect to primary in "prefer-standby" mode with primary,primary list.
    check_target_session_attrs(
        node_primary, node_primary, node_primary, "prefer-standby", 0
    )

    # Connect to standby1 in "prefer-standby" mode with primary,standby1 list.
    check_target_session_attrs(
        node_primary, node_standby_1, node_standby_1, "prefer-standby", 0
    )

    # Connect to standby1 in "prefer-standby" mode with standby1,primary list.
    check_target_session_attrs(
        node_standby_1, node_primary, node_standby_1, "prefer-standby", 0
    )

    # Connect to standby1 in "standby" mode with primary,standby1 list.
    check_target_session_attrs(
        node_primary, node_standby_1, node_standby_1, "standby", 0
    )

    # Connect to standby1 in "standby" mode with standby1,primary list.
    check_target_session_attrs(
        node_standby_1, node_primary, node_standby_1, "standby", 0
    )

    # Fail to connect in "read-write" mode with standby1,standby2 list.
    check_target_session_attrs(node_standby_1, node_standby_2, None, "read-write", 2)

    # Fail to connect in "primary" mode with standby1,standby2 list.
    check_target_session_attrs(node_standby_1, node_standby_2, None, "primary", 2)

    # Fail to connect in "read-only" mode with primary,primary list.
    check_target_session_attrs(node_primary, node_primary, None, "read-only", 2)
...
GRANT pg_read_all_settings TO repl_role;"""
    ...
    with Session(connstr=connstr_rep, libdir=node_primary.libdir) as sess:
        ...
        assert res.error_message is None, "READ_REPLICATION_SLOT exit code 0 on success"
        assert re.search(
            r"^\|\|$", res.psqlout
    ...
    assert res.error_message is None, "physical slot created on intermediate replica"
    ...
    assert catalog_xmin == "", "catalog xmin of cascaded slot null with no hs_feedback"
    ...
    def replay_check():
        ...
        assert (
            node_standby_1.safe_sql(f"SELECT 1 FROM replayed WHERE val = {newval}")
            == "1"
        ), f"standby_1 didn't replay primary value {newval}"
        assert (
            node_standby_2.safe_sql(f"SELECT 1 FROM replayed WHERE val = {newval}")
            == "1"
    ...
    xmin, catalog_xmin = get_slot_xmins(
        node_primary, slotname_1, "xmin IS NOT NULL AND catalog_xmin IS NULL"
    )
    ...
    xmin1, catalog_xmin1 = get_slot_xmins(
        node_standby_1, slotname_2, "xmin IS NOT NULL AND catalog_xmin IS NULL"
    )
...
"""
    ...
    xmin2, catalog_xmin2 = get_slot_xmins(node_primary, slotname_1, f"xmin <> '{xmin}'")
    ...
    assert stats_reads == "t", "WAL receiver generates statistics for WAL writes"
    ...
    assert (
        phys_restart_lsn_pre == phys_restart_lsn_post
