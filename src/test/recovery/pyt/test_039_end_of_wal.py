...
def build_record_header(xl_tot_len, xl_xid=0, xl_prev=0, xl_info=0,
                        xl_rmid=0, xl_crc=0):
...
def build_page_header(xlp_magic, xlp_info=0, xlp_tli=0, xlp_pageaddr=0,
                      xlp_rem_len=0):
...
def test_039_end_of_wal(create_pg, bindir):
    ...
    (magic,) = scan_server_header(
        bindir, "access/xlog_internal.h",
        r"#define\s+XLOG_PAGE_MAGIC\s+(\w+)")
    xlp_page_magic = int(magic, 16)
    (contrecord,) = scan_server_header(
        bindir, "access/xlog_internal.h",
        r"#define\s+XLP_FIRST_IS_CONTRECORD\s+(\w+)")
...
""")
    ...
    node.start()
    assert node.log_contains(
        "invalid magic number 0000 .* LSN .*", log_size), "xlp_magic zero"
    ...
    node.write_wal(tli, end_lsn, wal_segment_size,
                   build_record_header(2 * 1024 * 1024 * 1024, 0, prev_lsn))
    node.write_wal(tli, start_of_next_page(end_lsn, wal_block_size),
                   wal_segment_size, build_page_header(0xcafe, 0, 1, 0))
    log_size = node.log_position()
    node.start()
    assert node.log_contains(
        "invalid magic number CAFE .* LSN .*", log_size), "xlp_magic bad"
    ...
    node.write_wal(
        ...
        wal_segment_size,
        build_page_header(
            xlp_page_magic, 0x1234, 1,
            start_of_next_page(end_lsn, wal_block_size)))
    log_size = node.log_position()
    node.start()
    assert node.log_contains(
        "invalid info bits 1234 in .*, LSN .*,", log_size), "xlp_info bad"
    ...
    node.write_wal(tli, start_of_next_page(end_lsn, wal_block_size),
                   wal_segment_size,
                   build_page_header(xlp_page_magic, 0, 1,
    ...
    node.write_wal(
        ...
        build_page_header(
            ...
            1, start_of_next_page(end_lsn, wal_block_size),
            123456))
    log_size = node.log_position()
    node.start()
    assert node.log_contains(
