...
def test_003_recovery_targets(create_pg):
    ...
    def test_recovery_standby(
        test_name, node_name, node_primary, recovery_params, num_rows, until_lsn
    ):
        node_standby = create_pg(node_name, start=False)
        node_standby.init_from_backup(node_primary, "my_backup", has_restoring=True)
        ...
        assert result == str(num_rows), f"check standby content for {test_name}"
    ...
    node_primary.safe_sql("CREATE TABLE tab_int AS SELECT generate_series(1,1000) AS a")
    ...
    # target TXID.
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(1001,2000))")
    ret = node_primary.safe_sql("SELECT pg_current_wal_lsn(), pg_current_xact_id();")
    lsn2, recovery_txid = ret.split("|")

    # More data, with recovery target timestamp
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(2001,3000))")
    lsn3 = node_primary.safe_sql("SELECT pg_current_wal_lsn();")
    recovery_time = node_primary.safe_sql("SELECT now()")

    # Even more data, this time with a recovery target name
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(3001,4000))")
    recovery_name = "my_target"
    lsn4 = node_primary.safe_sql("SELECT pg_current_wal_lsn();")
    node_primary.safe_sql(f"SELECT pg_create_restore_point('{recovery_name}');")

    # And now for a recovery target LSN
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(4001,5000))")
    lsn5 = recovery_lsn = node_primary.safe_sql("SELECT pg_current_wal_lsn()")

    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(5001,6000))")
    ...
    test_recovery_standby(
        "immediate target", "standby_1", node_primary, recovery_params, "1000", lsn1
    )
    recovery_params = [f"recovery_target_xid = '{recovery_txid}'"]
    test_recovery_standby(
        "XID", "standby_2", node_primary, recovery_params, "2000", lsn2
    )
    recovery_params = [f"recovery_target_time = '{recovery_time}'"]
    test_recovery_standby(
        "time", "standby_3", node_primary, recovery_params, "3000", lsn3
    )
    recovery_params = [f"recovery_target_name = '{recovery_name}'"]
    test_recovery_standby(
        "name", "standby_4", node_primary, recovery_params, "4000", lsn4
    )
    recovery_params = [f"recovery_target_lsn = '{recovery_lsn}'"]
    test_recovery_standby(
        "LSN", "standby_5", node_primary, recovery_params, "5000", lsn5
    )
    ...
    assert re.search(
        r"multiple recovery targets specified", logfile
    ), "multiple conflicting settings"

    # Check behavior when recovery ends before target is reached
    node_standby = create_pg("standby_8", start=False)
    node_standby.init_from_backup(
        node_primary, "my_backup", has_restoring=True, standby=False
    )
    ...
    res = node_primary.sql("ALTER SYSTEM SET recovery_target_timeline TO 'bogus'")
    assert re.search(
        r"is not a valid number", res.error_message or ""
    ), "invalid recovery_target_timeline (bogus value)"

    res = node_primary.sql("ALTER SYSTEM SET recovery_target_timeline TO '0'")
    assert re.search(
        r"must be between 1 and 4294967295", res.error_message or ""
    ), "invalid recovery_target_timeline (lower bound check)"

    res = node_primary.sql("ALTER SYSTEM SET recovery_target_timeline TO '4294967296'")
    assert re.search(
        r"must be between 1 and 4294967295", res.error_message or ""
    ), "invalid recovery_target_timeline (upper bound check)"

    # Invalid recovery_target_xid tests
    res = node_primary.sql("ALTER SYSTEM SET recovery_target_xid TO 'bogus'")
    assert re.search(
        r"is not a valid number", res.error_message or ""
    ), "invalid recovery_target_xid (bogus value)"

    res = node_primary.sql("ALTER SYSTEM SET recovery_target_xid TO '-1'")
    assert re.search(
        r"is not a valid number", res.error_message or ""
    ), "invalid recovery_target_xid (negative)"

    res = node_primary.sql("ALTER SYSTEM SET recovery_target_xid TO '0'")
    assert re.search(
        r"without epoch must be greater than or equal to 3", res.error_message or ""
