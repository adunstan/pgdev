...
def _async_died(session, pattern):
    ...
    if res is None:
        ...
        assert (
            session.conn_status() != ConnStatusType.CONNECTION_OK
        ), "session should have died after crash"
        return
    msg = (res.error_message or "") + (res.psqlout or "")
    assert pattern.search(msg), f"session died successfully after crash; got: {msg!r}"
...
def test_013_crash_restart(create_pg):
    ...
    stats_reset = node.safe_sql("SELECT stats_reset FROM pg_stat_statements_info")
    ...
    try:
        ...
        assert res.error_message is None, res.error_message
        assert re.search("in-progress-before-sigquit", res.psqlout), res.psqlout
    ...
    assert node.poll_query_until("SELECT 1", expected="1"), "reconnected after SIGQUIT"
    ...
    try:
        ...
        assert res.error_message is None, res.error_message
        assert re.search("in-progress-before-sigkill", res.psqlout), res.psqlout
    ...
    # Wait till server restarts
    assert node.poll_query_until("SELECT 1", expected="1"), "reconnected after SIGKILL"

    # Make sure the committed rows survived, in-progress ones not
    assert (
        node.safe_sql("SELECT * FROM alive")
