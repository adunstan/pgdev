...
""")
    ...
    caughtup_query = (
        f"SELECT '{cur_primary_lsn}'::pg_lsn <= pg_last_wal_replay_lsn()")
    assert cur_standby.poll_query_until(caughtup_query), (
...
26|issued to london"""
    ...
    assert psql_out == "27|issued to paris", (
    ...
    cur_primary.pg_bin.command_ok(
        ["pgbench", "--no-vacuum", "--client=5", "--transactions=1000",
         "-f", pgb_script, "postgres"],
        "pgbench run to cause pg_subtrans traffic")
