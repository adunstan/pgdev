...
"""
    ...
    tli = int(primary.safe_sql("SELECT timeline_id FROM pg_control_checkpoint()"))
    ...
    standby1.init_from_backup(primary, "backup", standby=True, has_restoring=True)

    standby2 = create_pg("standby2", start=False)
    standby2.init_from_backup(primary, "backup", standby=True, has_restoring=True)
    ...
    standby1.safe_sql("INSERT INTO t SELECT * FROM generate_series(1, 1000)")
