...
def _verify(primary, standby, counter, message):
    ...
    assert (
        primary.safe_sql(query, dbname="conflict_db") == f"{counter}|4000"
    ), f"primary: {message}"

    primary.wait_for_catchup(standby)
    assert (
        standby.safe_sql(query, dbname="conflict_db") == f"{counter}|4000"
    ), f"standby: {message}"
...
"""
    ...
    node_primary.safe_sql("CREATE DATABASE conflict_db_template OID = 50000;")
    ...
    try:
        ...
        node_primary.safe_sql("UPDATE large SET datab = 1;", dbname="conflict_db")
        ...
        node_primary.safe_sql("UPDATE large SET datab = 2;", dbname="conflict_db")
        ...
        node_primary.safe_sql("UPDATE large SET datab = 3;", dbname="conflict_db")

        _verify(node_primary, node_standby, 3, "restored contents as expected")
        ...
        # cause dirty buffers
        node_primary.safe_sql("UPDATE large SET datab = 4;", dbname="conflict_db")
        ...
        node_primary.safe_sql("ALTER DATABASE conflict_db SET TABLESPACE pg_default")

        # cause dirty buffers
        node_primary.safe_sql("UPDATE large SET datab = 5;", dbname="conflict_db")
        _cause_eviction(psql_primary, psql_standby)

        _verify(node_primary, node_standby, 5, "post move contents as expected")

        node_primary.safe_sql(
            "ALTER DATABASE conflict_db SET TABLESPACE test_tablespace"
        )

        node_primary.safe_sql("UPDATE large SET datab = 7;", dbname="conflict_db")
        _cause_eviction(psql_primary, psql_standby)
        node_primary.safe_sql("UPDATE large SET datab = 8;", dbname="conflict_db")
        node_primary.safe_sql("DROP DATABASE conflict_db")
