...
"""
    ...
    node_primary.safe_sql(f"CREATE TABLESPACE {tablespace1} LOCATION ''")
...
""",
    ...
    try:
        ...
        check_conflict_log("User was holding shared buffer pin for too long", sect)
        ...
        assert res == "0", f"{sect}: cursor with conflicting snapshot established"
        ...
        assert res == "6000", f"{sect}: cursor with conflicting temp file established"
        ...
        check_conflict_log(
            "User was or might have been using tablespace that must be dropped", sect
        )
        psql_standby.reconnect()
        ...
        # what we hit first
        node_standby.append_conf(f"max_standby_streaming_delay = {TIMEOUT_DEFAULT}s")
        psql_standby.close()
        ...
        node_primary.safe_sql(f"CREATE TABLE {table1}(a int, b int);", dbname=test_db)
...
""",
        ...
        node_primary.safe_sql(f"INSERT INTO {table1}(a) VALUES (170);", dbname=test_db)
        ...
        assert node_standby.safe_sql(
            "SELECT conflicts FROM pg_stat_database " f"WHERE datname='{test_db}';",
            dbname=test_db,
        ) == str(
            expected_conflicts
