...
"""
    ...
    assert os.path.isfile(
        os.path.join(data_dir, archive_cleanup_command_file)
    ), "archive_cleanup_command executed on checkpoint"
    assert not os.path.isfile(
        os.path.join(data_dir, recovery_end_command_file)
...
"""
    ...
    assert os.path.isfile(
        os.path.join(node_standby2_data, "recovery.signal")
    ), "recovery.signal is present at the beginning of recovery"
    assert os.path.isfile(
        os.path.join(node_standby2_data, "standby.signal")
    ...
    assert (
        'restored log file "00000002.history" from archive' in log_contents
    ...
    assert re.search(
        r"WARNING:.*recovery_end_command", log_contents, re.S
