...
def test_020_archive_status(create_pg):
    ...
    primary.safe_sql(f"ALTER SYSTEM SET archive_command TO '{incorrect_command}'")
    ...
    segment_name_1 = primary.safe_sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
    ...
    assert os.path.isfile(os.path.join(primary_data, segment_path_1_ready)), (
        f".ready file exists for WAL segment {segment_name_1} " "waiting to be archived"
    )
    ...
    assert not os.path.isfile(
        os.path.join(primary_data, segment_path_1_ready)
    ), f".ready file for archived WAL segment {segment_name_1} removed"

    assert os.path.isfile(
        os.path.join(primary_data, segment_path_1_done)
    ), f".done file for archived WAL segment {segment_name_1} exists"

    assert (
        primary.safe_sql("SELECT last_archived_wal FROM pg_stat_archiver")
        == segment_name_1
    ), (
    ...
    segment_name_2 = primary.safe_sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
    ...
    assert (
        standby2.safe_sql("SELECT archived_count FROM pg_stat_archiver") == "2"
    ), "correct number of WAL segments archived from standby"

    assert not os.path.isfile(
        os.path.join(standby2_data, segment_path_1_ready)
    ) and not os.path.isfile(
        os.path.join(standby2_data, segment_path_2_ready)
    ), ".ready files removed after archive success with archive_mode=always on standby"

    assert os.path.isfile(
        os.path.join(standby2_data, segment_path_1_done)
    ) and os.path.isfile(
        os.path.join(standby2_data, segment_path_2_done)
    ...
    assert re.search(
        r"archiver process shutting down", logfile
    ...
    try:
        ...
        assert re.search(
            r"backup label too long", str(exc)
        ), "pg_backup_start fails gracefully"

    primary.safe_sql("SELECT pg_backup_start('onebackup'); SELECT pg_backup_stop();")
