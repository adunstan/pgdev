...
def test_046_checkpoint_logical_slot(create_pg):
    ...
    node.safe_sql(
        "select pg_create_logical_replication_slot('slot_logical', "
        "'test_decoding')")
    node.safe_sql(
    ...
    print("# waiting for injection_point")
    standby.wait_for_event(
        "checkpointer", "restartpoint-before-slot-invalidation")
    print("# injection_point is reached")
