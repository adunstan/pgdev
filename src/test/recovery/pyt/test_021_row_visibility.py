...
def test_021_row_visibility(create_pg):
    ...
    try:
        ...
        assert _standby_rows(psql_standby) == ["first insert"], \
            "insert visible"
        ...
        assert _standby_rows(psql_standby) == ["first update"], \
        ...
        assert psql_primary.do(
            "DELETE from test_visibility") is not None, "DELETE"
        res = psql_primary.query(
            "BEGIN;"
        ...
        assert _standby_rows(psql_standby) == \
            ["inserted in prepared will_commit"], "finished prepared visible"
    finally:
        psql_primary.close()
        psql_standby.close()
