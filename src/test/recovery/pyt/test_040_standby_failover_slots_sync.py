...
def expect_error(node, sql, pattern, msg, dbname="postgres", replication=None):
    ...
    with Session(connstr=connstr, libdir=node.libdir) as sess:
        res = sess.query(sql)
        assert res.error_message is not None and re.search(pattern, res.error_message), \
...
def test_040_standby_failover_slots_sync(create_pg):
    ...
    publisher_connstr = (
        f"host={publisher.host} port={publisher.port} dbname=postgres"
    )
    ...
    assert publisher.safe_sql(
        "SELECT failover from pg_replication_slots WHERE slot_name = 'lsub1_slot';"
    ) == "t", "logical slot has failover true on the publisher"
    ...
    assert publisher.safe_sql(
        "SELECT failover from pg_replication_slots WHERE slot_name = 'lsub1_slot';"
    ) == "f", "logical slot has failover false on the publisher"
    ...
    ) == "t", "synchronized slot has got its own inactive_since"
    ...
    ) == "t", "synchronized slot has been dropped"
    ...
    ) == "t", "synchronized slot has been invalidated"
...
"""
    ...
    assert standby2.safe_sql(
        f"SELECT count(*) = {primary_row_count} FROM tab_int;"
    ) == "t", "standby2 gets data from primary"
    ...
    assert subscriber2.safe_sql(
        f"SELECT count(*) = {primary_row_count} FROM tab_int;"
    ) == "t", "subscriber2 gets data from primary"
    ...
    assert standby1.safe_sql(
        f"SELECT count(*) = {primary_row_count} FROM tab_int;"
    ) == "t", "standby1 gets data from primary"
    ...
    assert subscriber1.safe_sql(
        f"SELECT count(*) = {primary_row_count} FROM tab_int;"
    ) == "t", "subscriber1 gets data from primary after standby1 acknowledges changes"
    ...
    ) == "t", "subscriber1 doesn't get data as the sb1_slot doesn't catch up"
    ...
    ) == "t", "the prepared transaction is not replicated to the subscriber"
    ...
    ) == "t", "synced slot retained on the new primary"
    ...
    assert subscriber1.safe_sql("SELECT count(*) FROM tab_int;") == "11", \
    ...
    assert subscriber1.safe_sql("SELECT count(*) FROM tab_int;") == "21", \
    ...
    assert standby1.safe_sql(
        ...
        "WHERE data ~ 'message*';"
    ) == "1", "data can be consumed using snap_test_slot"
    ...
    ) == "t", "all replication slots have been dropped except the physical slot used by standby2"
    ...
    ) == "wal_or_rows_removed", "check slot sync skip reason"
    ...
    ) == "t", "check slot sync skip count increments"
