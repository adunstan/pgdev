...
""")
    ...
    assert not re.search(
        ...
        notices), "invalid databases are ignored by vac_truncate_clog"
    ...
    assert bgpsql.do(
        ...
        "BEGIN;\n"
        "LOCK pg_tablespace;\n"
        "PREPARE TRANSACTION 'lock_tblspc';") == 1
    ...
    assert cancel_res.status == ExecStatusType.PGRES_TUPLES_OK, \
    ...
    assert ii_connect_err is not None, \
        "can't connect to invalid_interrupt database"

    # To properly drop the database, we need to release the lock previously
    # preventing doing so.
    assert bgpsql.do("ROLLBACK PREPARED 'lock_tblspc'") == \
        ExecStatusType.PGRES_COMMAND_OK, "unblock DROP DATABASE"
