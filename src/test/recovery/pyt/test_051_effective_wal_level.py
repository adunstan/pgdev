...
def test_051_effective_wal_level(create_pg):
    ...
    primary.safe_sql(
        "select pg_create_physical_replication_slot('test_phy_slot', "
        "false, false)")
    check_wal_level(
        primary, "replica|replica",
    ...
    assert primary.log_contains(
        "logical decoding is enabled upon creating a new logical "
        "replication slot"
    ...
    check_wal_level(
        primary, "replica|logical",
        "effective_wal_level increased to 'logical' upon a logical slot "
        "creation")
    ...
    assert primary.log_contains(
        r'logical replication slot "test_slot" exists, '
        r'but "wal_level" < "replica"',
        log_offset
    ...
    ) == "t", "test_slot gets invalidated due to wal_removed"
    ...
    standby1.promote()
    check_wal_level(
        standby1, "replica|logical",
        "effective_wal_level remains 'logical' even after the promotion")

    # Confirm if we can create a logical slot after the promotion.
    standby1.safe_sql(
        "select pg_create_logical_replication_slot('standby1_slot2', "
        "'pgoutput')")
    standby1.stop()
    ...
    if injection_available:
        ...
        psql_create_slot.do(
            "select injection_points_attach("
            "'logical-decoding-activation', 'wait')")
        psql_create_slot.do_async(
            "select pg_create_logical_replication_slot('slot_canceled', "
            "'pgoutput')")
