...
def test_010_logical_decoding_timelines(create_pg):
    ...
    node_primary = create_pg("primary", allows_streaming="logical",
                             has_archiving=True, start=False)
...
""")
    ...
    node_primary.safe_sql(
        "SELECT pg_create_logical_replication_slot('dropme_slot', "
        "'test_decoding');",
        dbname="dropme")
    ...
    assert node_replica.safe_sql(
        "SELECT 1 FROM pg_database WHERE datname = 'dropme'") == "", \
        "dropped DB dropme on standby"
    assert node_replica.slot("dropme_slot")["plugin"] == "", \
    ...
    assert stdout == "before_basebackup", \
    ...
    assert res.error_message is not None, \
        "replaying from after_basebackup slot fails"
    assert re.search(
        r'replication slot "after_basebackup" does not exist',
        res.error_message), "after_basebackup slot missing"
    ...
    assert res.error_message is None, \
        "replay from slot before_basebackup succeeds"
    ...
    assert stdout == final_expected_output_bb, \
    ...
    result = node_replica.pg_recvlogical_upto(
        ...
        endpos, TIMEOUT_DEFAULT,
        **{"include-xids": "0", "skip-empty-xacts": "1"})

    # walsender likes to add a newline
    stdout = result.stdout.rstrip("\n")
    assert stdout == final_expected_output_bb, \
        ("got same output from walsender via pg_recvlogical on "
         "before_basebackup")
