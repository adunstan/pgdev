...
def test_005_replay_delay(create_pg):
    ...
    # And some content
    node_primary.safe_sql("CREATE TABLE tab_int AS SELECT generate_series(1, 10) AS a")
...
"""
    ...
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(11, 20))")
    ...
    assert (
        node_standby2.safe_sql("SELECT pg_get_wal_replay_pause_state()") == "not paused"
    ...
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(21,30))")
    ...
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(31,40))")
    assert node_standby2.poll_query_until(
        f"SELECT '{receive_lsn}'::pg_lsn < pg_last_wal_receive_lsn()"
    ), "Timed out while waiting for new WAL to be streamed"
    assert (
        node_standby2.safe_sql("SELECT pg_last_wal_replay_lsn()") == replay_lsn
    ...
    node_standby2.safe_sql("SELECT pg_wal_replay_pause()")
    node_primary.safe_sql("INSERT INTO tab_int VALUES (generate_series(41,50))")
    assert node_standby2.poll_query_until(
        "SELECT pg_get_wal_replay_pause_state() = 'paused'"
    ), "Timed out while waiting for recovery to be paused"
