...
"""
    ...
    node_primary.safe_sql("SELECT pg_create_physical_replication_slot('rep1')")
    ...
    wait_for_slot_catchup(node_primary, "rep1", "restart", node_primary.lsn("write"))
    ...
    assert result == "reserved|t", "check that it is safe if WAL fits in max_wal_size"
...
"""
    ...
    assert result == "reserved", "check that slot remains reserved after advancing WAL"
    ...
    assert result == "unreserved|t", 'check that the slot state changes to "unreserved"'
...
"""
    ...
    node_primary2.safe_sql("SELECT pg_create_physical_replication_slot('rep1')")
...
"""
    ...
    node_primary3.safe_sql("SELECT pg_create_physical_replication_slot('rep3')")
    # Take backup
    backup_name = "my_backup"
    ...
    while True:
        ...
        if i == 10 * TIMEOUT_DEFAULT:
            ...
            raise RuntimeError("could not determine walsender pid, can't continue")
        i += 1

        time.sleep(0.1)

    assert re.fullmatch(r"[0-9]+", senderpid), f"have walsender pid {senderpid}"

    receiverpid = node_standby3.safe_sql(
        "SELECT pid FROM pg_stat_activity WHERE backend_type = 'walreceiver'"
    )
    assert re.fullmatch(r"[0-9]+", receiverpid), f"have walreceiver pid {receiverpid}"
    ...
    assert (
        primary4.safe_sql(
            ...
            f"WHERE slot_name = '{sb4_slot}' AND inactive_since IS NOT NULL"
        )
        == "t"
