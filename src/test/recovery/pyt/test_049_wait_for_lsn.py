...
def _stop_walreceiver(node):
    ...
    _resume_walreceiver().
    """
    global _saved_primary_conninfo  # pylint: disable=global-statement
...
def _resume_walreceiver(node):
    ...
    """
    node.safe_sql(f"ALTER SYSTEM SET primary_conninfo = {_saved_primary_conninfo};")
    node.safe_sql("SELECT pg_reload_conf();")
...
def test_049_wait_for_lsn(create_pg):
    ...
    node_primary.safe_sql("CREATE TABLE wait_test AS SELECT generate_series(1,10) AS a")
...
"""
    ...
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(11, 20))")
    ...
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(21, 30))")
    ...
    # primary_flush modes.
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(31, 40))")
    ...
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(41, 50))")
    ...
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(51, 60))")
    lsn_primary_flush = node_primary.safe_sql("SELECT pg_current_wal_insert_lsn()")
    ...
    lsn3 = node_primary.safe_sql("SELECT pg_current_wal_insert_lsn() + 10000000000")
    node_standby.safe_sql(f"WAIT FOR LSN '{lsn2}' WITH (timeout '10ms');")
    res = node_standby.sql(f"WAIT FOR LSN '{lsn3}' WITH (timeout '1000ms');")
    assert res.error_message is not None
    assert re.search(
        r"timed out while waiting for target LSN", res.error_message
    ...
    try:
        ...
        assert re.search(
            r"canceling statement due to user request", subxact_cancel_res.error_message
        ...
        # is still usable.
        assert (
            subxact_session.query_oneval("SELECT 1") == "1"
    ...
    res = node_primary.sql(f"WAIT FOR LSN '{lsn3}' WITH (MODE 'standby_flush');")
    ...
    res = node_standby.sql(f"WAIT FOR LSN '{lsn3}' WITH (MODE 'primary_flush');")
    assert res.error_message and re.search(
        r"recovery is in progress", res.error_message
    ), "get an error when running primary_flush on the standby"

    res = node_standby.sql(
        "BEGIN ISOLATION LEVEL REPEATABLE READ; SELECT 1; " f"WAIT FOR LSN '{lsn3}';"
    )
    assert res.error_message and re.search(
...
"""
    ...
    assert res.error_message and re.search(
        r"WAIT FOR can only be executed as a top-level statement", res.error_message
    ...
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (timeout '-1000ms');")
    assert res.error_message and re.search(
        r"timeout cannot be negative", res.error_message
    ), "get error for negative timeout"

    # Test unknown parameter with WITH clause
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (unknown_param 'value');")
    assert res.error_message and re.search(
    ...
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (no_throw, no_throw);")
    assert res.error_message and re.search(
        r"conflicting or redundant options", res.error_message
    ), "get error for duplicate NO_THROW parameter"

    # Test syntax error - options without WITH keyword
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' (timeout '100ms');")
    assert res.error_message and re.search(
    ...
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (timeout 'invalid');")
    assert res.error_message and re.search(
    ...
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (invalid_option 'value');")
    assert res.error_message and re.search(
        r'option "invalid_option" not recognized', res.error_message
    ), "get error for invalid WITH clause option"

    # Test invalid MODE value
    res = node_standby.sql(f"WAIT FOR LSN '{test_lsn}' WITH (MODE 'invalid');")
    assert res.error_message and re.search(
        r'unrecognized value for WAIT option "mode": "invalid"', res.error_message
...
"""
    ...
    for i in range(5):
        ...
        sess.do_async(f"WAIT FOR LSN '{lsn}';\n" f"SELECT log_count({i});")
    ...
    for i in range(5):
        ...
        write_lsns.append(node_primary.safe_sql("SELECT pg_current_wal_insert_lsn()"))
    ...
    for i in range(5):
        ...
        flush_lsns.append(node_primary.safe_sql("SELECT pg_current_wal_insert_lsn()"))
    ...
    output = node_standby.safe_sql(
        "SELECT pg_lsn_cmp(pg_last_wal_receive_lsn(), " f"'{flush_lsns[4]}'::pg_lsn);"
    )
    assert (
        int(output) >= 0
    ...
    node_primary.safe_sql("INSERT INTO wait_test VALUES (generate_series(301, 310));")
    mixed_target_lsn = node_primary.safe_sql("SELECT pg_current_wal_insert_lsn()")
    ...
    node_standby.safe_sql("SELECT pg_wal_replay_resume();")
    assert node_standby.poll_query_until("SELECT NOT pg_is_wal_replay_paused();")
    ...
    for i in range(5):
        node_primary.wait_for_log(f"primary_flush_done {i}", primary_flush_log_offset)
    ...
    lsn4 = node_primary.safe_sql("SELECT pg_current_wal_insert_lsn() + 10000000000")
    ...
    for i in range(3):
        ...
        sess.do_async(f"WAIT FOR LSN '{lsn4}' WITH (MODE '{wait_modes[i]}');")
    ...
    arc_primary.safe_sql("CREATE TABLE arc_test AS SELECT generate_series(1,10) AS a")
    ...
    arc_primary.safe_sql("INSERT INTO arc_test VALUES (generate_series(11, 20))")
    arc_target_lsn = arc_primary.safe_sql("SELECT pg_current_wal_insert_lsn()")

    # Force WAL to be archived by switching segments, then wait for archiving.
    arc_segment = arc_primary.safe_sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
    ...
    arc_standby.init_from_backup(arc_primary, arc_backup_name, has_restoring=True)
    arc_standby.start()
    ...
    output = arc_standby.safe_sql("SELECT count(*) FROM pg_stat_wal_receiver")
    ...
    arc_primary.safe_sql("INSERT INTO arc_test VALUES (generate_series(21, 30))")
    arc_target_lsn2 = arc_primary.safe_sql("SELECT pg_current_wal_insert_lsn()")

    arc_segment2 = arc_primary.safe_sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
    arc_primary.safe_sql("SELECT pg_switch_wal()")
    assert arc_primary.poll_query_until(
    ...
    rcv_primary.restart()
    rcv_primary.safe_sql("CREATE TABLE rcv_test AS SELECT generate_series(1,10) AS a")
    ...
    # final replay LSN.
    rcv_primary.safe_sql("INSERT INTO rcv_test VALUES (generate_series(11, 100))")
    rcv_primary.safe_sql("SELECT pg_switch_wal()")
    rcv_primary.safe_sql("INSERT INTO rcv_test VALUES (generate_series(101, 110))")
    ...
    rcv_primary.start()
    rcv_primary.safe_sql("INSERT INTO rcv_test VALUES (generate_series(111, 120))")
    rcv_primary.wait_for_catchup(rcv_standby)
    ...
    rcv_primary.safe_sql("INSERT INTO rcv_test VALUES (generate_series(200, 210))")
    ...
    tl_primary.restart()
    tl_primary.safe_sql("CREATE TABLE tl_test AS SELECT generate_series(1, 10) AS a")
    ...
    tl_primary.safe_sql("INSERT INTO tl_test VALUES (generate_series(11, 20))")
    ...
    tl_standby1.promote()
    tl_standby1.safe_sql("INSERT INTO tl_test VALUES (generate_series(21, 1020))")
