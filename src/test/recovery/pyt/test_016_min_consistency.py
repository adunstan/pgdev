...
def find_largest_lsn(blocksize, filename):
    """Find the largest LSN in the set of pages part of the given relation file.

    This is used for offline checks of page consistency.  The LSN is
    historically stored as a set of two numbers of 4 byte-length located at
    ...
    with open(filename, "rb") as fh:
        while True:
            ...
            assert (
                len(buf) == blocksize
            ), f"read only {len(buf)} of {blocksize} bytes from {filename}"
            hi, lo = struct.unpack("<LL", buf[:8])
            if hi > max_hi or (hi == max_hi and lo > max_lo):
                max_hi, max_lo = hi, lo

    return "%X/%08X" % (max_hi, max_lo)


def test_016_min_consistency(create_pg, pg_bin):
...
"""
    ...
    standby.init_from_backup(primary, "bkp", has_streaming=True)
    standby.start()
...
INSERT INTO test1 SELECT generate_series(1, 10000);"""
    ...
    primary.safe_sql("UPDATE test1 SET a = a + 1;")
    ...
    # previously.
    standby.safe_sql("SELECT count(*) FROM test1;")
    ...
    blocksize = primary.safe_sql(
        "SELECT setting::int FROM pg_settings WHERE name = 'block_size';"
    )
    primary.safe_sql(f"SELECT pg_relation_size('test1')::int / {blocksize} - 1;")
    relfilenode = primary.safe_sql("SELECT pg_relation_filepath('test1'::regclass);")
    ...
    standby_data = standby.data_dir
    offline_max_lsn = find_largest_lsn(
        blocksize, os.path.join(standby_data, relfilenode)
    )

    # Fetch minRecoveryPoint from the control file itself
    res = pg_bin.result(["pg_controldata", standby_data])
    offline_recovery_lsn = None
    for line in res.stdout.split("\n"):
        m = re.match(r"^Minimum recovery ending location:\s*(.*)$", line)
        if m:
            offline_recovery_lsn = m.group(1)
            break
    assert offline_recovery_lsn is not None, "No minRecoveryPoint in control file found"
