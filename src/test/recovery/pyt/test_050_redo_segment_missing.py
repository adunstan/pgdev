...
def test_050_redo_segment_missing(create_pg):
    ...
    redo_lsn = node.safe_sql(
        "SELECT redo_lsn FROM pg_control_checkpoint()")
    redo_walfile_name = node.safe_sql(
        f"SELECT pg_walfile_name('{redo_lsn}')")
    checkpoint_lsn = node.safe_sql(
    ...
    assert redo_walfile_name != checkpoint_walfile_name, \
    ...
    assert re.search(
        ...
        r"referenced by checkpoint record at .*", logfile), \
