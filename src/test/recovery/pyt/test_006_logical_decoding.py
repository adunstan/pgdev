...
def test_006_logical_decoding(create_pg):
    ...
    assert len(result.splitlines()) == 12, \
        "Decoding produced 12 rows inc BEGIN/COMMIT"
    ...
    stdout_recv = node_primary.pg_recvlogical_upto(
        "postgres", "test_slot", endpos, TIMEOUT_DEFAULT,
        **{"include-xids": "0", "skip-empty-xacts": "1"},
    ).stdout
    stdout_recv = stdout_recv.rstrip("\n")
    assert stdout_recv == expected, \
    ...
    assert node_primary.sql(
        "SELECT lsn FROM pg_logical_slot_peek_changes('test_slot', NULL, NULL) "
        "ORDER BY lsn DESC LIMIT 1;",
        "otherdb",
    ).error_message is not None, "replaying logical slot from another database fails"
    ...
    if sys.platform == "win32":
        ...
        pg_recvlogical = subprocess.Popen(
            [
                node_primary._resolve("pg_recvlogical"),
        ...
        try:
            ...
            pg_recvlogical.kill()
            pg_recvlogical.wait()
        assert node_primary.slot("otherdb_slot")["plugin"] == "test_decoding", \
    ...
    assert node_primary.sql("DROP DATABASE otherdb").error_message is None, \
        "dropping a DB with inactive logical slots succeeds"
    assert node_primary.slot("otherdb_slot")["plugin"] == "", \
    ...
    ).error_message is None, "slot advancing with logical slot"
    ...
    assert logical_restart_lsn_pre == logical_restart_lsn_post, \
