...
"""
    ...
    initfile = node.safe_sql("SELECT pg_walfile_name(pg_current_wal_insert_lsn())")
    node.safe_sql(
        "SELECT pg_logical_emit_message(true, 'test 026', repeat('xyzxz', 123456))"
    )

    endfile = node.safe_sql("SELECT pg_walfile_name(pg_current_wal_insert_lsn())")
    ...
    node.safe_sql("SELECT pg_logical_emit_message(true, 'test 026', 'AABBCC')")
    ...
    assert (
        node_standby.safe_sql("select * from foo") == "hello"
