...
def test_034_create_database(create_pg):
    ...
    node.safe_sql(f"CREATE DATABASE {db_new} STRATEGY WAL_LOG TEMPLATE {db_template};")

    node.safe_sql("CREATE TABLE tab_db_after_create_1 (a INT);", dbname=db_template)
    ...
    result = node.safe_sql(
        "SELECT count(*) FROM pg_class WHERE relname LIKE 'tab_db_%';", dbname=db_new
    )
    assert (
        result == "0"
