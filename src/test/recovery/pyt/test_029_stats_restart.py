...
def _wal_stats(node):
    ...
    results["reset"] = node.safe_sql("SELECT stats_reset FROM pg_stat_wal", CONNECT_DB)
    return results
...
def test_029_stats_restart(create_pg, tmp_path):
    ...
    sect = "initial"
    assert _have_stats(node, "database", dboid, 0) == "t", f"{sect}: db stats do exist"
    assert (
        _have_stats(node, "function", dboid, funcoid) == "t"
    ), f"{sect}: function stats do exist"
    assert (
        _have_stats(node, "relation", dboid, tableoid) == "t"
    ...
    sect = "post immediate"
    assert (
        _have_stats(node, "database", dboid, 0) == "f"
    ), f"{sect}: db stats do not exist"
    assert (
        _have_stats(node, "function", dboid, funcoid) == "f"
    ), f"{sect}: function stats do exist"
    assert (
        _have_stats(node, "relation", dboid, tableoid) == "f"
    ...
    assert int(ckpt_start["count"]) < int(
        ckpt_restart["count"]
    ), f"{sect}: increased checkpoint count"
    assert int(wal_start["records"]) < int(
        wal_restart["records"]
    ), f"{sect}: increased wal record count"
    assert int(wal_start["bytes"]) < int(
        wal_restart["bytes"]
    ), f"{sect}: increased wal bytes"
    assert (
        ckpt_start["reset"] == ckpt_restart["reset"]
    ), f"{sect}: checkpoint stats_reset equal"
    assert wal_start["reset"] == wal_restart["reset"], f"{sect}: wal stats_reset equal"
    ...
    assert int(ckpt_restart["count"]) > int(
        ckpt_reset["count"]
    ), f"{sect}: checkpoint count smaller"
    assert ckpt_start["reset"] < ckpt_reset["reset"], f"{sect}: stats_reset newer"

    assert int(wal_restart["records"]) <= int(
        wal_ckpt_reset["records"]
    ), f"{sect}: wal record count not affected by reset"
    assert (
        wal_start["reset"] == wal_ckpt_reset["reset"]
    ...
    # on slow machines
    assert int(ckpt_restart_reset["count"]) < int(
        ckpt_restart["count"]
    ), f"{sect}: checkpoint still reset"
    assert (
        ckpt_restart_reset["reset"] == ckpt_reset["reset"]
    ), f"{sect}: stats_reset same"

    assert int(wal_ckpt_reset["records"]) < int(
        wal_restart2["records"]
    ), f"{sect}: increased wal record count"
    assert int(wal_ckpt_reset["bytes"]) < int(
        wal_restart2["bytes"]
    ), f"{sect}: increased wal bytes"
    assert wal_start["reset"] == wal_restart2["reset"], f"{sect}: wal stats_reset equal"
    ...
    assert int(wal_reset["records"]) < int(
        wal_restart2["records"]
    ), f"{sect}: smaller record count"
    assert int(wal_reset["bytes"]) < int(
        wal_restart2["bytes"]
    ), f"{sect}: smaller bytes"
    assert wal_reset["reset"] > wal_restart2["reset"], f"{sect}: newer stats_reset"
    ...
    assert int(wal_reset_restart["records"]) < int(
        wal_restart2["records"]
    ), f"{sect}: smaller record count"
    assert int(wal_reset["bytes"]) < int(
        wal_restart2["bytes"]
    ...
    assert (
        wal_reset_restart["reset"] < wal_restart_immediate["reset"]
