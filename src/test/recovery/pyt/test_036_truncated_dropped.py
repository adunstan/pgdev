...
""")
    ...
    assert node.safe_sql("select count(*), sum(i) FROM truncme") == "10|55", \
    ...
    assert node.safe_sql(
        "select count(*), sum(i), min(i), max(i) FROM truncme"
    ) == "510|136304|2|1010", "table contents as expected after recovery"
    node.safe_sql("DROP TABLE truncme")
