...
def test_027_stream_regress(create_pg, tmp_path):
    ...
    node_primary.start()
    res = node_primary.sql("SELECT pg_create_physical_replication_slot('standby_1')")
    ...
    assert node_primary.postmaster_alive(), "primary alive after regression test run"
    assert node_standby_1.postmaster_alive(), "standby alive after regression test run"
    ...
    node_standby_1.command_ok(
        [
            ...
            "test",
            "--port",
            str(node_standby_1.port),
        ],
        "dump standby server",
    )
    _compare_files(primary_dump, standby_dump, "compare primary and standby dumps")
    ...
    _compare_files(
        primary_catalogs, standby_catalogs, "compare primary and standby catalog dumps"
    )
