...
def check_pg_recvlogical_stderr(handle, check_stderr):
    ...
    assert re.search(check_stderr, handle.stderr), \
...
def check_slots_dropped(standby, slot_prefix, handle):
    ...
    assert standby.slot(slot_prefix + "inactiveslot")["slot_type"] == "", \
        "inactiveslot on standby dropped"
    assert standby.slot(slot_prefix + "activeslot")["slot_type"] == "", \
...
def change_hot_standby_feedback_and_wait_for_xmins(standby, primary, hsf, invalidated):
    ...
    if hsf and invalidated:
        ...
        wait_for_xmins(primary, PRIMARY_SLOTNAME,
...
def test_035_standby_logical_decoding(create_pg):
    ...
    wait_until_vacuum_can_remove(
        node_primary, node_standby, "full",
        "CREATE TABLE conflict_test(x integer, y text); DROP TABLE conflict_test;",
        "pg_class",
    )

    # Check invalidation in the logfile and in pg_stat_database_conflicts
    check_for_invalidation(node_standby, "vacuum_full_", 1,
                           "with vacuum FULL on pg_class")
    ...
    try:
        ...
        assert alter_res.error_message is not None and re.search(
        ...
        assert node_standby.safe_sql(
            ...
            "'vacuum_full_activeslot'",
            dbname="testdb",
        ) == "t", "replication slot stats not removed after invalidation"
    ...
    walfile_name = node_primary.safe_sql(
        f"SELECT pg_walfile_name('{restart_lsn}')"
    )
    ...
    assert not os.path.isfile(standby_walfile), \
    ...
    # This should trigger the conflict
    wait_until_vacuum_can_remove(
        node_primary, node_standby, "",
        "CREATE TABLE conflict_test(x integer, y text); DROP TABLE conflict_test;",
        "pg_class",
    )

    check_for_invalidation(node_standby, "row_removal_", logstart,
    ...
    check_for_invalidation(node_standby, "shared_row_removal_", logstart,
    ...
    ) == "f", "Logical slots are reported as non conflicting"
    ...
    check_for_invalidation(node_standby, "pruning_", logstart,
                           "with on-access pruning")
    ...
    ) == "f", "database dropped on standby"

    check_slots_dropped(node_standby, "drop_db_", handle)

    assert node_standby.slot("otherslot")["slot_type"] == "logical", \
    ...
    try:
        ...
        assert handle.pump_until(r"^.*COMMIT.*COMMIT$"), \
            "got 2 COMMIT from pg_recvlogical output"
        assert handle.stdout.rstrip("\n") == expected, \
            "got same expected output from pg_recvlogical decoding session"
        ...
        # standby
        assert cascading_handle.pump_until(r"^.*COMMIT.*COMMIT$"), \
            "got 2 COMMIT from pg_recvlogical output"
