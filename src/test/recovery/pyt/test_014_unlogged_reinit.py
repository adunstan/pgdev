...
def test_014_unlogged_reinit(create_pg, tmp_path):
    ...
    seq_unlogged_path = node.safe_sql("select pg_relation_filepath('seq_unlogged')")

    # Test that main and init forks exist.
    assert os.path.isfile(
        f"{pgdata}/{base_unlogged_path}_init"
    ), "table init fork exists"
    assert os.path.isfile(f"{pgdata}/{base_unlogged_path}"), "table main fork exists"
    assert os.path.isfile(
        f"{pgdata}/{seq_unlogged_path}_init"
    ), "sequence init fork exists"
    assert os.path.isfile(f"{pgdata}/{seq_unlogged_path}"), "sequence main fork exists"

    # Test the sequence
    assert node.safe_sql("SELECT nextval('seq_unlogged')") == "1", "sequence nextval"
    assert (
        node.safe_sql("SELECT nextval('seq_unlogged')") == "2"
    ...
    node.safe_sql("CREATE UNLOGGED TABLE ts1_unlogged (id int) TABLESPACE ts1")

    ts1_unlogged_path = node.safe_sql("select pg_relation_filepath('ts1_unlogged')")

    # Test that main and init forks exist.
    assert os.path.isfile(
        f"{pgdata}/{ts1_unlogged_path}_init"
    ), "init fork in tablespace exists"
    assert os.path.isfile(
        f"{pgdata}/{ts1_unlogged_path}"
    ...
    assert os.path.isfile(
        f"{pgdata}/{base_unlogged_path}"
    ), "table main fork in base recreated at startup"
    assert not os.path.isfile(
        f"{pgdata}/{base_unlogged_path}_vm"
    ), "vm fork in base removed at startup"
    assert not os.path.isfile(
        f"{pgdata}/{base_unlogged_path}_fsm"
    ...
    assert not os.path.isfile(
        f"{pgdata}/{ts1_unlogged_path}_vm"
    ), "vm fork in tablespace removed at startup"
    assert not os.path.isfile(
        f"{pgdata}/{ts1_unlogged_path}_fsm"
    ), "fsm fork in tablespace removed at startup"

    # Test other sequences
    assert (
        node.safe_sql("SELECT nextval('seq_unlogged2')") == "1"
    ), "altered sequence nextval after restart"
    assert (
        node.safe_sql("SELECT nextval('seq_unlogged2')") == "3"
    ), "altered sequence nextval after restart again"

    node.safe_sql("INSERT INTO tab_seq_unlogged3 VALUES (DEFAULT), (DEFAULT)")
    assert (
        node.safe_sql("SELECT * FROM tab_seq_unlogged3") == "1\n2"
