...
""",
    ...
    assert node1.safe_sql(
        "SELECT icu_unicode_version() IS NOT NULL", dbname="dbicu"
    ) == "t", "ICU unicode version defined"

    assert node1.safe_sql(
        "SELECT def FROM icu ORDER BY def", dbname="dbicu"
    ) == "A\na\nB\nb", "sort by database default locale"

    assert node1.safe_sql(
        'SELECT def FROM icu ORDER BY def COLLATE "en-x-icu"', dbname="dbicu"
    ) == "a\nA\nb\nB", "sort by explicit collation standard"

    assert node1.safe_sql(
        "SELECT def FROM icu ORDER BY en COLLATE upperfirst", dbname="dbicu"
    ) == "A\na\nB\nb", "sort by explicit collation upper first"
