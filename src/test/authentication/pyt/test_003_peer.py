...
def reset_pg_ident(node, map_name, system_user, pg_user):
    ...
    node.append_conf(
        f"{map_name} {system_user} {pg_user}", filename="pg_ident.conf"
    )
    node.reload()
...
def test_003_peer(create_pg):
    ...
    system_user = node.safe_sql(
        "select (string_to_array(SYSTEM_USER, ':'))[2]"
    )
    ...
    _test_role(
        node, "testmapuser", "peer", 0, "with user name map",
    ...
    reset_pg_ident(
        node, "mypeermap", rf"/^.*{regex_test_string}$", "testmapuser"
    )
    _test_role(
        node, "testmapuser", "peer", 0,
    ...
    reset_pg_ident(
        node, "mypeermap", rf"/^.*{regex_test_string}$", r"/^testm.*$"
    )
    _test_role(
        node, "testmapuser", "peer", 0,
    ...
    reset_pg_ident(
        node, "mypeermap", rf"/^.*{bad_regex_test_string}$", "testmapuser"
    )
    _test_role(
        node, "testmapuser", "peer", 2,
        "with regexp of system user in user name map",
    ...
    _test_role(
        node, "testmapuser", "peer", 0, "plain user with group",
        log_like=[rf'connection authenticated: identity="{su_re}" method=peer'],
    )
    _test_role(
        node, "testmapgroup", "peer", 2,
    ...
    reset_pg_ident(
        node, "mypeermap", rf"/^.*{regex_test_string}$", "+testmapgroup"
    )
    _test_role(
        node, "testmapuser", "peer", 0,
    ...
    reset_pg_ident(
        node, "mypeermap", rf"/^.*{regex_test_string}(.*)$",
    ...
    _test_role(
        node, r"testmapgroupliteral\\1", "peer", 0,
