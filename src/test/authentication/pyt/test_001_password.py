...
def _test_conn(node, connstr, method, expected_res, **params):
    ...
    testname = (
        f"authentication {status_string} for method {method}, connstr {connstr}"
    )
...
def _run_body(node):
    ...
    _test_conn(
        node, "user=scram_role", "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="scram_role" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=scram_role_iter", "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="scram_role_iter" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=md5_role", "scram-sha-256", 2,
        log_unlike=[r"connection authenticated:"],
    )
    ...
    _test_conn(
        node, "user=scram_role", "scram-sha-256", 2,
        log_unlike=[r"connection authenticated:"],
    )
    os.environ["PGPASSWORD"] = "pass"
    ...
    _test_conn(
        node, "user=scram_role", "password, matching regexp for username", 0,
        log_like=[r'connection authenticated: identity="scram_role" method=password'],
    )
    ...
    _test_conn(
        node, "user=scram_role", "password, non matching regexp for username", 2,
        log_unlike=[r"connection authenticated:"],
    )
    ...
    _test_conn(
        node, "user=scram,role", "password, matching regexp for username", 0,
    ...
    _test_conn(
        node, "user=scram_role dbname=regex_testdb",
        "password, matching regexp for dbname", 0,
        log_like=[r'connection authenticated: identity="scram_role" method=password'],
    )
    ...
    _test_conn(
        node, "user=scram_role dbname=regex_testdb",
        "password, non matching regexp for dbname", 2,
        log_unlike=[r"connection authenticated:"],
    )
    ...
    _test_conn(
        node, "user=regress_regression_group", "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="regress_regression_group" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=regress_member", "scram-sha-256", 2,
        log_unlike=[r'connection authenticated: identity="regress_member" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=regress_not_member", "scram-sha-256", 2,
        log_unlike=[r'connection authenticated: identity="regress_not_member" method=scram-sha-256'],
    )
    ...
    _test_conn(
        node, "user=regress_member", "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="regress_member" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=regress_not_member", "scram-sha-256", 2,
        log_unlike=[r'connection authenticated: identity="regress_not_member" method=scram-sha-256'],
    ...
    _test_conn(
        node, "user=regress_regression_group dbname=regress_regression_group",
        "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="regress_regression_group" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=regress_member dbname=regress_regression_group",
        "scram-sha-256", 0,
        log_like=[r'connection authenticated: identity="regress_member" method=scram-sha-256'],
    )
    _test_conn(
        node, "user=regress_not_member dbname=regress_regression_group",
        "scram-sha-256", 2,
        log_unlike=[r'connection authenticated: identity="regress_not_member" method=scram-sha-256'],
    )
