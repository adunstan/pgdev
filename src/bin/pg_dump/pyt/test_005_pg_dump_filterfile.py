...
def _seed(node):
    ...
    node.safe_sql(
        "CREATE FUNCTION foo1(a int) RETURNS int AS $$ select $1 $$ "
        "LANGUAGE sql",
        "sourcedb",
    )
    node.safe_sql(
        "CREATE FUNCTION foo2(a int) RETURNS int AS $$ select $1 $$ "
        "LANGUAGE sql",
        "sourcedb",
    )
    node.safe_sql(
...
def test_pg_dump_filterfile(pg, tmp_path):
    ...
    node.command_ok(
        ["pg_dump", "--port", str(port), "--file", plainfile,
         "--filter", inputfile, "postgres"],
        "filter file without patterns",
    )

    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE TABLE public\.table_one", dump, re.M), \
        "table one dumped"
    assert re.search(r"^CREATE TABLE public\.table_two", dump, re.M), \
        "table two dumped"
    assert re.search(r"^CREATE TABLE public\.table_three", dump, re.M), \
        "table three dumped"
    assert re.search(r"^CREATE TABLE public\.table_three_one", dump, re.M), \
    ...
    assert re.search(r"^CREATE TABLE public\.table_two", dump, re.M), \
        "dumped table two"
    assert not re.search(r"^CREATE TABLE public\.table_three", dump, re.M), \
        "table three not dumped"
    assert not re.search(
        r"^CREATE TABLE public\.table_three_one", dump, re.M), \
        "table three_one not dumped"
    assert not re.search(r"^COPY public\.table_one", dump, re.M), \
        "content of table one is not included"
    assert re.search(r"^COPY public\.table_two", dump, re.M), \
    ...
    dump = slurp_file(plainfile)
    assert not re.search(r"^CREATE TABLE public\.table_one", dump, re.M), \
    ...
    assert not re.search(r"^CREATE TABLE public\.table_one", dump, re.M), \
        "table one not dumped"
    assert not re.search(r"^CREATE TABLE public\.table_two", dump, re.M), \
    ...
    dump = slurp_file(plainfile)
    assert re.search(r'^CREATE TABLE public.\"strange aaa', dump, re.M), \
    ...
    dump = slurp_file(plainfile)
    assert not re.search(r'^CREATE TABLE public.\"strange aaa', dump, re.M), \
        "dump table with new line in name"
    ...
    node.command_ok(
        ["pg_dump", "--port", str(port), "--file", plainfile,
         "--filter", inputfile, "--filter", inputfile2, "postgres"],
    ...
    dump = slurp_file(plainfile)
    assert re.search(r'^CREATE TABLE public.\"\nt\nt\n\" \($', dump,
                     re.M | re.S), \
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE SERVER dummyserver", dump, re.M), \
        "dump foreign server"
    ...
    node.command_ok(
        ["pg_dump", "--port", str(port), "--file", plainfile,
         "--filter", inputfile, "--strict-names", "postgres"],
    ...
    node.command_ok(
        ["pg_dumpall", "--port", str(port), "--file", plainfile,
         "--filter", inputfile],
        "dump tables with exclusion of a database",
    )

    dump = slurp_file(plainfile)
    assert not re.search(r"^\\connect postgres", dump, re.M), \
        "database postgres is not dumped"
    assert re.search(r"^\\connect template1", dump, re.M), \
    ...
    node.command_fails_like(
        ["pg_dumpall", "--port", str(port), "--file", plainfile,
         "--filter", inputfile, "--globals-only"],
        re.escape("pg_dumpall: error: options --exclude-database and "
    ...
    node.command_ok(
        ["pg_dump", "--port", str(port), "--file", dumpfile,
    ...
    node.command_ok(
        ["pg_restore", "--port", str(port), "--file", plainfile,
         "--filter", inputfile, "--format", "custom", dumpfile],
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE FUNCTION public\.foo1", dump, re.M), \
        "wanted function restored"
    assert not re.search(r"^CREATE TABLE public\.foo2", dump, re.M), \
        "unwanted function is not restored"

    # this should be white space tolerant (against the -P argument)
    _write_filter(
        inputfile,
        "include function  foo3 ( double  precision ,   integer)  ")
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE FUNCTION public\.foo3", dump, re.M), \
        "wanted function restored"
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE INDEX t1_idx1", dump, re.M), \
        "wanted index restored"
    assert not re.search(r"^CREATE INDEX t2_idx2", dump, re.M), \
        "unwanted index are not restored"
    assert re.search(r"^CREATE TRIGGER trg1", dump, re.M), \
        "wanted trigger restored"
    assert not re.search(r"^CREATE TRIGGER trg2", dump, re.M), \
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE TABLE s1\.t1", dump, re.M), \
        "wanted table from schema restored"
    assert re.search(r"^CREATE SEQUENCE s1\.s1", dump, re.M), \
        "wanted sequence from schema restored"
    assert not re.search(r"^CREATE TABLE s2\t2", dump, re.M), \
        "unwanted table is not restored"
    ...
    dump = slurp_file(plainfile)
    assert not re.search(r"^CREATE TABLE s1\.t1", dump, re.M), \
        "unwanted table from schema is not restored"
    assert not re.search(r"^CREATE SEQUENCE s1\.s1", dump, re.M), \
        "unwanted sequence from schema is not restored"
    assert re.search(r"^CREATE TABLE s2\.t2", dump, re.M), \
        "wanted table restored"
    assert re.search(r"^CREATE TABLE public\.t1", dump, re.M), \
        "wanted table restored"
    ...
    dump = slurp_file(plainfile)
    assert re.search(r"^CREATE TABLE public\.bootab", dump, re.M), \
    ...
    dump = slurp_file(plainfile)
    assert not re.search(r"^CREATE TABLE public\.bootab", dump, re.M), \
    ...
    assert re.search(r"^CREATE TABLE public\.bootab", dump, re.M), \
        "dumped children table"
    assert not re.search(r"^COPY public\.bootab", dump, re.M), \
        "exclude dumped children table"
