...
def _run_bytes(node, argv, msg, extra_env=None, check=True):
    ...
    if node.libdir:
        env["LD_LIBRARY_PATH"] = (
            node.libdir + os.pathsep + env.get("LD_LIBRARY_PATH", "")
        )
    if extra_env:
    ...
    proc = subprocess.run(
        bargv, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False
    )
...
def test_dump_connstr(create_pg):
    ...
    _run_bytes(
        ...
        [
            ...
            discard,
            "--dbname",
            _connstr_dbname(node.host, node.port, dbname2),
            "--username",
            username3,
        ],
    ...
    _run_bytes(
        ...
        [
            ...
            discard,
            "--dbname",
            _connstr_dbname(node.host, node.port, dbname3),
            "--username",
            username2,
        ],
    ...
    _run_bytes(
        ...
        [
            ...
            discard,
            "--dbname",
            _connstr_dbname(node.host, node.port, dbname4),
            "--username",
            username1,
        ],
    ...
    assert proc.stderr == b"", "no dump errors\nstderr:\n" + proc.stderr.decode(
        "latin-1", "replace"
    )
