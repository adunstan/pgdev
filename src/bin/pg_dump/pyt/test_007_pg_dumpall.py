...
def _pgdumpall_runs(tempdir, tablespace1, tablespace2, tablespace2_orig):
    ...
    return {
        "restore_roles": {
            "setup_sql": [
                ("CREATE ROLE dumpall WITH ENCRYPTED PASSWORD 'admin' "
                 "SUPERUSER;", "postgres"),
            ...
            "dump_cmd": [
                "pg_dumpall",
                "--format", "directory",
                "--file", f"{tempdir}/restore_roles",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "directory",
                "--file", f"{tempdir}/restore_roles.sql",
        ...
        "restore_tablespace": {
            "setup_sql": [
                ...
                (f"CREATE TABLESPACE tbl2 OWNER tap LOCATION '{tablespace2}' "
                 "WITH (seq_page_cost=1.0);", "postgres"),
            ],
            "dump_cmd": [
                "pg_dumpall",
                "--format", "directory",
                "--file", f"{tempdir}/restore_tablespace",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "directory",
                "--file", f"{tempdir}/restore_tablespace.sql",
        ...
        "restore_grants": {
            ...
            "dump_cmd": [
                "pg_dumpall",
                "--format", "directory",
                "--file", f"{tempdir}/restore_grants",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "directory",
                "--file", f"{tempdir}/restore_grants.sql",
        ...
        "format_directory": {
            "setup_sql": [
                ("CREATE TABLE format_directory(a int, b boolean, c text);\n"
                 ...
                 "(1, true, 'name1'), (2, false, 'name2');", "postgres"),
            ],
            "dump_cmd": [
                "pg_dumpall",
                "--format", "directory",
                "--file", f"{tempdir}/format_directory",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "directory",
                "--file", f"{tempdir}/format_directory.sql",
        ...
        "format_tar": {
            "setup_sql": [
                ("CREATE TABLE format_tar(a int, b boolean, c text);\n"
                 ...
                 "(1, false, 'name3'), (2, true, 'name4');", "postgres"),
            ],
            "dump_cmd": [
                "pg_dumpall",
                "--format", "tar",
                "--file", f"{tempdir}/format_tar",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "tar",
                "--file", f"{tempdir}/format_tar.sql",
        ...
        "format_custom": {
            "setup_sql": [
                ("CREATE TABLE format_custom(a int, b boolean, c text);\n"
                 ...
                 "(1, false, 'name5'), (2, true, 'name6');", "postgres"),
            ],
            "dump_cmd": [
                "pg_dumpall",
                "--format", "custom",
                "--file", f"{tempdir}/format_custom",
            ],
            "restore_cmd": [
                "pg_restore", "-C",
                "--format", "custom",
                "--file", f"{tempdir}/format_custom.sql",
        ...
        "dump_globals_only": {
            ...
            "dump_cmd": [
                ...
                "--format", "directory",
                "--globals-only",
                "--file", f"{tempdir}/dump_globals_only",
            ],
            "restore_cmd": [
                "pg_restore", "-C", "--globals-only",
                "--format", "directory",
                "--file", f"{tempdir}/dump_globals_only.sql",
        ...
        "restore_no_globals": {
            "setup_sql": [
                ("CREATE TABLE no_globals_test(a int, b text);\n"
                 ...
                 "(1, 'hello'), (2, 'world');", "postgres"),
            ],
            "dump_cmd": [
                "pg_dumpall",
                "--format", "directory",
                "--file", f"{tempdir}/restore_no_globals",
            ],
            "restore_cmd": [
                "pg_restore", "-C", "--no-globals",
                "--format", "directory",
                "--file", f"{tempdir}/restore_no_globals.sql",
...
def test_pg_dumpall(pg, create_pg, tmp_path):
    ...
    for run in sorted(pgdumpall_runs):
        ...
        restore_cmd = list(spec["restore_cmd"]) + [
            "--host", target_node.host, "--port", str(target_node.port),
        ]
        node.pg_bin.result(restore_cmd)

        output_file = slurp_file(os.path.join(tempdir, f"{run}.sql"))

        assert spec.get("like") or spec.get("unlike"), (
            f'missing "like" or "unlike" in test "{run}"'
        )

        if spec.get("like"):
            assert spec["like"].search(output_file), (
                f"should dump {run}\nReview results in {tempdir}"
            )
        if spec.get("unlike"):
            assert not spec["unlike"].search(output_file), (
    ...
    node.command_fails_like(
        [
            "pg_restore", custom,
            "--format", "custom",
            "--file", f"{tempdir}/error_test.sql",
        ],
        re.escape("pg_restore: error: option -C/--create must be specified "
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option -l/--list cannot be used when "
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option -L/--use-list cannot be used "
                  "when restoring an archive created by pg_dumpall"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option --strict-names cannot be used "
                  "when restoring an archive created by pg_dumpall"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: options --clean and -g/--globals-only "
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option --no-schema cannot be used when "
                  "restoring an archive created by pg_dumpall"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option -a/--data-only cannot be used "
                  "when restoring an archive created by pg_dumpall"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option --statistics-only cannot be used "
                  "when restoring an archive created by pg_dumpall"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: option --section cannot exclude "
                  "--pre-data when restoring a pg_dumpall archive"),
        "When --section=post-data is used in pg_restore with dump of "
        "pg_dumpall",
    )
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: options -a/--data-only and "
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: options -g/--globals-only and "
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: options --statistics and "
                  "-g/--globals-only cannot be used together"),
    ...
    node.command_fails_like(
        ...
        re.escape("pg_restore: error: options --exit-on-error and "
                  "-g/--globals-only cannot be used together"),
    ...
    map_dat_content = slurp_file(
        os.path.join(tempdir, "format_directory", "map.dat")
    )
    assert re.search(
    ...
    node.safe_sql(
        "CREATE TABLE comment_test_table (id int);", dbname="comment_test_db"
    )
    ...
    node.command_ok(
        [
            "pg_restore", "-C",
            "--format", "directory",
            "--file", f"{tempdir}/comment_test_restore.sql",
            "--host", target_comment.host,
            "--port", str(target_comment.port),
    ...
    assert not re.search(r"CREATE DATABASE comment_test_db", restore_output), (
    ...
    assert re.search(r"DROP ROLE IF EXISTS", clean_output), (
