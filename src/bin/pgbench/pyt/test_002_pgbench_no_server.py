...
OPTIONS = [
    ["bad option", "-h home -p 5432 -U calvin ---debug --bad-option",
     [r"--help.*more information"]],
    ["no file", "-f no-such-file", [r'could not open file "no-such-file":']],
    ["no builtin", "-b no-such-builtin",
     [r'no builtin script .* "no-such-builtin"']],
    ["invalid weight", "--builtin=select-only@one",
     [r"invalid weight specification: \@one"]],
    ["invalid weight", "-b select-only@-1",
     [r"weight spec.* out of range .*: -1"]],
    ["too many scripts", "-S " * 129, [r"at most 128 SQL scripts"]],
    ["bad #clients", "-c three",
     [r'invalid value "three" for option -c/--clients']],
    ["bad #threads", "-j eleven",
     [r'invalid value "eleven" for option -j/--jobs']],
    ["bad scale", "-i -s two",
     [r'invalid value "two" for option -s/--scale']],
    ["invalid #transactions", "-t zil",
     [r'invalid value "zil" for option -t/--transactions']],
    ["invalid duration", "-T ten",
     [r'invalid value "ten" for option -T/--time']],
    ["-t XOR -T",
    ...
    ["-I without init option", "-I dtg",
     [r"cannot be used in benchmarking mode"]],
    ...
    ["bad partition method", "-i --partition-method=BAD",
     [r'"range"', r'"hash"', r'"BAD"']],
    ["bad partition number", "-i --partitions -1",
     [r"--partitions must be in range"]],
    ["partition method without partitioning", "-i --partition-method=hash",
    ...
    ["an infinite number of tries", "--max-tries 0",
    ...
    ["aggregate => log", "--aggregate-interval=3",
     [r"aggregation .* only when"]],
    ["log-prefix => log", "--log-prefix=x", [r"prefix .* only when"]],
    ["duration & aggregation", "-l -T 1 --aggregate-interval=3",
     [r"aggr.* not be higher"]],
    ["duration % aggregation", "-l -T 5 --aggregate-interval=3",
     [r"multiple"]],
]


@pytest.mark.parametrize(
    "name,opts,err_checks", OPTIONS,
    ids=[o[0] for o in OPTIONS],
)
def test_option_errors(pg_bin, name, opts, err_checks):
    _pgbench(pg_bin, opts, 1, EMPTY, err_checks,
...
def test_builtin_list(pg_bin):
    # list of builtins
    _pgbench(pg_bin, "-b list", 0, EMPTY,
             [r"Available builtin scripts:", r"tpcb-like",
              r"simple-update", r"select-only"],
             "pgbench builtin list")


def test_builtin_listing(pg_bin):
    # builtin listing
    _pgbench(pg_bin, "--show-script se", 0, EMPTY,
             [r"select-only: ",
...
SCRIPT_TESTS = [
    ["missing endif", [r"\\if without matching \\endif"],
     {"if-noendif.sql": "\\if 1"}],
    ["missing if on elif", [r"\\elif without matching \\if"],
     {"elif-noif.sql": "\\elif 1"}],
    ["missing if on else", [r"\\else without matching \\if"],
     {"else-noif.sql": "\\else"}],
    ["missing if on endif", [r"\\endif without matching \\if"],
     {"endif-noif.sql": "\\endif"}],
    ["elif after else", [r"\\elif after \\else"],
     {"else-elif.sql": "\\if 1\n\\else\n\\elif 0\n\\endif"}],
    ["else after else", [r"\\else after \\else"],
     {"else-else.sql": "\\if 1\n\\else\n\\else\n\\endif"}],
    ["if syntax error", [r'syntax error in command "if"'],
     {"if-bad.sql": "\\if\n\\endif\n"}],
    ["elif syntax error", [r'syntax error in command "elif"'],
     {"elif-bad.sql": "\\if 0\n\\elif +\n\\endif\n"}],
    ["else syntax error", [r'unexpected argument in command "else"'],
     {"else-bad.sql": "\\if 0\n\\else BAD\n\\endif\n"}],
    ["endif syntax error", [r'unexpected argument in command "endif"'],
     {"endif-bad.sql": "\\if 0\n\\endif BAD\n"}],
    ...
    # overflow
    ["bigint overflow 1", [r"bigint constant overflow"],
     {"overflow-1.sql": "\\set i 100000000000000000000\n"}],
    ["double overflow 2", [r"double constant overflow"],
     {"overflow-2.sql": "\\set d 1.0E309\n"}],
    ["double overflow 3", [r"double constant overflow"],
     {"overflow-3.sql": "\\set d .1E310\n"}],
    ["set i", [r"set i 1 ", r"\^ error found here"],
     {"set_i_op": "\\set i 1 +\n"}],
