...
def pgbench(pg, opts, stat, out, err, name, files=None, args=None, extra_env=None):
...
def check_data_state(pg, kind):
    ...
    assert (
        ...
        == "0"
    ), f"{kind}: filler column of pgbench_accounts has no NULL data"
    assert (
        pg.safe_sql(
            "SELECT count(*) AS null_count FROM pgbench_branches WHERE filler IS NULL;"
        )
        == "1"
    ), f"{kind}: filler column of pgbench_branches has only NULL data"
    assert (
        pg.safe_sql(
            "SELECT count(*) AS null_count FROM pgbench_tellers WHERE filler IS NULL;"
        )
        == "10"
    ), f"{kind}: filler column of pgbench_tellers has only NULL data"
    assert (
        pg.safe_sql("SELECT count(*) AS data_count FROM pgbench_history;") == "0"
    ), f"{kind}: pgbench_history has no data"
...
def test_concurrent_oid_generation(pg):
    ...
    pgbench(
        ...
        {
            "001_pgbench_concurrent_insert": f"CREATE TYPE pg_temp.e AS ENUM ({labels}); DROP TYPE pg_temp.e;"
        },
    )
...
def test_random_seeded_determinism(pg):
    ...
    assert re.search(
        rf"\b{seed}\|uniform\|1\d\d\d\|2", out
    ), "psql seeded_random count uniform"
    assert re.search(
        rf"\b{seed}\|exponential\|2\d\d\d\|2", out
    ), "psql seeded_random count exponential"
    assert re.search(
        rf"\b{seed}\|gaussian\|3\d\d\d\|2", out
    ), "psql seeded_random count gaussian"
    assert re.search(
        rf"\b{seed}\|zipfian\|4\d\d\d\|2", out
...
# Each row is a heterogeneous [str, int, List[str], str] record.
_ERRORS: List[List[Any]] = [
    # SQL
    [
        "sql syntax error",
        2,
    ...
    ["set not a bool", 2, [r"cannot coerce double to boolean"], "\\set b NOT 0.0"],
    ["set not an int", 2, [r"cannot coerce boolean to int"], "\\set i TRUE + 2"],
    ["set not a double", 2, [r"cannot coerce boolean to double"], "\\set d ln(TRUE)"],
    [
        "set case error",
        1,
    ...
    ["set bigint add overflow", 2, [r"int add out"], "\\set i (1<<62) + (1<<62)"],
    [
    ...
    ["bad boolean", 2, [r"malformed variable.*trueXXX"], "\\set b :badtrue or true"],
    [
    ...
    # GSET
    ["gset no row", 2, [r"expected one row, got 0\b"], "SELECT WHERE FALSE \\gset"],
    ["gset alone", 1, [r"gset must follow an SQL command"], "\\gset"],
    ["gset no SQL", 1, [r"gset must follow an SQL command"], "\\set i +1\n\\gset"],
    ["gset too many arguments", 1, [r"too many arguments"], "SELECT 1 \\gset a b"],
    [
        "gset after gset",
        1,
...
def _list_files(directory, regex):
    ...
    return [os.path.join(directory, f) for f in os.listdir(directory) if pat.search(f)]
...
def check_pgbench_logs(directory, prefix, nb, min_lines, max_lines, line_re):
    ...
    assert len([log for log in logs if name_re.search(log)]) == nb, "file name format"
    ...
    for log in sorted(logs):
        ...
        while contents and contents[-1] == "":
            contents.pop()
        clen = len(contents)
        assert clen >= min_lines, f"transaction count for {log} ({clen}) is above min"
        assert clen <= max_lines, f"transaction count for {log} ({clen}) is below max"
...
def test_logs_sampling(pg):
    ...
    check_pgbench_logs(
        bdir, "001_pgbench_log_2", 1, 8, 92, r"^[01] \d{1,2} \d+ \d \d+ \d+$"
    )
...
def test_logs_contents(pg):
    ...
    check_pgbench_logs(
        bdir, "001_pgbench_log_3", 1, 10, 10, r"^0 \d{1,2} \d+ \d \d+ \d+$"
    )
...
def test_serialization_retry(pg, retry_tables):
    ...
    pgbench(
        ...
        extra_env={"PGOPTIONS": "-c default_transaction_isolation=repeatable\\ read"},
    )
    # Clean up
...
def test_deadlock_retry(pg, retry_tables):
    ...
    pgbench(
        ...
        extra_env={"PGOPTIONS": "-c default_transaction_isolation=read\\ committed"},
    )
