...
def check_log_pattern(fmt, logfiles, pattern, node):
    ...
    assert re.search(pattern, logcontents), \
        f"found expected log file content for {fmt}"

    # While we're at it, test pg_current_logfile() function
    assert node.safe_sql(f"SELECT pg_current_logfile('{fmt}')") == lfname, \
...
""")
    ...
    assert re.search(
        ...
        current_logfiles), "current_logfiles is sane"
    ...
    assert re.search(
        ...
        r"csvlog log/postgresql-.*csv\n"
        r"jsonlog log/postgresql-.*json$",
        new_current_logfiles), "new current_logfiles is sane"
