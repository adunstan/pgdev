...
def test_010_tab_completion(pg, interactive_psql, tmp_path):
    ...
    try:
        ...
        psql = interactive_psql(
            node, dbname="postgres", history_file=historyfile
        )
...
def _run_cases(psql):
    ...
    for case, in_, out in (
        ...
        check_completion(
            ...
            re.compile(out),
            f"offer keyword {out} for input {in_}<TAB>, "
            f"COMP_KEYWORD_CASE = {case}",
        )
        clear_query(psql)
    ...
    check_completion(
        psql, "O\t", re.compile(r"OF "), "complete PORTION O<tab> to OF"
    )
