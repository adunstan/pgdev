...
def test_timing_query_error(node):
    ret, stdout, _stderr = _run_psql(node, "\\timing on\nSELECT error")
...
def test_errverbose(node, sql, pattern, name):
    _ret, _stdout, stderr = _run_psql(node, sql, on_error_stop=False)
...
def test_single_transaction_multiple_switches(node, tmp_path):
    ...
    node.command_fails(
        [
            ...
            "--command",
            f"\\copy tab_psql_single FROM '{tempdir}/nonexistent'",
        ],
...
WATCH_ERROR_CASES = [
    (
        "SELECT 1 \\watch -10",
        r'incorrect interval value "-10"',
        "\\watch, negative interval",
    ),
    (
        "SELECT 1 \\watch 10ab",
        r'incorrect interval value "10ab"',
        "\\watch, incorrect interval",
    ),
    (
        "SELECT 1 \\watch 10e400",
        r'incorrect interval value "10e400"',
        "\\watch, out-of-range interval",
    ),
    (
        "SELECT 1 \\watch 1 1",
        r"interval value is specified more than once",
        "\\watch, interval value is specified more than once",
    ),
    (
        "SELECT 1 \\watch c=1 c=1",
        r"iteration count is specified more than once",
...
def test_g_pipe(node, tmp_path):
    ...
    _psql_like(
        node, f"SELECT 'one' \\g | {pipe_cmd}", re.compile(r""), "one command \\g"
    )
    ...
    _psql_like(
        node,
        f"SELECT 'two' \\; SELECT 'three' \\g | {pipe_cmd}",
        re.compile(r""),
        "two commands \\g",
    )
...
def test_meta_command_in_restrict_mode(node):
    _psql_fails_like(
        node,
        "\\restrict test\n\\! should_fail",
        re.compile(r"backslash commands are restricted; only \\unrestrict is allowed"),
