...
def header(blkno=None, offnum=None, attnum=None):
    ...
    if offnum is not None:
        return r'heap table "postgres\.public\.test", block %d, offset %d:\s+' % (
            blkno,
            offnum,
        )
    ...
    with open(relpath, "r+b") as file:
        for tupidx in range(ROWCOUNT):
            ...
            if offnum == 1:
                ...
                expected.append(
                    re.compile(
                        hdr + r"xmin %d precedes relation freeze threshold 0:\d+" % xmin
                    )
                )
            elif offnum == 2:
                ...
                expected.append(
                    re.compile(
                        hdr
                        + r"xmin %d precedes oldest valid transaction ID 0:\d+" % xmin
                    )
                )
            elif offnum == 3:
                ...
                expected.append(
                    re.compile(
                        hdr
                        + r"xmax %d precedes oldest valid transaction ID 0:\d+" % xmax
                    )
                )
            elif offnum == 5:
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple data should begin at byte 24, but actually "
                ...
                expected.append(
                    re.compile(
                        hdr + r"number of attributes 2047 exceeds maximum 3 "
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple data should begin at byte 280, but actually "
                ...
                expected.append(
                    re.compile(
                        hdr + r"number of attributes 67 exceeds maximum 3 "
                        r"expected for table"
                    )
                )
            elif offnum == 12:
                ...
                expected.append(
                    re.compile(
                        hdr + r"attribute with length \d+ ends at offset \d+ "
                ...
                expected.append(
                    re.compile(
                        hdr + r"multitransaction ID 4 equals or exceeds next valid "
                ...
                expected.append(
                    re.compile(
                        hdr + r"multitransaction ID 4000000000 precedes relation "
                ...
                expected.append(
                    re.compile(
                        hdr + r"xmin %d equals or exceeds next valid transaction "
                        r"ID 0:\d+" % xmin
                    )
                )
            elif offnum == 17:
                ...
                expected.append(
                    re.compile(
                        hdr + r"redirected line pointer points to a non-heap-only "
                        r"tuple at offset \d+"
                    )
                )
            elif offnum == 18:
                ...
                expected.append(
                    re.compile(
                        hdr + r"redirected line pointer points to another "
                ...
                expected.append(
                    re.compile(
                        hdr + r"redirect line pointer points to offset \d+, but "
                ...
                expected.append(
                    re.compile(
                        hdr + r"non-heap-only update produced a heap-only tuple at "
                        r"offset \d+"
                    )
                )
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple points to new version at offset \d+, but "
                        r"offset \d+ also points there"
                    )
                )
            elif offnum == 30:
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple with aborted xmin \d+ was updated to produce "
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple is root of chain but is marked as heap-only "
                        r"tuple"
                    )
                )
                expected.append(
                    re.compile(
                        hdr + r"tuple is heap only, but not the result of an update"
                    )
                )
            elif offnum == 33:
                ...
                expected.append(
                    re.compile(
                        hdr + r"heap-only update produced a non-heap only tuple at "
                        r"offset \d+"
                    )
                )
            elif offnum == 34:
                ...
                expected.append(
                    re.compile(
                        hdr + r"tuple with in-progress xmin \d+ was updated to "
