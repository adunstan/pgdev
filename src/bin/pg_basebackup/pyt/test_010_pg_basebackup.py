...
def check_mode_recursive(path, dir_mode, file_mode):
    ...
    for root, _dirs, files in os.walk(path):
        ...
        if stat.S_IMODE(st.st_mode) != dir_mode:
            print(
                f"mode of directory {root} is {oct(stat.S_IMODE(st.st_mode))}, "
        ...
        for name in files:
            ...
            if stat.S_IMODE(st.st_mode) != file_mode:
                print(
                    f"mode of file {fp} is {oct(stat.S_IMODE(st.st_mode))}, "
...
def _run_body(create_pg, tempdir):
    ...
    assert not os.path.isdir(f"{tempdir}/backup"), "backup directory was cleaned up"
    ...
    if not HAVE_LIBZ:
        ...
        for spec, errmsg, label in compression_failure_tests:
            ...
            node.command_fails_like(
                ["pg_basebackup", "--pgdata", f"{tempdir}/backup", "--compress", spec],
                cfail,
                "client " + label,
            )
            node.command_fails_like(
    ...
    base_unlogged_path = node.safe_sql("select pg_relation_filepath('base_unlogged')")

    # Make sure main and init forks exist.
    assert os.path.isfile(
        os.path.join(pgdata, base_unlogged_path + "_init")
    ), "unlogged init fork in base"
    assert os.path.isfile(
        os.path.join(pgdata, base_unlogged_path)
    ...
    for dirname in (
        ...
        assert sorted(_slurp_dir(f"{tempdir}/backup/{dirname}/")) == sorted(
            [".", ".."]
        ), f"contents of {dirname}/ not copied"
    ...
    for filename in (
        ...
        assert not os.path.isfile(
            f"{tempdir}/backup/{filename}"
        ), f"{filename} not copied"

    # We only test .DS_Store files being skipped on non-macOS systems.
    if sys.platform != "darwin":
        assert not os.path.isfile(f"{tempdir}/backup/.DS_Store"), ".DS_Store not copied"

    # Unlogged relation forks other than init should not be copied.
    assert os.path.isfile(
        f"{tempdir}/backup/{base_unlogged_path}_init"
    ), "unlogged init fork in backup"
    assert not os.path.isfile(
        f"{tempdir}/backup/{base_unlogged_path}"
    ...
    assert (
        slurp_file(f"{tempdir}/backup/backup_label") != "DONOTCOPY"
    ...
    assert not os.path.isfile(
        f"{tempdir}/backup2/backup_manifest"
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS + ["--pgdata", f"{tempdir}/tarbackup", "--format", "tar"],
        "tar format",
    )
    assert os.path.isfile(f"{tempdir}/tarbackup/base.tar"), "backup tar was created"
    ...
    with open(superlongpath, "w", encoding="utf-8"):
        pass
    node.command_fails(
        PG_BASEBACKUP_DEFS + ["--pgdata", f"{tempdir}/tarbackup_l1", "--format", "tar"],
    ...
    dir_symlink(
        os.path.join(sys_tempdir, "pg_replslot"), os.path.join(pgdata, "pg_replslot")
    )
    ...
    # Create an unlogged table to test that forks other than init are not
    # copied.
    node.safe_sql("CREATE UNLOGGED TABLE tblspc1_unlogged (id int) TABLESPACE tblspc1;")
    ...
    assert os.path.isfile(
        os.path.join(pgdata, tblspc1_unlogged_path)
    ...
    tblspc1_id = os.path.basename(os.path.dirname(os.path.dirname(test1_path)))
    ...
    node.command_fails(
        PG_BASEBACKUP_DEFS + ["--pgdata", f"{tempdir}/backup1", "--format", "plain"],
    ...
    assert os.path.isdir(f"{tempdir}/tbackup/tblspc1"), "tablespace was relocated"
    ...
    if not WINDOWS_OS:
        ...
        for entry in os.listdir(os.path.join(pgdata, "pg_tblspc")):
            ...
            if (
                os.path.islink(link)
                and os.readlink(link) == f"{tempdir}/tbackup/tblspc1"
            ):
                found = True
                break
    ...
    for filename in temp_relation_files:
        ...
        filepath = f"{real_sys_tempdir}/tblspc1/{tblspc1_id}/{postgres_oid}/{filename}"
        os.unlink(filepath)

    assert os.path.isdir(
        f"{tempdir}/backup1/pg_replslot"
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS + ["--pgdata", f"{tempdir}/tarbackup_l3", "--format", "tar"],
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backupR", "--write-recovery-conf"],
        "pg_basebackup --write-recovery-conf runs",
    )
    assert os.path.isfile(
        f"{tempdir}/backupR/postgresql.auto.conf"
    ), "postgresql.auto.conf exists"
    assert os.path.isfile(
        f"{tempdir}/backupR/standby.signal"
    ...
    assert any(
        re.match(r"^[0-9A-F]{24}$", f) for f in _slurp_dir(f"{tempdir}/backupxd/pg_wal")
    ), "WAL files copied"
    _rmtree(f"{tempdir}/backupxd")

    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backupxf", "--wal-method", "fetch"],
        "pg_basebackup --wal-method fetch runs",
    )
    assert any(
        re.match(r"^[0-9A-F]{24}$", f) for f in _slurp_dir(f"{tempdir}/backupxf/pg_wal")
    ), "WAL files copied"
    _rmtree(f"{tempdir}/backupxf")

    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backupxs", "--wal-method", "stream"],
        "pg_basebackup --wal-method stream runs",
    )
    assert any(
        re.match(r"^[0-9A-F]{24}$", f) for f in _slurp_dir(f"{tempdir}/backupxs/pg_wal")
    ), "WAL files copied"
    ...
    assert os.path.isfile(f"{tempdir}/backupxst/pg_wal.tar"), "tar file was created"
    ...
    node.command_fails_like(
        PG_BASEBACKUP_DEFS
        + ["--target", "blackhole", "--wal-method", "none", "--format", "tar"],
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--target", f"server:{tempdir}/backuponserver", "--wal-method", "none"],
        "backup target server",
    )
    assert os.path.isfile(
        f"{tempdir}/backuponserver/base.tar"
    ...
    node.command_ok(
        ...
        + [
            ...
            "backupuser",
            "--target",
            f"server:{tempdir}/backuponserver",
            "--wal-method",
            "none",
        ],
        "backup target server",
    ...
    node.command_fails_like(
        PG_BASEBACKUP_DEFS + ["--pgdata", f"{tempdir}/backupxs_slot", "--create-slot"],
    ...
    node.command_fails_like(
        PG_BASEBACKUP_DEFS
        + ["--target", "blackhole", "--pgdata", f"{tempdir}/blackhole"],
        r"cannot specify both output directory and backup target",
        "backup target and output directory",
    )
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backuptr/co", "--wal-method", "none"],
        "pg_basebackup --wal-method fetch runs",
    )

    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backupxs_slot", "--create-slot", "--slot", "slot0"],
    ...
    node.command_fails_like(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backupxs_slot1", "--create-slot", "--slot", "slot0"],
        r'replication slot "slot0" already exists',
        "pg_basebackup fails with --create-slot --slot and a previously "
        "existing slot",
    )

    node.safe_sql("SELECT * FROM pg_create_physical_replication_slot('slot1')")
    lsn = node.safe_sql(
    ...
    node.command_fails(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/fail", "--slot", "slot1", "--wal-method", "none"],
    ...
    assert re.search(
        r"dbname=db1",
        slurp_file(f"{tempdir}/backup_dbname_R/postgresql.auto.conf"),
        re.M,
    ...
    node.command_ok(
        PG_BASEBACKUP_DEFS
        + ["--pgdata", f"{tempdir}/backup_corrupt4", "--no-verify-checksums"],
    ...
    if not HAVE_LIBZ:
        print("# postgres was not built with ZLIB support; skipping compression tests")
    else:
        node.command_ok(
            PG_BASEBACKUP_DEFS
            + [
                "--pgdata",
                f"{tempdir}/backup_gzip",
                "--compress",
                "1",
                "--format",
                "t",
        ...
        node.command_ok(
            PG_BASEBACKUP_DEFS
            + ["--pgdata", f"{tempdir}/backup_gzip2", "--gzip", "--format", "t"],
    ...
    node.safe_sql("CREATE TABLE t AS SELECT a FROM generate_series(1,10000) AS a;")
    ...
    with subprocess.Popen(
        [
            os.path.join(node.bindir, "pg_basebackup"),
            "--no-sync",
            "-cfast",
            "--wal-method=stream",
            "--pgdata",
    ...
    assert re.search(
        r"background process terminated unexpectedly", stderr
    ...
    node2 = create_pg("node2", start=False, has_archiving=True, allows_streaming=True)
    ...
    node2.command_fails_like(
        ...
        + [
            "--pgdata",
            f"{tempdir}/diff_sysid",
            "--incremental",
            f"{backupdir}/backup_manifest",
        ],
