...
def _slot(node, slot_name):
    ...
    """
    fields = ["plugin", "slot_type", "datoid", "database", "active",
...
def _check_mode_recursive(directory, expected_dir_mode, expected_file_mode):
    ...
    for root, dirs, files in os.walk(directory):
        for name in [*dirs, *files]:
            ...
            if actual != expected:
                print(
                    f"mode of {path} is {actual:#o} but expected {expected:#o}"
                )
                return False
    return True
...
def test_020_pg_receivewal(pg_bin, pg_config, create_pg):
    ...
    primary = create_pg(
        "primary", start=False, allows_streaming=True,
    ...
    assert _slot(primary, slot_name)["slot_type"] == "", \
    ...
    if not _check_pg_config(pg_config, "#define HAVE_LIBZ 1"):
        ...
        if not gzip:
            ...
            gzip_is_valid = subprocess.run(
                [gzip, "--test", *zlib_wals]).returncode
            assert gzip_is_valid == 0, \
                "gzip verified the integrity of compressed WAL segments"

    # Check LZ4 compression if available
    if not _check_pg_config(pg_config, "#define USE_LZ4 1"):
        ...
        if not lz4:
            ...
            lz4_is_valid = subprocess.run([lz4, "-t", *lz4_wals]).returncode
            assert lz4_is_valid == 0, \
    ...
    primary.command_fails_like(
        ...
        r'pg_receivewal: error: replication slot "nonexistentslot" does '
        r"not exist",
    ...
    assert os.path.exists(os.path.join(slot_dir, walfile_streamed)), \
    ...
    assert os.path.exists(os.path.join(timeline_dir, walfile_before_promotion)), \
        f"WAL segment {walfile_before_promotion} archived after timeline jump"
    assert os.path.exists(os.path.join(timeline_dir, walfile_after_promotion)), \
        f"WAL segment {walfile_after_promotion} archived after timeline jump"
    assert os.path.exists(os.path.join(timeline_dir, "00000002.history")), \
