...
""")
    ...
    node_t.command_fails(
        [
            ...
            "--publisher-server", _connstr(node_p, db1),
    ...
    node_c.command_fails(
        [
            ...
            "--publisher-server", _connstr(node_s, db1),
...
""")
    ...
    assert _safe_sql(node_s, db1, "SELECT COUNT(*) FROM pg_publication") == "2", \
    ...
    assert re.search(r"target server reached the consistent state", internal_log), \
        "log shows consistent state reached"

    # Check if node S is still a standby
    node_s.start()
    assert node_s.safe_sql("SELECT pg_catalog.pg_is_in_recovery()") == "t", \
    ...
    res = node_s.pg_bin.command_ok(
        [
            ...
            "--recovery-timeout", "180",
            "--pgdata", node_s.data_dir,
            "--publisher-server", _connstr(node_p),
            "--socketdir", node_s.host,
            "--subscriber-port", node_s.port,
            "--all",
        ],
    ...
    assert len(re.findall(r"would create publication", stderr)) == 3, \
        "verify publications are created for all databases"
    assert len(re.findall(r"would create the replication slot", stderr)) == 3, \
        "verify replication slots are created for all databases"
    assert len(re.findall(r"would create subscription", stderr)) == 3, \
    ...
    assert _safe_sql(node_s, db1, "SELECT COUNT(*) FROM pg_publication") == "0", \
        "all publications were removed from db1"
    assert _safe_sql(node_s, db2, "SELECT COUNT(*) FROM pg_publication") == "0", \
    ...
    sysid_p = node_p.safe_sql(
        "SELECT system_identifier FROM pg_control_system()")
    sysid_s = node_s.safe_sql(
        "SELECT system_identifier FROM pg_control_system()")
...
""")
    ...
    node_k.command_ok(
        [
            ...
            "--log", node_k.logfile,
            "--options", f"--cluster-name={node_k_name}",
            "start",
        ],
        "node K has started",
    )
