...
def test_mode(create_pg, pg_bin, tmp_path, mode):
    ...
    old.safe_sql(
        "CREATE TABLE test2 AS SELECT generate_series(200, 300)", dbname="testdb1"
    )
    ...
    old.safe_sql(
        "CREATE TABLE test6 AS SELECT generate_series(607, 711)", dbname="testdb3"
    )
    ...
    if result:
        new.start()
        assert (
            new.safe_sql("SELECT COUNT(*) FROM test1") == "100"
        ), f"test1 data after pg_upgrade {mode}"
        assert (
            new.safe_sql("SELECT COUNT(*) FROM test2", dbname="testdb1") == "101"
        ), f"test2 data after pg_upgrade {mode}"
        assert (
            new.safe_sql("SELECT nextval('testseq')", dbname="testdb1") == "5432"
        ...
        assert (
            new.safe_sql("SELECT COUNT(*) FROM test5") == "104"
        ), f"test5 data after pg_upgrade {mode}"
        assert (
            new.safe_sql("SELECT COUNT(*) FROM test6", dbname="testdb3") == "105"
        ), f"test6 data after pg_upgrade {mode}"

        # Tests for large objects
        assert (
            new.safe_sql("SELECT lo_get(4532)") == r"\xffffff00"
        ), "LO contents after upgrade"
        assert (
            new.safe_sql("SELECT obj_description(4532, 'pg_largeobject')") == "test"
        ), "comment on LO after pg_upgrade"

        assert (
            new.safe_sql("SELECT lo_get(4533)") == r"\x0f0f0f0f"
        ), "LO contents after upgrade"
        assert (
            new.safe_sql(
        ...
        assert (
            new.safe_sql("SELECT lomacl FROM pg_largeobject_metadata WHERE oid = 4533")
            == "{regress_lo_1=rw/regress_lo_1,regress_lo_2=r/regress_lo_1}"
        ), "LO ACL after upgrade"

        if test_seclabel:
            assert (
                new.safe_sql("SELECT lo_get(4534)") == r"\x00ffffff"
            ), "LO contents after upgrade"
            assert (
                new.safe_sql(
                    """
            ...
            """
                ...
                == "classified"
            ), "seclabel on LO after pg_upgrade"
        new.stop()
