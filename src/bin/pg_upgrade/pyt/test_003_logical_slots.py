...
def test_003_logical_slots(create_pg, pg_bin):
    ...
    # 1MB.
    newpub.append_conf("bgwriter_lru_maxpages = 0\ncheckpoint_timeout = 1h\n")
    ...
    oldpub.start()
    oldpub.safe_sql("CREATE TABLE tbl AS SELECT generate_series(1, 10) AS a")
    oldpub.safe_sql(
    ...
    assert not re.search(r"test_slot3", content), "caught-up slot is not reported"
    ...
    #    tests)
    old_connstr = f"host={oldpub.host} port={oldpub.port} dbname=postgres"
    ...
    new_connstr = f"host={newpub.host} port={newpub.port} dbname=postgres"
    sub.safe_sql(
