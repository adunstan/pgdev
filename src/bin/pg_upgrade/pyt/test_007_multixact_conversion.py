...
def reset_mxid_mxoffset_pre_v19(pg_bin, node, mxid, mxoffset):
    ...
    pg_bin.command_ok(
        [
            ...
            "--multixact-offset", str(mxoffset),
            "--multixact-ids", f"{mxid},{mxid}",
        ],
...
def upgrade_and_compare(pg_bin, bindir, oldnode, newnode):
    ...
    assert old_table_contents == new_table_contents, (
        "test table contents from original and upgraded clusters match")
    assert old_multixacts == new_multixacts, (
...
def test_007_multixact_conversion(pg_bin, create_pg, bindir, tmp_path):
    ...
    assert int(finish_mxid) < int(start_mxid), (
        "multixid wrapped around in old cluster")
    assert int(finish_mxoff) < int(start_mxoff), (
