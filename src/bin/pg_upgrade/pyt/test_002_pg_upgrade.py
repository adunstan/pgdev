...
ADJUST_DUMP_PL = os.path.join(REPO, "src", "bin", "pg_upgrade", "pyt", "adjust_dump.pl")
...
def test_002_pg_upgrade(create_pg, pg_bin, tmp_path):
    ...
    if (
        ...
        dstnode = create_pg("dst_node", start=False, initdb_extra=old_initdb_params)
    ...
    if oldinstall:
        ...
        with open(dump1_file, "w", encoding="utf-8") as fh:
            fh.write(dump_data)
        output = oldnode.safe_sql("SELECT datname FROM pg_database WHERE datallowconn;")
