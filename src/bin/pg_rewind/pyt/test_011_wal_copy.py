...
def test_011_wal_copy(rewind, bindir):
    ...
    assert os.path.exists(corrupt_wal_seg_in_target_path), \
        f"segment {corrupt_wal_seg} exists in target before rewind"
    corrupt_wal_seg_size_before_rewind = \
        os.stat(corrupt_wal_seg_in_target_path).st_size
    ...
    assert not os.path.exists(new_timeline_wal_seg_path), \
    ...
    pg_bin.command_checks_all(
        ...
        [
            ...
            re.compile(
                r"pg_wal/" + re.escape(new_timeline_wal_seg) + r" \(COPY\)"),
        ],
        "run pg_rewind",
    )
    ...
    # target.
    assert os.path.exists(new_timeline_wal_seg_path), \
    ...
    assert os.path.exists(corrupt_wal_seg_in_source_path), \
        f"corrupted {corrupt_wal_seg} exists in source after rewind"
    corrupt_wal_seg_source_size = \
        os.stat(corrupt_wal_seg_in_source_path).st_size

    assert os.path.exists(corrupt_wal_seg_in_target_path), \
        f"corrupted {corrupt_wal_seg} exists in target after rewind"
    corrupt_wal_seg_size_after_rewind = \
        os.stat(corrupt_wal_seg_in_target_path).st_size
    ...
    assert corrupt_wal_seg_size_after_rewind == corrupt_wal_seg_source_size, \
        (f"same size of corrupted {corrupt_wal_seg} in source and target "
         "after rewind")
