...
def check_mode_recursive(path, dir_mode, file_mode):
    ...
    for root, dirs, files in os.walk(path):
        for name in dirs:
            ...
            if actual != dir_mode:
                print(
                    f"mode of directory {full} is {actual:#o}, "
        ...
        for name in files:
            ...
            if actual != file_mode:
                print(
                    f"mode of file {full} is {actual:#o}, " f"expected {file_mode:#o}"
                )
                ok = False
    return ok
...
def run_test(rewind, bindir, test_mode):
    ...
    rewind.primary_psql("CREATE TABLE space_tbl (d text) TABLESPACE space_test")
    rewind.primary_psql("INSERT INTO space_tbl VALUES ('in primary, before promotion')")
    ...
    rewind.primary_psql("INSERT INTO trunc_tbl values ('in primary, before promotion')")
    rewind.primary_psql(
    ...
    rewind.primary_psql("insert into drop_tbl values ('in primary, after promotion')")
    rewind.primary_psql("DROP TABLE drop_tbl")

    # Insert some data in the in-place tablespace for the old primary and the
    # standby.
    rewind.primary_psql("INSERT INTO space_tbl VALUES ('in primary, after promotion')")
    rewind.standby_psql("INSERT INTO space_tbl VALUES ('in standby, after promotion')")
    ...
    if not WINDOWS_OS:
        assert check_mode_recursive(
            rewind.node_primary.data_dir, 0o700, 0o600
