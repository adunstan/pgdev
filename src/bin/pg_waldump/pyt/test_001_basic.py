...
def _tar_portability_options(tar):
    ...
    # OpenBSD tar
    rc = subprocess.run(
        f"{tar} -F ustar -cf {devnull} {devnull} 2>{devnull}",
        shell=True,
        check=False,
    ).returncode
    if rc == 0:
...
def _generate_archive(tar, tar_p_flags, archive, directory, compression_flags):
    ...
    proc = subprocess.run(argv, cwd=directory, check=False)
...
"""
    ...
    rel_t1_oid = node.safe_sql("SELECT oid FROM pg_class WHERE relname = 't1'")
    rel_i1a_oid = node.safe_sql("SELECT oid FROM pg_class WHERE relname = 'i1a'")
    ...
    have_libz = check_pg_config(pg_config, "#define HAVE_LIBZ 1")
    ...
    for scenario in scenarios:
        ...
        assert (
            len([ln for ln in lines if not re.match(r"^rmgr: \w", ln)]) == 0
        ...
        assert (
            len([ln for ln in lines if not re.search(r"^rmgr:.*\bFPW\b", ln)]) == 0
        ...
        assert (
            len([ln for ln in lines if re.search(r"^rmgr:", ln)]) == 0
        ...
        assert (
            len([ln for ln in lines if not re.search(r"^rmgr: Btree", ln)]) == 0
        ), "only Btree lines"

        lines = _test_pg_waldump(pg_bin, path, start_lsn, end_lsn, "--fork", "init")
        assert (
            len([ln for ln in lines if not re.search(r"fork init", ln)]) == 0
        ...
        assert (
            len([ln for ln in lines if not relre.search(ln)]) == 0
        ...
        assert (
            len([ln for ln in lines if not re.search(r"\bblk 1\b", ln)]) == 0
