...
def _check_mode_recursive(
    directory, expected_dir_mode, expected_file_mode, ignore_list=None
):
...
def with_icu(pg_config):
    return check_pg_config(pg_config, "#define USE_ICU 1")


@pytest.fixture(scope="module")
def supports_syncfs(pg_config):
    return check_pg_config(pg_config, "#define HAVE_SYNCFS 1")
...
@pytest.mark.skipif(WINDOWS_OS, reason="group access not supported on Windows")
...
def test_set_case_insensitive(pg_bin, test_datadir):
    ...
    assert not re.search(
        r"^WORK_MEM = ", conf, re.MULTILINE
    ), "WORK_MEM should not be configured"
    assert not re.search(
        r"^Work_Mem = ", conf, re.MULTILINE
    ), "Work_Mem should not be configured"
    assert re.search(
        r"^work_mem = 512", conf, re.MULTILINE
