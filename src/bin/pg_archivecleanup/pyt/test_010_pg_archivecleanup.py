...
def test_pg_archivecleanup(pg_bin, tmp_path):
    ...
    for entry in WALFILES_VERBOSE:
        assert os.path.isfile(
            os.path.join(tempdir, entry["name"])
    ...
    def run_check(testdata, oldestkeptwalfile, test_name, *options):
        ...
        for entry in testdata:
            ...
            if entry["present"]:
                assert os.path.isfile(
                    path
                ), f"{test_name}:{entry['name']} was not cleaned up"
            else:
                assert not os.path.isfile(
                    path
                ), f"{test_name}:{entry['name']} was cleaned up"
        _remove_files(tempdir, testdata)

    run_check(
        WALFILES_WITH_GZ, "00000001000000370000000E", "pg_archivecleanup", "-x.gz"
    )
    run_check(
        WALFILES_WITH_GZ,
