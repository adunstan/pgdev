...
def test_createdb(pg_bin, create_pg):
    ...
    node.pg_bin.command_checks_all(
        ...
        1,
        [re.compile(r"^$")],
        [re.compile(r"^createdb: error: \"foo'; SELECT '1\" is not a valid "
                    r"encoding name", re.S)],
    ...
    node.issues_sql_like(
        ...
        re.compile(
            r"statement: CREATE DATABASE foobar6 STRATEGY wal_log "
            r"TEMPLATE foobar2"
        ),
    ...
    node.issues_sql_like(
        ...
        re.compile(
            r"statement: CREATE DATABASE foobar8 OWNER role_foobar "
            r"TEMPLATE foobar2"
        ),
