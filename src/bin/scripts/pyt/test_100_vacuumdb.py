...
"""Tests for vacuumdb option handling, the SQL it issues, and error cases."""
...
@pytest.fixture
def node(create_pg):
    """A started server with statement logging enabled (for issues_sql_like)."""
...
def test_vacuumdb_options(node):
    node.issues_sql_like(
        ["vacuumdb", "postgres"],
        re.compile(r"statement: VACUUM.*;"),
        "SQL VACUUM run",
    )
    node.issues_sql_like(
        ["vacuumdb", "-f", "postgres"],
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, FULL\).*;"),
        "vacuumdb -f",
    )
    node.issues_sql_like(
        ["vacuumdb", "-F", "postgres"],
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, FREEZE\).*;"),
        "vacuumdb -F",
    )
    node.issues_sql_like(
        ["vacuumdb", "-zj2", "postgres"],
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, ANALYZE\).*;"),
        "vacuumdb -zj2",
    )
    node.issues_sql_like(
        ["vacuumdb", "-Z", "postgres"],
        re.compile(r"statement: ANALYZE.*;"),
        "vacuumdb -Z",
    )
    node.issues_sql_like(
        ["vacuumdb", "--disable-page-skipping", "postgres"],
        re.compile(r"statement: VACUUM \(DISABLE_PAGE_SKIPPING, SKIP_DATABASE_STATS\).*;"),
        "vacuumdb --disable-page-skipping",
    )
    node.issues_sql_like(
        ["vacuumdb", "--skip-locked", "postgres"],
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, SKIP_LOCKED\).*;"),
        "vacuumdb --skip-locked",
    )
    node.issues_sql_like(
        ["vacuumdb", "--skip-locked", "--analyze-only", "postgres"],
        re.compile(r"statement: ANALYZE \(SKIP_LOCKED\).*;"),
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: VACUUM \(INDEX_CLEANUP FALSE, SKIP_DATABASE_STATS\).*;"),
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: VACUUM \(TRUNCATE FALSE, SKIP_DATABASE_STATS\).*;"),
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: VACUUM \(PROCESS_MAIN FALSE, SKIP_DATABASE_STATS\).*;"),
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: VACUUM \(PROCESS_TOAST FALSE, SKIP_DATABASE_STATS\).*;"),
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, PARALLEL 2\).*;"),
        "vacuumdb -P 2",
    )
    node.issues_sql_like(
        ["vacuumdb", "--parallel", "0", "postgres"],
        re.compile(r"statement: VACUUM \(SKIP_DATABASE_STATS, PARALLEL 0\).*;"),
        "vacuumdb -P 0",
    )
    node.command_ok(
...
def test_vacuumdb_tables_schemas(node):
    node.safe_sql(
        """
...
"""
    ...
    node.issues_sql_like(
        ...
        re.compile(r"GREATEST.*relminmxid.*2147483000"),
        "vacuumdb --table --min-mxid-age",
    )
    node.issues_sql_like(
        ["vacuumdb", "--min-xid-age", "2147483001", "postgres"],
        re.compile(r"GREATEST.*relfrozenxid.*2147483001"),
        "vacuumdb --table --min-xid-age",
    )
    node.issues_sql_like(
        ["vacuumdb", "--schema", '"Foo"', "postgres"],
        re.compile(r'VACUUM \(SKIP_DATABASE_STATS\) "Foo".bar'),
        "vacuumdb --schema",
    )
    ...
    node.issues_sql_like(
        ...
        re.compile(r'^(?!.*VACUUM \(SKIP_DATABASE_STATS\) "Foo".bar).*$', re.S),
    ...
    node.command_fails_like(
        ["vacuumdb", "--exclude-schema", "pg_catalog", "--table", "pg_class", "postgres"],
        re.compile(r"cannot vacuum specific table\(s\) and exclude schema\(s\) at the same time"),
        "cannot use options --exclude-schema and ---table at the same time",
    )
    node.command_fails_like(
        ["vacuumdb", "--schema", "pg_catalog", "--table", "pg_class", "postgres"],
        re.compile(r"cannot vacuum all tables in schema\(s\) and specific table\(s\) at the same time"),
        "cannot use options --schema and ---table at the same time",
    )
    node.command_fails_like(
        ["vacuumdb", "--schema", "pg_catalog", "--exclude-schema", '"Foo"', "postgres"],
        re.compile(r"cannot vacuum all tables in schema\(s\) and exclude schema\(s\) at the same time"),
        "cannot use options --schema and --exclude-schema at the same time",
    )
    node.issues_sql_like(
        ["vacuumdb", "--all", "--exclude-schema", "pg_catalog"],
        re.compile(r"(?:(?!VACUUM \(SKIP_DATABASE_STATS\) pg_catalog.pg_class).)*"),
    ...
    node.command_fails_like(
        ...
        re.compile(r"cannot vacuum all databases and a specific one at the same time"),
...
def test_missing_stats_only(node):
    node.safe_sql(
        """
...
"""
    ...
    node.issues_sql_unlike(
        ["vacuumdb", "--analyze-only", "--dry-run", "--missing-stats-only",
         "-t", "regression_vacuumdb_test", "postgres"],
        re.compile(r"statement:\ ANALYZE", re.S | re.X),
        "--missing-stats-only --dry-run",
    )
    node.issues_sql_like(
        ["vacuumdb", "--analyze-only", "--missing-stats-only",
         "-t", "regression_vacuumdb_test", "postgres"],
        re.compile(r"statement:\ ANALYZE", re.S | re.X),
...
def test_analyze_only_partitioned(node):
    node.safe_sql(
    ...
    node.issues_sql_like(
        ...
        re.compile(r"statement: ANALYZE public.parent_table", re.S),
    ...
    node.issues_sql_unlike(
        ["vacuumdb", "--analyze-only", "postgres"],
        re.compile(r"statement:\ VACUUM", re.S | re.X),
