...
def _combine_and_test_one_backup(
    node, original_backup_path, backup_name, failure_pattern, *extra_options
):
    ...
    if failure_pattern is not None:
        node.command_fails_like(
            ...
            failure_pattern,
            f"unable to verify backup {backup_name}",
        )
    else:
        node.command_ok(
            ["pg_verifybackup", revised_backup_path], f"verify backup {backup_name}"
        )
...
def test_004_manifest(create_pg):
    ...
    node.command_ok(["pg_verifybackup", original_backup_path], "verify original backup")
    ...
    _combine_and_test_one_backup(
        node, original_backup_path, "csum_none", None, "--manifest-checksums=NONE", MODE
    )
    _combine_and_test_one_backup(
        node,
        original_backup_path,
    ...
    assert sha224_count > 100, "SHA224 is mentioned many times in SHA224 manifest"
    ...
    nocsum_count = len(re.findall("Checksum-Algorithm", nocsum_manifest, re.M | re.I))
    assert (
        nocsum_count == 0
