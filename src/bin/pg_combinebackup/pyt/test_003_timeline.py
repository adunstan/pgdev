...
def _combine_backup(node, root_node, prior_backups, final_backup, combine_mode=None):
    ...
    prior_paths = [os.path.join(root_node.backup_dir, name) for name in prior_backups]
...
def test_003_timeline(create_pg, tmp_path):
    ...
    _combine_backup(node3, node1, ["backup1", "backup2"], "backup3", combine_mode=mode)
    node3.start()
