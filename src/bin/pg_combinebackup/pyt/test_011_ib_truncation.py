...
def test_011_ib_truncation(create_pg, tmp_path):
    ...
    target_blocks = 6
    block_size = int(primary.safe_sql("SELECT current_setting('block_size')::int;"))
...
"""
    ...
    primary.safe_sql(f"DELETE FROM t WHERE id > ({rows_after_truncation});")
    ...
    assert t_blocks == rows_after_truncation, "post-truncation row count as expected"
    assert t_blocks > target_blocks, "post-truncation block count as expected"

    # Take an incremental backup based on the full backup manifest
    primary.backup(
        "incr",
        backup_options=["--incremental", os.path.join(full_backup, "backup_manifest")],
    )
    ...
    assert (
        int(restored_count) == rows_after_truncation
