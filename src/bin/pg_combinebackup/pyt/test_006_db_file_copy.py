...
def test_006_db_file_copy(create_pg, tmp_path):
    ...
    # Initial setup.
    primary.safe_sql("CREATE DATABASE lakh OID = 100000 STRATEGY = FILE_COPY")
    ...
    restore.command_ok(
        ["pg_combinebackup", backup1path, backup2path, "--output", combined, mode],
        "combine backups",
    )
    ...
    assert (
        res.status == ExecStatusType.PGRES_FATAL_ERROR
    ), "SELECT * FROM t1: query should fail"
    assert res.psqlout == "", "SELECT * FROM t1: no stdout"
    assert 'relation "t1" does not exist' in (
