...
"""
    ...
    tsoids = [
        e
        for e in os.listdir(os.path.join(primary.data_dir, "pg_tblspc"))
        if re.match(r"^\d+", e)
    ]
...
"""
    ...
    pitr2.command_ok(
        [
            ...
            combinedpath,
            "--tablespace-mapping",
            f"{tsbackup2path}={tscombinedpath}",
            mode,
        ],
...
def _compare_dumps(dump1, dump2, msg):
    ...
    def _norm(line):
        return re.sub(
            r"(create tablespace .* location .*\btspitr)[12]",
            r"\1N",
            line,
            flags=re.IGNORECASE,
        )
