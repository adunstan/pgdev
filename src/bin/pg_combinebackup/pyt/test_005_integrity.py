...
def test_005_integrity(create_pg):
    ...
    node1 = create_pg("node1", start=False, has_archiving=True, allows_streaming=True)
    ...
    strangely_named_config_file = os.path.join(node1.data_dir, "INCREMENTAL.config")
    ...
    node2.command_ok(
        [
            ...
            "fast",
            "--incremental",
            os.path.join(backupother1path, "backup_manifest"),
        ],
    ...
    shutil.copy(
        os.path.join(backupother2path, "backup_manifest"),
        os.path.join(backup2path, "backup_manifest"),
    )
    node1.command_fails_like(
        [
