...
def test_008_untar(create_pg):
    ...
    primary.safe_sql("CREATE TABLE regress_tbl1(i int) TABLESPACE regress_ts1;")
    ...
    for tc in test_configuration:
        ...
        backup_files = ",".join(
            sorted(e for e in _slurp_dir(backup_path) if e not in (".", ".."))
        )
        expected_backup_files = ",".join(
            sorted(["backup_manifest", *tc["backup_archive"]])
        )
        assert (
            backup_files == expected_backup_files
