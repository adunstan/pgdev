...
def mutilate_extra_tablespace_file(backup_path):
    ...
    (catvdir,) = _slurp_dir_entries(os.path.join(backup_path, "pg_tblspc", tsoid))
    (tsdboid,) = _slurp_dir_entries(
        os.path.join(backup_path, "pg_tblspc", tsoid, catvdir)
    )
    _create_extra_file(
        backup_path, os.path.join("pg_tblspc", tsoid, catvdir, tsdboid, "extra_ts_file")
    )
...
# Heterogeneous per-scenario records: str names/regexps, callables for
# mutilate/cleanup hooks, and a bool flag.
SCENARIOS: List[Dict[str, Any]] = [
    {
        "name": "extra_file",
        "mutilate": mutilate_extra_file,
        "fails_like": r'extra_file.*present (on disk|in archive "[^"]+") '
        r"but not in the manifest",
    },
    {
        "name": "extra_tablespace_file",
        "mutilate": mutilate_extra_tablespace_file,
        "fails_like": r'extra_ts_file.*present (on disk|in archive "[^"]+") '
        r"but not in the manifest",
    },
    {
        "name": "missing_file",
        "mutilate": mutilate_missing_file,
        "fails_like": r"pg_xact\/0000.*present in the manifest but not "
        r'(on disk|in archive "[^"]+")',
    },
    {
        "name": "missing_tablespace",
        "mutilate": mutilate_missing_tablespace,
        "fails_like": r"pg_tblspc.*present in the manifest but not "
        r'(on disk|in archive "[^"]+")',
    },
    {
        "name": "append_to_file",
        "mutilate": mutilate_append_to_file,
        "fails_like": r'has size \d+ (on disk|in archive "[^"]+") '
        r"but size \d+ in the manifest",
    },
    {
        "name": "truncate_file",
        "mutilate": mutilate_truncate_file,
        "fails_like": r'has size 0 (on disk|in archive "[^"]+") '
        r"but size \d+ in the manifest",
    },
    {
        "name": "replace_file",
    ...
    },
    {
        ...
        "mutilate": None,
        "fails_like": r"manifest system identifier is .*, but control file has",
    },
    {
        "name": "bad_manifest",
...
def test_003_corruption(scenario, create_pg, tmp_path):
    ...
    primary.command_ok(
        [
            ...
            "fast",
            "--tablespace-mapping",
            f"{source_ts_path}={backup_ts_path}",
        ],
        "base backup ok",
    )
    ...
    if not scenario.get("needs_unix_permissions") and tar:
        ...
        for tsoid in tsoids:
            ...
            _tar_in(
                tar, tar_p_flags, tspath, os.path.join(tar_backup_path, f"{tsoid}.tar")
            )
        ...
        _tar_in(
            tar,
            tar_p_flags,
            os.path.join(backup_path, "pg_wal"),
        ...
        _tar_in(
            tar, tar_p_flags, backup_path, os.path.join(tar_backup_path, "base.tar")
        )
...
def _rmtree_like_perl(path):
    ...
    # onexc (the non-deprecated spelling) requires Python 3.12; the suite's
    # floor is 3.8, so stick with onerror.
