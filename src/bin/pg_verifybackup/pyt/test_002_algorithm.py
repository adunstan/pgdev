...
def _test_checksums(primary, fmt, algorithm):
    ...
    backup_path = "/".join([primary.backup_dir.replace("\\", "/"), fmt, algorithm])
    backup = [
        "pg_basebackup",
        "--pgdata",
        backup_path,
    ...
    if algorithm == "bogus":
        primary.command_fails(
            backup, f'{fmt} format backup fails with algorithm "{algorithm}"'
        )
        return

    # A backup with a valid algorithm should work.
    primary.command_ok(backup, f'{fmt} format backup ok with algorithm "{algorithm}"')
    ...
    if algorithm == "none":
        ...
        count_of_algorithm_in_manifest = len(
            re.findall(algorithm, manifest, re.M | re.I)
        )
        assert (
            count_of_algorithm_in_manifest > 100
        ), f"{algorithm} is mentioned many times in the manifest"

    # Make sure that it verifies OK.
    primary.command_ok(
        verify, f'verify {fmt} format backup with algorithm "{algorithm}"'
    )
